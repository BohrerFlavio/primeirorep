...
User Function MLR16()
	...
	Private limite        := 80
	Private tamanho       := "G"
	Private nomeprog      := "MLR16" // Coloque aqui o nome do programa para impressao no cabecalho
	Private nTipo         := 18
	...
	cQuery := " SELECT FT_EMISSAO, FT_NFISCAL, FT_CFOP, FT_PRODUTO, FT_CSTCOF, FT_CSTPIS, FT_POSIPI, FT_TOTAL, FT_DESCONT, A1_COD AS CODCLI, A1_LOJA AS LOJACLI, A1_NOME AS NOMECLI "
	cQuery += " FROM "  + RetSQLTab('SFT') + " , " + RetSQLTab('SA1')
	cQuery += " WHERE " + RetSQLFil('SFT') + " AND " + RetSQLFil('SA1') + " AND (FT_EMISSAO BETWEEN '" + DTOS(mv_par01) + "' AND '" + DTOS(mv_par02) + "')"
	cQuery += " AND A1_COD = FT_CLIEFOR AND A1_LOJA = FT_LOJA AND A1_TIPO = 'F'
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	While TMP->(!EOF())
		...
		@nlin,130 psay alltrim(TMP->CODCLI)
		@nlin,139 psay alltrim(TMP->LOJACLI)
		@nlin,145 psay substr(TMP->NOMECLI,1,30)
