...
/*/
User Function MLR110( cAlias, nReg, nOpcx )
	...
	While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == cFilSC7 .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
		...
		For nVias := 1 to mv_par09
			...
			Else
				...
				Else
					oReport:PrintText( STR0078 + If((SC7->C7_TIPO == 1 .OR. SC7->C7_TIPO == 3),IF(lRejeit,"P E D I D O   R E J E I T A D O",STR0051),STR0086) ,nLinPC,1390 )
				EndIf
			EndIf
			oReport:SkipLine()
			...
			If !lNewAlc
				...
				oReport:PrintText( STR0080 + IF( SC7->C7_TPFRETE $ "F","FOB",IF(SC7->C7_TPFRETE $ "C","CIF",IF(SC7->C7_TPFRETE $ "R","Por Conta Remetente",IF(SC7->C7_TPFRETE $ "D","Por Conta Destinatario",IF(SC7->C7_TPFRETE $ "T","Por Conta Terceiros"," " ) )))) ,nLinPC,1820 ) //Por conta remetente, Por conta destinatario, Por Conta Terceiros.
				oReport:SkipLine()
			...
			Else
				...
				oReport:PrintText( If((SC7->C7_TIPO == 1 .OR. SC7->C7_TIPO == 3), If( lLiber , STR0050 , IF(lRejeit,"P E D I D O   R E J E I T A D O",STR0051) ) , If( lLiber , STR0085 , STR0086 ) ),nLinPC,1290 ) //"     P E D I D O   L I B E R A D O"#"|     P E D I D O   B L O Q U E A D O !!!"
				...
				oReport:PrintText(       "|  "+"REJ:Rejeitado" ,nLinPC, 830 ) 	//"Ok:REJEITADO"
...
//"Local de Cobranca : "
	...
	@ li,104 PSAY AllTrim(GetMV("MV_SIMB"+cMoeda))
	@ li,108 PSAY xMoeda(nTotal,SC7->C7_MOEDA,MV_PAR12,SC7->C7_DATPRF,MsDecimais(SC7->C7_MOEDA),nTxMoeda) Picture tm(nTotal,12,MsDecimais(MV_PAR12))
	@ li,142 - nDifColCC PSAY "|"
	li++
	@ li,001 PSAY "|"
	...
	Else
		...
		@ li,104 PSAY AllTrim(GetMV("MV_SIMB"+cMoeda))
		@ li,108 PSAY xMoeda(nTotMerc,SC7->C7_MOEDA,MV_PAR12,SC7->C7_DATPRF,MsDecimais(SC7->C7_MOEDA),nTxMoeda) Picture tm(nTotMerc,12,MsDecimais(MV_PAR12))
	Endif
	@ li,142 - nDifColCC PSAY "|"
	li++
	@ li,001 PSAY "|"
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	Else
		If lLiber
			@ li,094 PSAY xMoeda(nTotalNF,SC7->C7_MOEDA,MV_PAR12,SC7->C7_DATPRF,MsDecimais(SC7->C7_MOEDA),nTxMoeda) Picture tm(nTotalNF,14,MsDecimais(MV_PAR12))
		Else
			@ li,080 PSAY (IF(lRejeit,"P E D I D O   R E J E I T A D O",STR0051))
		EndIf
	EndIf
	...
	Else
		...
		@ li,003 PSAY cObs04
		@ li,054 PSAY "|"
		@ li,059 PSAY IF(lLiber,STR0050,IF(lRejeit,"P E D I D O   R E J E I T A D O",STR0051))		//"     P E D I D O   L I B E R A D O"#"|     P E D I D O   B L O Q U E A D O !!!"
		@ li,102 PSAY STR0020		//"| Obs. do Frete: "
		@ li,119 PSAY Substr(RetTipoFrete(SC7->C7_TPFRETE),3,10)
		@ li,142 - nDifColCC PSAY "|"
		li++
