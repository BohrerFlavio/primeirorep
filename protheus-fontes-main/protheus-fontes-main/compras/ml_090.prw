...
User Function ML_090()

	/*/
	ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Programa  ³ ML_090   ³ Autor ³ Mario Zimmermann      ³ Data ³ 01.07.04 ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descricao ³ Emissao da relacao de compras                              ³±±
	±±³          ³                                                            ³±±
	...
	PRIVATE titulo   := "Relacao de Compras"
	PRIVATE cDesc1   := "Este relatorio ira imprimir a relacao de itens"
	PRIVATE cDesc2   := "referentes a compras efetuadas."
	PRIVATE cDesc3   := ""
	PRIVATE cPerg    := "MTR090"
	PRIVATE tamanho  := "G"
	PRIVATE aOrd     := {OemToAnsi(" Fornecedor         "),OemToAnsi(" Data De Digitacao  "),OemToAnsi(" Tipo+Grupo+Codigo  ") ,OemToAnsi(" Grupo+Codigo   ")}
	PRIVATE aReturn  := {OemToAnsi("Zebrado"),1,OemToAnsi("Administracao"), 1, 2, 1, "",1} 
	PRIVATE aLinha   := { }
	PRIVATE nomeprog := "ML_090"
	PRIVATE nLastKey := 0
	PRIVATE _xTotM3  := 0
	PRIVATE _xGruM3  := 0
	...
	//³ mv_par11             // Considera Estoque (S/N/A)  Inc. Silva³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funcao    ³ CA090Imp ³ Autor ³Alexandre Inacio Lemes ³ Data ³25/04/2002³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
Static Function C090Imp(lEnd,WnRel,cString)
	...
	If aTamSXG[1] <> aTamSXG[3]
		cabec1:= "  NOTA                                                            QUANTIDADE UM      PR.UNITARIO  IPI             VALOR  ICM  CODIGO               RAZAO SOCIAL               TP TES TIPO GRUPO DATA DIG.           CUSTO LO"
	...
	Else
		cabec1:= "  NOTA                                                            QUANTIDADE UM      PR.UNITARIO  IPI             VALOR  ICM  CODIGO RAZAO SOCIAL                             TP TES TP  GRUPO  DATA DIG.           CUSTO LO"
	...
	If mv_par07 == 1
		...
		dbSelectArea("SD2")
		IndRegua("SD2",cNomeArq1,"D2_COD+D2_NFORI+D2_ITEMORI+D2_SERIORI+D2_CLIENTE+D2_LOJA",,cCondSD2,"Selecionando Registros...")       
	...
	If (TcSrvType()#'AS/400')
		...
		cQuery += "SF4.F4_CODIGO = SD1.D1_TES AND "
		cQuery += "SF4.D_E_L_E_T_ <> '*' AND "
		If mv_par11==1  // Movta Estoque
			cQuery += "SF4.F4_ESTOQUE = 'S' AND "
		Endif
		If mv_par11==2  // Nao Movta Estoque
			cQuery += "SF4.F4_ESTOQUE = 'N' AND "
		Endif
		...
		MsAguarde({|| dbUseArea(.T.,"TOPCONN",TCGenQry(,,cQuery),cALiasSD1,.T.,.T.)},OemToAnsi("Seleccionado registros")) 
	...
	Else 
		...
		IndRegua("SD1",cNomeArq,cIndice,,Condicao,"Selecionando Registros...")     
	...
	While !Eof()
		...
		If lFiltro
			...
			If _xProdA <> (cAliasSD1)->D1_COD
				@ li, 000 PSAY "Produto.: "+Alltrim((cAliasSD1)->D1_COD)+" "+_xDescri
				li ++
			EndIf                                               
			_xProdA := (cAliasSD1)->D1_COD
			@ li,002 PSAY (cAliasSD1)->D1_DOC Picture PesqPict("SD1","D1_DOC",Len((cAliasSD1)->D1_DOC))
			//		@ li,013 PSAY Alltrim((cAliasSD1)->D1_COD)
			//		@ li,029 PSAY Substr(_xDescri,1,30)
	...
	If li <> 80
		If nTotal <> 0
			...
			@ li,002 PSAY "TOTAL GERAL       --> "
			@ li,060 PSAY nTotQt2   		Picture PesqPict("SD1","D1_QUANT",16)
			If _xTotM3 > 0
				@ li,077 PSAY "=> M3.: "+Alltrim(Str(_xTotM3,14,2))
			EndIf
			If cPaisLoc == "BRA"
...
Static Function ImpTotPr(nTotProd,cTipGrp,titulo,nOrdem,cProdAnt)
	If li > 60
		cabec(titulo+" ("+alltrim(aOrd[nOrdem])+")",cabec1,cabec2,nomeprog,tamanho,15)
	Endif
	@ li,002 PSAY "TOTAL PRODUTO     --> "
	@ li,060 PSAY nTotQt   		Picture PesqPict("SD1","D1_QUANT",16)
	_MetCubP := fBuscaCpo("SB1",1,xFilial("SB1")+cProdAnt,"B1_METCUBP")
	If _MetCubP > 0
		@ li, 077 Psay "=> M3.: "+Alltrim(Str((_MetCubP * nTotQt),14,2))
		_xTotM3 += (_MetCubP * nTotQt)
		_xGruM3 += (_MetCubP * nTotQt)
	EndIf
	If cPaisLoc == "BRA"
...
Static Function ImpTotGrp(nTotGrp,cGrupo,cTipAnt,titulo,nOrdem,nTotQt1,nTotVal1,aTotais)
	If li > 60
		cabec(titulo+" ("+alltrim(aOrd[nOrdem])+")",cabec1,cabec2,nomeprog,tamanho,15)
	Endif
	@ li,002 PSAY "TOTAL GRUPO "+cGrupo+"  --> "		
	@ li,060 PSAY nTotQt1  			Picture PesqPict("SD1","D1_QUANT",16)
	If _xGruM3 > 0
		@ li,077 PSAY "=> M3.: "+Alltrim(Str(_xGruM3,14,2))
		_xGruM3 := 0
	EndIf
...
Static Function ImpTotDt(nTotCus,titulo,nOrdem,nTotQt,nTotVal,cQuebraDtDig,aTotais)
	...
	Endif
	If nTotVal <> 0
		@ li,000 PSAY "TOT.DA DATA "+DTOC(cQuebraDtDig) 
		@ li,060 PSAY nTotQt               Picture PesqPict("SD1","D1_QUANT",16)
		If cPaisLoc == "BRA"
			@ li,103 PSAY nTotVal           Picture PesqPict("SD1","D1_TOTAL",16,mv_par09)
		Else
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
// Substituido pelo assistente de conversao do AP5 IDE em 21/11/00 ==> Function ValidPerg
Static Function ValidPerg()
	...
	AADD(aRegs,{cPerg,"11","Movto Estoque   ?","","","mv_chc","N",01,0,0,"C","","mv_par11","Sim","","","","","Nao","","","","","Ambos","","","","","","","","","","","","","","",""})
