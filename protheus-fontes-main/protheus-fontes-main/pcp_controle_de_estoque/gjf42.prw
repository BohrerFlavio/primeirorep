...
±±ºPrograma  ³GJF42     º Autor ³ Giuliano Forgiariniº Data ³  30/08/08   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Relatorio de Posição de Estoque de PA                      º±±
±±º          ³                                                            º±±
...
User Function GJF42()
	...
	Private limite           := 80
	Private tamanho          := "M"
	Private nomeprog         := "GJF42" // Coloque aqui o nome do programa para impressao no cabecalho
	Private nTipo            := 18
	...
	Private CONTFL     := 01
	Private m_pag      := 01
	Private wnrel      := "GJF42" // Coloque aqui o nome do arquivo usado para impressao em disco    
	...
	cQuery += " AND B1_TIPO IN('PR','PA') AND B1_FILIAL = '" + xFilial("SB1") + "' AND SBM010.D_E_L_E_T_ <> '*'  " 
...
Static Function Transfere(_cod)
	...
	cQuery2 := "SELECT Z8_FILORI, COUNT(*) AS QUANT, SUM(Z8_PESO) AS PESO " +;
	" FROM " + RetSqlName("SZ8")  + " SZ8 " + " WHERE SZ8.D_E_L_E_T_ <> '*'  " +;
	" AND SZ8.Z8_FILIAL = '"+ xfilial("SZ8") + "' AND  SZ8.Z8_FIL <> '"+ cFilAnt + "' AND " +;
	"  SZ8.Z8_COD = '" + alltrim(_des) + "' AND "+;
	"  (Z8_DTRANSF BETWEEN '" + dtos(mv_par01) + "' AND '" + dtos(mv_par02) + "' )" +;
	"  GROUP BY Z8_FILORI "
	...
	if SZ8->Z8_FILORI <> xfilial('SB1')
		...
		" AND Z8_FILIAL = '"+ xfilial("SZ8") + "' AND  Z8_FIL = '"+cFilAnt + "' AND " +;
