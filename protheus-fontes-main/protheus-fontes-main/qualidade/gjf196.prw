...
±±ºPrograma  ³GJF196   º Autor ³ Giuliano Forgiarini  º Data ³  12/01/15  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Relatório de Conferência de devoluções                     º±±
±±º          ³                                                            º±±
...
User Function GJF196()
	...
	Local cDesc2         := "de conferencia de devoluções de produto acabado     "
	Local cDesc3         := "executadas e controladas pelo Setor d Qualidde da   "
	Local cPict          := "empresa"
	Local titulo         := "CONFERENCIA DE DEVOLUÇOES DE PRODUTO ACABADO"
	Local nLin           := 80

	Local Cabec1         := space(25) + "Numero      Codigo      Codigo   Descrição       Peso      Data       Data         Nota"                   
	Local Cabec2         := space(25) + "Devolução   Caixa       Produto  Produto         Liquido   Saída      Prod.        Serie"
	...
	_cDest := iif(mv_par03 = 1,"'T'",iif(mv_par03 = 2,"'E'",iif(mv_par03 = 3,"'R','P'",iif(mv_par03 = 4,"'C'","'G'"))))
	...
	cQuery += " AND ZN_DTENTR BETWEEN '" + dtos(mv_par01) + "' AND '" + dtos(mv_par02) + "' "
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	While DEV->(!EOF())
		...
		if _cDestino <> DEV->ZN_DESTINO      
			...
			iif(DEV->ZN_DESTINO  = 'R','Repesagem',;
			iif(DEV->ZN_DESTINO  = 'P','Reprocesso',;
			iif(DEV->ZN_DESTINO  = 'C','Charque','Graxaria'))))
			nlin++
			_cDestino := DEV->ZN_DESTINO
		endif 

		if !empty(DEV->ZN_CONTROL)		   
			_cPredes := fBuscaCpo('SZ8',3,xFilial('SZ8') + DEV->ZN_CONTROL,'Z8_PREDES')
			_dDtProd := fBuscaCpo('SZ2',2,xFilial('SZ2') + _cPredes, 'Z2_DATAABT')	
		endif

		@nlin,020 psay DEV->ZN_NUM  
		@nlin,032 psay DEV->ZN_CONTROL
		@nlin,044 psay substr(DEV->ZN_COD,1,6)
		@nlin,052 psay substr(DEV->ZN_DESC,1,25)
		@nlin,075 psay transform(DEV->ZN_PESOL,'@E 999.99')
		@nlin,085 psay dtoc(stod(DEV->ZN_DTSAIDA))
		if !empty(DEV->ZN_CONTROL)
			@nlin,092 psay dtoc(_dDtProd)
		endif      
		@nlin,105 psay alltrim(DEV->ZN_DOC) + '/' + alltrim(DEV->ZN_SERIE)
