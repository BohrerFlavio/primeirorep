...
//--------------------------------------------------------------------------------------
/*/{Protheus.doc} ETWRAIOX 
@Type			: Função de Usuário
@Sample			: U_ETQRAIOX()
@Description	: Rotina para geração de etiquetas Raio X a partir dos dados do servidor
                  postgreSQL
@Param			: Nulo
@Return			: Nulo
@ --------------|-----------------------------------------------------------------------
...
Static Function ETQRAIOX()
	...
	Else
		...
		cQuery := "SELECT * "
		cQuery += "  FROM object" 
		cQuery += " WHERE objectkey > 96360" 
		cQuery += "   AND bankidx = -1" 
		
		cQuery := "SELECT * "
		cQuery += "  FROM result" 
		cQuery += " WHERE resultkey = 195406" 
		*/

		// Função de entrada - sugestão com leitura de pré etiqueta
			
		// localizar produto para filtrar na batch

		// BUscar o registro por produto
		/*
		cProd := str(26)
		cQuery := "SELECT * "
		cQuery += "  FROM batch" 
		cQuery += " WHERE productkey = "+ cProd
		cQuery += " order by batchkey DESC limit 1"
		*/
		// Buscar o produto
		...
		cQuery := "SELECT *"
		cQuery += "  FROM object" 
		cQuery += " WHERE productkey = "+ str(cProd)
...
User function LerPreE()
	...
	*/
	...
	_oGet1   := TGet():New(45,30, {|u| If(PCount() > 0, cPreE:= u, cPreE)}, oDlg,, 009, "@!",{||ETQRAIOX()}, 0,,, .F.,, .T.,, .F.,, .F., .F.,, .F., .F.,, cPreE,,,,.t.,)
