...
/*/{Protheus.doc} IMP_ZB1
Rotina efetua importação da tabela de espécie (ZB1) conforme arquivo Excel (.CSV)
@author     Evandro
@since      19/10/2020
@return     Nil
@obs        N/A
...
User Function IMP_ZB1()
	...
	Private _aDados    := {}
	Private _cCampo    := ""
	Private _cDlgTab   := "ZB1"	
	Private _cNL       := CHR(13) + CHR(10)
	Private _oDlgArq
	Private _oDlgTab

	// Inicio da tela de importacao
	DEFINE MSDIALOG _oDlg FROM 001,001 TO 140,450 TITLE "Importação Espécies Novas para Tabela ZB1" PIXEL
	...
	_oBtn1:cToolTip := "Importar dados do arquivo para atualizar a tabela de espécie ZB1."
...
Static Function _PREPARIMP()
	...
	If MsgYesNo("Leitura Concluída com Sucesso." + _cNl + "Deseja continuar e realizar a atualização dos registros?")
		...
		MsgAlert("Importação Concluída com Sucesso!" + _cNl + "Verifique a Situação da Importação no Arquivo de LOG em C:\Temp\LogZB1_Especie.LOG")
	Else
...
Static Function _IMPORTA()
	...
	For _nPos := 1 To Len(_aDados)
		...
		IncProc("Incluindo registro " + cValToChar(_nNumLendo) + " de " + cValToChar(_nNumReg) + " na tabela ZB1...")

		DbSelectArea("ZB1")
		ZB1->(dbSetOrder(1))
		ZB1->(dbGoTop())
		If !ZB1->(dbSeek(xFilial("ZB1")+_aDados[_nPos,1])) 

			/*
			DbSelectArea("ZB1")
			RecLock("ZB1", .T.)				
			ZB1->ZB1_FILIAL := xFilial("ZB1")
			ZB1->ZB1_CODIGO := _aDados[_nPos,1]
			ZB1->ZB1_DESCRI := _aDados[_nPos,2]
			ZB1->ZB1_VUTIL  := Val( StrTran( AllTrim(_aDados[_nPos,3]), ",", "." ) )
			ZB1->ZB1_PERCDP := Val( StrTran( AllTrim(_aDados[_nPos,4]), ",", "." ) )
			MsUnlock()
			*/
			_LOG("INCLUIDO ZB1         => CODIGO: " + _aDados[_nPos,1] + " - DESCRI: " + _aDados[_nPos,2] + " - VUTIL " + _aDados[_nPos,3] + " - TAXA: " +  _aDados[_nPos,4] )
		Else

			_LOG("ANTES ALTERACAO ZB1  => CODIGO: " + ZB1->ZB1_CODIGO + " - DESCRI: " + ZB1->ZB1_DESCRI + " - VUTIL " + Transform(ZB1->ZB1_VUTIL, "@E 999.99") + " - TAXA: " +   Transform(ZB1->ZB1_PERCDP, "@E 999.99999999") )
			...
			_LOG("DEPOIS ALTERACAO ZB1 => CODIGO: " + _aDados[_nPos,1] + " - DESCRI: " + _aDados[_nPos,2] + " - VUTIL " + _aDados[_nPos,3] + " - TAXA: " +  _aDados[_nPos,4] )
			_LOG("")
		EndIf
...
Static Function _LOG(_sTexto)
	...
	Local _nHdl     := 0
	Local _cDir     := "C:\Temp\"
	Local _cNomeArq := "LogZB1_Especie_" + cEmpAnt + ".LOG"
	Local _sArqLog  := ""
