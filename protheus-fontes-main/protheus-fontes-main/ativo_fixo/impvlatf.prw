...
Rotina efetua importação dos valroes de depreciação Moeda 1 e 3 informada em arquivo Excel (.CSV)
@author     Evandro
@since      18/06/2020
@return     Nil
@obs        N/A
...
User Function IMPVLATF()
	...
	DEFINE MSDIALOG _oDlg FROM 001,001 TO 140,450 TITLE "Importação Valores Depreciação M1 e M3 para Tabela SN3" PIXEL
	...
	_oBtn1:cToolTip := "Importar dados do arquivo para atualizar valores depreciação M1 e M3 na tabela SN3."
...
Static Function _PREPARIMP()
	...
	If MsgYesNo("Leitura Concluída com Sucesso." + _cNl + "Deseja continuar e realizar a atualização dos registros?")
		...
		MsgAlert("Importação Concluída com Sucesso!" + _cNl + "Verifique a Situação da Importação no Arquivo de LOG em C:\Temp\LogSN3_Valores.LOG")
	Else
...
Static Function _IMPORTA()
	...
	For _nPos := 1 To Len(_aDados)
		...
		If SN3->(dbSeek(_aDados[_nPos,1] + _aDados[_nPos,2] + _aDados[_nPos,3] + _cTipo + "0")) 
			...
			DbSelectArea("SN3")
			RecLock("SN3", .F.)				
			SN3->N3_VRDBAL1 := Val( StrTran( AllTrim(_aDados[_nPos,4]), ",", "." ) )
			SN3->N3_VRDMES1 := Val( StrTran( AllTrim(_aDados[_nPos,5]), ",", "." ) )
			SN3->N3_VRDACM1 := Val( StrTran( AllTrim(_aDados[_nPos,6]), ",", "." ) )
			SN3->N3_VRDBAL3 := Val( StrTran( AllTrim(_aDados[_nPos,7]), ",", "." ) )
			SN3->N3_VRDMES3 := Val( StrTran( AllTrim(_aDados[_nPos,8]), ",", "." ) )
			SN3->N3_VRDACM3 := Val( StrTran( AllTrim(_aDados[_nPos,9]), ",", "." ) )
			MsUnlock()

			_LOG("SUCESSO            => FILIAL: " + _aDados[_nPos,1] + " - CODIGO BEM: " + _aDados[_nPos,2] + " - ITEM: " + _aDados[_nPos,3] + ;
			     " - ATUALIZADOS -> Depre Bal M1:" +  _aDados[_nPos,4] + " - Depr Mes M1:" +  _aDados[_nPos,5] + " - Depr.Acum M1:" +  _aDados[_nPos,6] + ;
			     " - ATUALIZADOS -> Depre Bal M3:" +  _aDados[_nPos,7] + " - Depr Mes M3:" +  _aDados[_nPos,8] + " - Depr.Acum M3:" +  _aDados[_nPos,9] )
		Else
			_LOG("NÃO ENCONTRADO SN3 => FILIAL: " + _aDados[_nPos,1] + " - CODIGO BEM: " + _aDados[_nPos,2] + " - ITEM: " + _aDados[_nPos,3] + ;
			     " - ATUALIZADOS -> Depre Bal M1:" +  _aDados[_nPos,4] + " - Depr Mes M1:" +  _aDados[_nPos,5] + " - Depr.Acum M1:" +  _aDados[_nPos,6] + ;
			     " - ATUALIZADOS -> Depre Bal M3:" +  _aDados[_nPos,7] + " - Depr Mes M3:" +  _aDados[_nPos,8] + " - Depr.Acum M3:" +  _aDados[_nPos,9] )
		EndIf
...
Static Function _LOG(_sTexto)
	...
	Local _nHdl     := 0
	Local _cDir     := "C:\Temp\"
	Local _cNomeArq := "LogSN3_Valores_Empresa_" + cEmpAnt + ".LOG"
	Local _sArqLog  := ""
