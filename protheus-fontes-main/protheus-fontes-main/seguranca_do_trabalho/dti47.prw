...
User Function DTI47()
	...
	Local Cabec1         := " Cent.Custo/Setor Matricula  Nome                                   Funcao         CBO2002   Sexo      PIS             CPF           RG         Dt.Nasc.       Dt.Admiss.     Dt.Demiss.  Sit.Folha        Peric/Insal"
	Local Cabec2         := ""
	Local imprime        := .T.
	Local aOrd           := {}
	Private nLin         := 80
	...
	Private limite       := 132
	Private tamanho      := "M"
	Private nomeprog     := "DTI47" // Coloque aqui o nome do programa para impressao no cabecalho
	Private nTipo        := 18
	...
	Private CONTFL       := 01
	Private m_pag        := 01
	Private wnrel        := "DTI47" // Coloque aqui o nome do arquivo usado para impressao em disco
	Private _aPrd 		 := {}
	pergunte(cPerg,.F.)
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	While TMP->(!EOF())
		...
		//@nlin,130 psay TMP->RA_RG
		//@nlin,145 psay dtoc(stod(TMP->RA_NASC))	
		//@nlin,160 psay dtoc(stod(TMP->RA_ADMISSA))
		//@nlin,175 psay dtoc(stod(TMP->RA_DEMISSA))
		//@nlin,188 psay iif(empty(TMP->RA_SITFOLH), 'Normal',;
		//			   iif(TMP->RA_SITFOLH = 'D', 'Demitido',;
		//			   iif(TMP->RA_SITFOLH = 'T', 'Transferido',;
		//			   iif(TMP->RA_SITFOLH = 'F', 'Ferias',;
		//			   iif(TMP->RA_SITFOLH = 'A', 'Afastado','')))))


		//if TMP->RA_ADCPERI = '2'

		//	@nlin,202 psay 'Peric.: ' + transform(TMP->RA_PERICUL,'@E 999.99')

		//elseif TMP->RA_ADCINS <> '1'

		//			@nlin,202 psay iif(TMP->RA_ADCINS =  '2', 'Ins.Min: ' + transform(TMP->RA_INSMAX,'@E 999.99'),;
