...
While !Eof() .And. &( cWhileTMT )
	...
	While !Eof() .And. &( cWhile2TMT )
		...
		cMatricula := '0' + Alltrim(SubStr(TMT->TMT_CODUSU,2,6))
		cNome := GetAdvFval('TMK','TMK_NOMUSU',FWxFilial('TMK') + TMT->TMT_CODUSU,1)
		oPrint:Say(lin + 5, 60, 'Médico/Fisioterapeuta:', oFont07)
		oPrint:Say(lin + 30, 100, Alltrim(cNome), oFont08)
		...
		If !Empty( TMT->TMT_OUTROS ) .And. ( MLCOUNT( TMT->TMT_OUTROS, nTam ) >= 4 .Or. !Empty( Memoline( TMT->TMT_OUTROS, nTam, nMemo ) ) )
			nMemo++
...
ElseIf File( "\SYSTEM\LGRL" + cSMCOD + cSMFIL + ".BMP" )
	oPrint:SayBitMap( 60, 1650, "\SYSTEM\LGRL" + cSMCOD + cSMFIL + ".BMP", 550, 180 )
ElseIf File( "\SYSTEM\LGRL" + cSMCOD + ".BMP" )
	oPrint:SayBitMap( 60, 1650, "\SYSTEM\LGRL" + cSMCOD + ".BMP", 550, 180 )
EndIf
