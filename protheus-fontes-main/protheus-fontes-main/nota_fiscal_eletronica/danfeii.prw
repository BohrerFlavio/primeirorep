...
#DEFINE MAXMENLIN  100					// Máximo de caracteres por linha de dados adicionais
...
If _cTipoNFE == "S"
	If !SF2->F2_TIPO $ "DB"
		...
		If DbSeek(xFilial("SA1") + SF2->F2_CLIENTE + SF2->F2_LOJA)
			_cTelefone := "(" + AllTrim(SA1->A1_DDD) + ") " + AllTrim(SA1->A1_TEL) + IIF(!Empty(SA1->A1_FAX), " / " + AllTrim(SA1->A1_FAX), "")
		Endif
	Else
		DbSelectArea("SA2")
		DbSetOrder(1)
		...
		If DbSeek(xFilial("SA2") + SF2->F2_CLIENTE + SF2->F2_LOJA)
			_cTelefone := "(" + AllTrim(SA2->A2_DDD) + ") " + AllTrim(SA2->A2_TEL) + IIF(!Empty(SA2->A2_FAX), " / " + AllTrim(SA2->A2_FAX), "")
		Endif
	Endif
Else
	If !SF1->F1_TIPO $ "DB"
...
IIF(Type("oDestino:_EnderDest:_fone")=="U","",_cTelefone),;    // oDestino:_EnderDest:_fone:Text),; Alterado padrão - SILVA
...
If _cTipoNFE == "S"
	If SF2->F2_TIPO == "I"
		aTotais[11] := Transform(0,"@e 999,999,999.99")
	Else
		aTotais[11] := Transform(Val(oTotal:_ICMSTOT:_vNF:TEXT) - SF1->F1_CONTSOC - SF1->F1_VALIMP1 ,"@e 999,999,999.99")
	Endif
Else
	If SF1->F1_TIPO == "I"
		aTotais[11] := Transform(0,"@ze 999,999,999.99")
	Else
		//aTotais[11] := Transform(Val(oTotal:_ICMSTOT:_vNF:TEXT) - SF1->F1_CONTSOC - SF1->F1_VALIMP1 ,"@e 999,999,999.99")
...
If Type("oTransp:_Transporta")<>"U"
	...
	//---------------- Início bloco personalizado - SILVA
	/* Para Impressão das placas quando origem RS e destino fora do estado*/
	//aTransp[04] := IIf(Type("oTransp:_VeicTransp:_Placa:TEXT")<>"U",oTransp:_VeicTransp:_Placa:TEXT,"")
	if Empty(mv_par09) 
		aTransp[04] := IIf(Type("oTransp:_VeicTransp:_Placa:TEXT")<>"U",oTransp:_VeicTransp:_Placa:TEXT,"") + "  " + IIf(Type("oTransp:_reboque:_Placa:TEXT")<>"U",oTransp:_reboque:_Placa:TEXT,"")
...
If Type("oTransp:_Vol")<>"U"
	If ValType(oTransp:_Vol) == "A"
		...
		For nX := 1 to nLenVol
			...
			Do Case
				Case nX == 1
					nVolume1 := IIF(!ValAtrib("oTransp:_Vol[nXAux]:_QVOL:TEXT")=="U",Val(oTransp:_Vol[nXAux]:_QVOL:TEXT),0)
					cEspeci1 := IIf(ValType("oTransp:_Vol[nXAux]:_Esp")=="U","",oTransp:_Vol[nXAux]:_Esp:TEXT)
				Case nX == 2
					nVolume2 := IIF(!ValAtrib("oTransp:_Vol[nXAux]:_QVOL:TEXT")=="U",Val(oTransp:_Vol[nXAux]:_QVOL:TEXT),0)
					cEspeci2 := IIf(ValType("oTransp:_Vol[nXAux]:_Esp")=="U","",oTransp:_Vol[nXAux]:_Esp:TEXT)
				Case nX == 3
					nVolume3 := IIF(!ValAtrib("oTransp:_Vol[nXAux]:_QVOL:TEXT")=="U",Val(oTransp:_Vol[nXAux]:_QVOL:TEXT),0)
					cEspeci3 := IIf(ValType("oTransp:_Vol[nXAux]:_Esp")=="U","",oTransp:_Vol[nXAux]:_Esp:TEXT)
				Case nX == 4
					nVolume4 := IIF(!ValAtrib("oTransp:_Vol[nXAux]:_QVOL:TEXT")=="U",Val(oTransp:_Vol[nXAux]:_QVOL:TEXT),0)
					cEspeci4 := IIf(ValType("oTransp:_Vol[nXAux]:_Esp")=="U","",oTransp:_Vol[nXAux]:_Esp:TEXT)
			EndCase
			//---------------- Fim bloco personalizado - SILVA
		Next nX
		...
		cVolumes := ""
		If nVolume1 > 0
			cVolumes += AllTrim(str(nVolume1))
		Endif
		If nVolume2 > 0
			cVolumes += " / " + AllTrim(str(nVolume2))
		Endif
		If nVolume3 > 0
			cVolumes += " / " + AllTrim(str(nVolume3))
		Endif
		If nVolume4 > 0
			cVolumes += " / " + AllTrim(str(nVolume4))
		Endif
		cEspecie := ""
		If !Empty(cEspeci1)
			cEspecie += AllTrim(cEspeci1)
		Endif
		If !Empty(cEspeci2)
			cEspecie += " / " + AllTrim(cEspeci2)
		Endif
		If !Empty(cEspeci3)
			cEspecie += " / " + AllTrim(cEspeci3)
		Endif
		If !Empty(cEspeci4)
			cEspecie += " / " + AllTrim(cEspeci4)
		Endif
		//---------------- Fim bloco personalizado - SILVA
		...
		aTransp[11] := AllTrim(cVolumes)
		aTransp[12] := AllTrim(cEspecie)

		If _cTipoNfe == "E"
			aTransp[13] := cMarca
		Else
			aTransp[13] := SF2->F2_MARCA
		Endif
...
For nZ := 1 To nLenDet
	...
	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_TOTAL")[1],TamSX3("D2_TOTAL")[2]))))		// Ajuste para impressão de dois campos após a virgula - SILVA
	//AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_PRCVEN")[2]))))
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_TOTAL")[1],TamSX3("D2_TOTAL")[2]))))		// Ajuste para impressão de dois campos após a virgula - SILVA
...
//---------------- Início bloco personalizado - SILVA
// Bloco para inserir a data de emissao no recibo de entrega
If oNF:_INFNFE:_VERSAO:TEXT >= "3.10"
	oDanfe:Say(034, 510, "DATA EMIS. "+ConvDate(oIdent:_DhEmi:Text), oFont07:oFont) 
Else
	oDanfe:Say(034, 510, "DATA EMIS. "+ConvDate(oIdent:_DEmi:Text), oFont07:oFont) 
Endif	
//---------------- Fim bloco personalizado - SILVA
...
Else
	...
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
	For nX := 1 To nForTo
		oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*24)+1),23), oFont12N:oFont )
		nLinCalc+=10
	Next nX
Endif	
...
//---------------- Início bloco personalizado - SILVA
// Bloco para informar UF quando houver mensagem na NF 
Local cCNPJCPF := oNF:_InfNfe:_DEST:_CNPJ:Text
Local	cUF      := aUF[aScan(aUF,{|x| x[1] == oNFe:_NFE:_INFNFE:_DEST:_ENDERDEST:_UF:Text})][02]
Local	cDataEmi := Substr(oNFe:_NFE:_INFNFE:_IDE:_DHEMI:Text,9,2)
Local	cTPEmis  := oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT
Local	cValIcm  := StrZero(Val(StrTran(oNFe:_NFE:_INFNFE:_TOTAL:_ICMSTOT:_VNF:TEXT,".","")),14)
Local	cICMSp   := iif(Val(oNFe:_NFE:_INFNFE:_TOTAL:_ICMSTOT:_VICMS:TEXT)>0,"1","2")
Local	cICMSs   := iif(Val(oNFe:_NFE:_INFNFE:_TOTAL:_ICMSTOT:_VST:TEXT)>0,"1","2")
local cLogoTotvs 	:= "Powered_by_TOTVS.bmp"
local cStartPath 	:= GetSrvProfString("Startpath","")
Local lNFCE 		:= Substr(oNFe:_NFe:_InfNfe:_ID:Text,24,2) == "65"
//---------------- Fim bloco personalizado - SILVA
