...
Local _j            := 0 	//Incluido - SILVA
Local _n			:= 0 	//Incluido - SILVA
Local _nNFDV		:= 0 	//Incluido - SILVA
Local _nNFCM		:= 0 	//Incluido - SILVA
Local i				:= 0 	//Incluido - SILVA
Local nA			:= 0
Local nX         	:= 0
Local nZ			:= 0
Local nCon       	:= 1  
...
Private _lExistEDI  := .f. 	//Incluido - SILVA
Private _cnf 	    := "" 	//Incluido - SILVA
Private lDifal		:= .F.
Private lDifer		:= .F.
...
If cTipo == "1"
	...
	If FindFunction("LjR30Sped")
		...
		dbSelectArea("SF2") 
		dbSetOrder(1)
		If MsSeek(FWxFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(FWxFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			dbSelectArea("SF3")
			dbSetOrder(4)
			If MsSeek(FWxFilial("SF3")+cClieFor+cLoja+cNota+cSerie)
				While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
					...
					MsSeek(FWxFilial("SFT")+"S"+SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_SERIE+SF3->F3_NFISCAL+SF3->F3_IDENTFT)
					
					dbSelectArea("SD2")
					dbSetOrder(3)
					MsSeek(FWxFilial("SD2")+SFT->FT_NFISCAL+SFT->FT_SERIE+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_PRODUTO)
					dbSelectArea("SF4")
					dbSetOrder(1)
					MsSeek(FWxFilial("SF4")+SD2->D2_TES)
					If SF3->F3_TIPO =="S"
						If SF3->F3_RECISS =="1"
							cSitTrib := "R"
					...
					dbSelectArea("SB1")
					dbSetOrder(1)
					MsSeek(FWxFilial("SB1")+SD2->D2_COD)
					...
					dbSelectArea("SD2")
					dbSetOrder(3)
					MsSeek(FWxFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA)
					
					dbSelectArea("SA1")
					dbSetOrder(1) 
					MsSeek(FWxFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA)
					
					cTpPessoa	:= SA1->A1_TPESSOA
						
					If nCont == 1
						Do While !SD2->(Eof ()) .And. FWxFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
						...
						dbSetOrder(3)
		   				...
		   				dbSelectArea("CD2")
						dbSetOrder(1)
						If MsSeek(FWxFilial("CD2")+"S"+SF2->F2_SERIE+SF2->F2_DOC+cClieFor+cLoja+PadR(SD2->D2_ITEM,4)+(cAliasSD2)->D2_COD)
			...
			ElseIf FunName() == "SPEDNFSE"
				...
				aadd(aDest,AllTrim(SA1->A1_NOME) + "  " + SA1->A1_COD + "-" + SA1->A1_LOJA)  	// Alterado - SILVA
				...
				If !Upper(SA1->A1_EST) == "EX"
					SC6->(dbSetOrder(4))
					SC5->(dbSetOrder(1))
					If (SC6->(MsSeek(FWxFilial("SC6")+SF2->F2_DOC+SF2->F2_SERIE)))
						...
						Else
							SA2->(dbSetOrder(1))
							SA2->(MsSeek(FWxFilial("SA2")+SC5->C5_FORNISS+"00"))
				...
				dbSelectArea("SF3")
				dbSetOrder(4)
				MsSeek(FWxFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
				
				While !Eof() .And. FWxFilial("SF3") == SF3->F3_FILIAL .And.;
					...
					dbSelectArea("SE1")
					SE1->(dbSetOrder(2))
					If SE1->(MsSeek(FWxFilial("SE1")+SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_SERIE+SF3->F3_NFISCAL))
						While !SE1->(Eof()) .And. FWxFilial("SE1") == SE1->E1_FILIAL .And.;
					...
					If len(aSX560) > 0
						...
						If cDescServ$"1"
							SC6->(dbSetOrder(4))
							SC5->(dbSetOrder(1))
							MsSeek(FWxFilial("SC6")+SF3->F3_NFISCAL+SF3->F3_SERIE)
							MsSeek(FWxFilial("SC5")+SC6->C6_NUM)
			...
			Else
				...
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			  	...
			  	If ("CF" $ SF2->F2_ESPECIE .OR. "NFCE" $ SF2->F2_ESPECIE .OR. "SATCE" $ SF2->F2_ESPECIE .OR. (LjAnalisaLeg(18)[1] .AND. "ECF" $ SF2->F2_ESPECIE .AND. ("S" $ SF2->F2_ECF) )) .AND. !Empty(SF2->F2_NFCUPOM) 
					...
					If MsSeek( FWxFilial("SF2") + cNumNfCup + cSerNfCup)
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					If (SF2->(ColumnPos("F2_CLIRET")) > 0 .And. SF2->(ColumnPos("F2_LOJARET")) > 0) .And. !Empty(SF2->F2_CLIRET+SF2->F2_LOJARET) .And. SF2->F2_CLIRET+SF2->F2_LOJARET<>SF2->F2_CLIENTE+SF2->F2_LOJA
					    dbSelectArea("SA1")
						dbSetOrder(1)
						If MsSeek(FWxFilial("SA1")+SF2->F2_CLIRET+SF2->F2_LOJARET)
							...
							aadd(aRetirada,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,FWxFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP")))
							aadd(aRetirada,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,FWxFilial("SYA")+SA1->A1_PAIS,"YA_DESCR" )))
					...
					If AliasIndic("CD9")			
						dbSelectArea("CD9")
						dbSetOrder(1)
						if MsSeek(FWxFilial("CD9") + cChaveF2 )
					...
					(SF2->(ColumnPos("F2_CLIREM"))<>0 .And. SF2->(ColumnPos("F2_LOJAREM"))<>0 .And. !Empty(SF2->F2_CLIREM + SF2->F2_LOJAREM) .And. (SF2->F2_CLIREM+SF2->F2_LOJAREM <> SF2->F2_CLIENT+SF2->F2_LOJENT .OR. cTpOper == "1" ))
						...
						If (SF2->(ColumnPos("F2_CLIREM"))<>0 .AND. !Empty(SF2->F2_CLIREM + SF2->F2_LOJAREM))//verifica se existe cliente remessa preenchido
							cFiltro := FWxFilial("SA1") + SF2->F2_CLIREM + SF2->F2_LOJAREM
						Else
							cFiltro := FWxFilial("SA1") + SF2->F2_CLIENT + SF2->F2_LOJENT
						EndIF
                        ...
                        If MsSeek(cFiltro)
							...
							aadd(aEntrega,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,FWxFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP")))
							aadd(aEntrega,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,FWxFilial("SYA")+SA1->A1_PAIS,"YA_DESCR" )))
				    ...
				    dbSelectArea("SA1")
					dbSetOrder(1)
					SA1->(MsSeek(FWxFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))							
					...
					*/
					If !lUsaCliEnt
						...
						If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))			
							If Len(aEntrega) > 0											
								...
								If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST)
									SA1->(MsSeek(FWxFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT))		
								EndIf
						...
						Else
							If !Empty(cCliNota+cCliLoja)
								SA1->(MsSeek(FWxFilial("SA1")+cCliNota+cCliLoja))   //Busca os dados do cliente da Nota sobre Cupom para montar os dados do destinatário do XML
							Else
								SA1->(MsSeek(FWxFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))
							EndIf
						EndIf
					...
					aadd(aDest,Upper(SA1->A1_EST))
					aadd(aDest,SA1->A1_CEP)
					aadd(aDest,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,FWxFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP")))
					aadd(aDest,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,FWxFilial("SYA")+SA1->A1_PAIS,"YA_DESCR" )))
					aadd(aDest,AllTrim(SA1->A1_DDD)+SA1->A1_TEL)                                                 				
					If !Upper(SA1->A1_EST) == "EX"                                                      				
				...
				Else
					...
					If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))
						MsSeek(FWxFilial("SA2")+SF2->F2_CLIENT+SF2->F2_LOJENT)
					Else
						MsSeek(FWxFilial("SA2")+SF2->F2_CLIENTE+SF2->F2_LOJA)
					EndIf
					aDest := {}
					...
					aadd(aDest,IIF(Empty(SA2->A2_PAIS),"1058"  ,Posicione("SYA",1,FWxFilial("SYA")+SA2->A2_PAIS,"YA_SISEXP")))
					aadd(aDest,IIF(Empty(SA2->A2_PAIS),"BRASIL",Posicione("SYA",1,FWxFilial("SYA")+SA2->A2_PAIS,"YA_DESCR")))
				...
				If !Empty(SF2->F2_TRANSP)
					dbSelectArea("SA4")
					dbSetOrder(1)
					MsSeek(FWxFilial("SA4")+SF2->F2_TRANSP)
					...
					If !Empty(SF2->F2_PLACA)
						...
						aadd(aVeiculo,SF2->F2_ESTPLA)
						aadd(aVeiculo,"")	//RNTC

						If !Empty(SF2->F2_PLACA2)
							aadd(aReboque,SF2->F2_PLACA2)
							aadd(aReboque,SF2->F2_ESTPLA)
							aadd(aReboque,"") //RNTC
						Endif

						_lPlaca := .F.
					Endif
					//---------------- Final bloco personalizado - SILVA
					...
					If !Empty(SF2->F2_VEICUL1) .And. _lPlaca		// Incluído - SILVA
						dbSelectArea("DA3")
						dbSetOrder(1)
						MsSeek(FWxFilial("DA3")+SF2->F2_VEICUL1)
						...
						If !Empty(SF2->F2_VEICUL2)
							...
							dbSelectArea("DA3")
							dbSetOrder(1)
							MsSeek(FWxFilial("DA3")+SF2->F2_VEICUL2)
							...
							If !Empty(SF2->F2_VEICUL3)
								...
								dbSelectArea("DA3")
								dbSetOrder(1)
								MsSeek(FWxFilial("DA3")+SF2->F2_VEICUL3)
					...
					ElseIf lNfCup   
						SL1->(dbSetOrder(2))
						SL1->(MsSeek(FWxFilial("SL1")+SF2->F2_SERIE+SF2->F2_DOC))
				...
				cChave:=""
				If !lNfCup
					cChave :=  FWxFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE
				Else
					cChave :=  FWxFilial("SF3")+cCliNota+cCliLoja+cNumNfCup+cSerNfCup
				Endif
				...
				If MsSeek(cChave)
					...
					If lSimpNac
						...
						cChave := FWxFilial("SD2") + SFT->FT_NFISCAL + SFT->FT_SERIE + SFT->FT_CLIEFOR + SFT->FT_LOJA
				...
				If !Empty(SF2->F2_DUPL)	.Or. lVLojaDir
					...
					If lVLojaDir
						...
						If !Empty(SL1->L1_ORCRES)
							cFilTit := FWxFilial("SE1",SL1->L1_FILRES)
							SL1->(DbSetOrder(1)) //L1_FILIAL + L1_NUM
							//Posiciona no Orçamento Pai
							SL1->(MsSeek(FWxFilial("SL1",SL1->L1_FILRES)+SL1->L1_ORCRES))
						EndIf
					...
					While (cAliasSE1)->(!Eof()) .And. ( ( lLJPRFPad .AND. (cAliasSE1)->E1_FILIAL+(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM == cChaveSE1 ) .OR.;
							...
							If empty(cNatFin) .Or. !(cNatFin == (cAliasSE1)->E1_NATUREZ)
								...
								SED->(MsSeek( FWxFilial("SED") + cNatFin))
				...
				If lNfCup .And. SM0->M0_ESTCOB $ "CE/DF" 
					...
					DbSelectArea( "SF2" )
					DbSetOrder(1)  // F2_DOC + F2_SERIE + F2_CLIENTE + F2_LOJA
					MsSeek( FWxFilial("SF2") + cNumNfCup + cSerNfCup)
				EndIf
				...
				If !Empty(cIdRecopi)
					If AliasIndic("CE3")
						CE3->(DbSetOrder(1))
						If CE3->(MsSeek(FWxFilial("CE3")+Alltrim(cIdRecopi)))
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								SELECT D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_ITEM,D2_CF, D2_QTSEGUM,
				...
				//---------------- Início bloco personalizado - SILVA
				aFormulas  := {}   // Formulas / Mensagens de Nota
				aMensTes   := {}   // Mensagens do TES
				xNUM_NFDV  := {}   // Numero quando houver devolucao
				xPREF_DV   := {}   // Serie quando houver devolucao
				xDATA_DV   := {}   // Data quando houver devolucao
				xNUM_NFCM  := {}   // Numero quando houver Complemento
				xPREF_CM   := {}   // Serie quando houver Complemento
				xDATA_CM   := {}   // Data quando houver Complemento
				xPEDVEN    := {}   // Numero do Pedido de Venda
				...
				nCountIT := 0
				While !Eof() .And. FWxFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lGE .and. lNfCup 
						...
						If MsSeek(FWxFilial("SB1")+(cAliasSD2)->D2_COD)
					...
					If lContinua
						...
						If lNfCup
							...
							If MsSeek(FWxFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA)
								cD2Tes	:= SD2->D2_TES
							EndIf	
							
							If MsSeek(FWxFilial("SD2")+cNumNfCup+cSerNfCup+cCliNota+cCliLoja+(cAliasSD2)->D2_COD+cNumitem)
								cD2Cfop := SD2->D2_CF
						...
						//---------------- Início bloco personalizado - SILVA
						_nRecAlias := SF2 -> (RecNo ())                 // Guarda o registro corrente do Alias() em uso

						AADD(xPEDVEN, (cAliasSD2)->D2_PEDIDO)

						If SF4 -> (fieldpos ("F4_FORMNF")) > 0 .And. !Empty(SF4->F4_FORMNF) .And. aScan(aFormulas, SF4->F4_FORMNF) == 0
							AADD(aFormulas, SF4->F4_FORMNF)
						EndIf
						If SF4 -> (fieldpos ("F4_FORMNF2")) > 0 .And. !Empty(SF4->F4_FORMNF2) .And. aScan(aFormulas, SF4->F4_FORMNF2) == 0
							AADD(aFormulas, SF4->F4_FORMNF2)
						EndIf
						If SF4 -> (fieldpos ("F4_FORMNF3")) > 0 .And. !Empty(SF4->F4_FORMNF3) .And. aScan(aFormulas, SF4->F4_FORMNF3) == 0
							AADD(aFormulas, SF4->F4_FORMNF3)
						EndIf
						If SF4 -> (fieldpos ("F4_MENS1")) > 0 .And. !Empty(SF4->F4_MENS1) .And. aScan(aMensTes, SF4->F4_MENS1) == 0
							AADD(aMensTes, SF4->F4_MENS1)
						EndIf
						If SF4 -> (fieldpos ("F4_MENS2")) > 0 .And. !Empty(SF4->F4_MENS2) .And. aScan(aMensTes, SF4->F4_MENS2) == 0
							AADD(aMensTes, SF4->F4_MENS2)
						EndIf
						If SF4 -> (fieldpos ("F4_MENS3")) > 0 .And. !Empty(SF4->F4_MENS3) .And. aScan(aMensTes, SF4->F4_MENS3) == 0
							AADD(aMensTes, SF4->F4_MENS3)
						EndIf
						If SF4 -> (fieldpos ("F4_MENS4")) > 0 .And. !Empty(SF4->F4_MENS4) .And. aScan(aMensTes, SF4->F4_MENS4) == 0
							AADD(aMensTes, SF4->F4_MENS4)
						EndIf
						If SF4 -> (fieldpos ("F4_MENS5")) > 0 .And. !Empty(SF4->F4_MENS5) .And. aScan(aMensTes, SF4->F4_MENS5) == 0
							AADD(aMensTes, SF4->F4_MENS5)
						EndIf
						If SF4 -> (fieldpos ("F4_MENS6")) > 0 .And. !Empty(SF4->F4_MENS6) .And. aScan(aMensTes, SF4->F4_MENS6) == 0
							AADD(aMensTes, SF4->F4_MENS6)
						EndIf

						// Devolucao / Beneficiamento
						_cTipoNF := (cAliasSD2)->D2_TIPO
						If aScan(xNUM_NFDV, (cAliasSD2)->D2_NFORI) == 0 .And. (SF2->F2_TIPO $ "B/D") .And. !Empty((cAliasSD2)->D2_NFORI)
							AADD(xNUM_NFDV , IIf(Empty((cAliasSD2)->D2_NFORI),"",(cAliasSD2)->D2_NFORI))
							AADD(xPREF_DV  , (cAliasSD2)->D2_SERIORI)
							AADD(xDATA_DV  , GetAdvFVal("SF1","F1_EMISSAO",FWxFilial("SF1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+SF2->F2_CLIENTE+SF2->F2_LOJA,1))
						Else
							AADD(xNUM_NFDV , "")
							AADD(xPREF_DV  , "")
							AADD(xDATA_DV  , Ctod(""))
						...
						// Complementos (IPI/ICMS/Preco/)
						If aScan(xNUM_NFCM, (cAliasSD2)->D2_NFORI)==0 .And. !(SF2->F2_TIPO $ "N/B/D")
							AADD(xNUM_NFCM , IIf(Empty((cAliasSD2)->D2_NFORI),"",(cAliasSD2)->D2_NFORI))
							AADD(xPREF_CM  , (cAliasSD2)->D2_SERIORI)
							AADD(xDATA_CM  , GetAdvFVal("SF2","F2_EMISSAO",FWxFilial("SF2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+SF2->F2_CLIENTE+SF2->F2_LOJA,1))
						Else
							AADD(xNUM_NFCM , "")
							AADD(xPREF_CM  , "")
							AADD(xDATA_CM  , Ctod(""))
						Endif
						SF2 -> (DbGoto(_nRecAlias))                 // Retorna o registro corrente do Alias() guardado	    		
						//---------------- Fim bloco personalizado - SILVA
						...
						If !Empty((cAliasSD2)->D2_NFORI)
							If (cAliasSD2)->D2_TIPO $ "DBN"
								...
								dbSelectArea("SD1")
								dbSetOrder(1)
								If	( MsSeek(FWxFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+PADL(alltrim((cAliasSD2)->D2_ITEMORI),TamSx3("D2_ITEMORI")[1],"0")) ) .OR. ;
									( MsSeek(FWxFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA) .And. Empty(cMVREFNFE) )  .Or. ;
									...
									If nRecSD1 == 0 .and. !Empty((cAliasSD2)->D2_IDENTB6)    									
										...
										dbSelectArea("SD1")
										dbSetOrder(4)
										If MsSeek(FWxFilial("SD1")+(cAliasSD2)->D2_IDENTB6)
											dbSetOrder(1)
										Else
											dbSetOrder(1)
											SD1->(DbGoTo(nSD1Pos))
									...
									dbSelectArea("SF1")
									dbSetOrder(1)
									MsSeek(FWxFilial("SF1")+SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE+SD1->D1_LOJA+SD1->D1_TIPO)
									If SD1->D1_TIPO $ "DB"
										dbSelectArea("SA1")
										dbSetOrder(1)
										MsSeek(FWxFilial("SA1")+SD1->D1_FORNECE+SD1->D1_LOJA)
									Else
										dbSelectArea("SA2")
										dbSetOrder(1)
										MsSeek(FWxFilial("SA2")+SD1->D1_FORNECE+SD1->D1_LOJA)
									EndIf
								...
								ElseIf (cAliasSD2)->D2_TIPO == "N" .Or. (lVinEstDev :=  VldEstDev((cAliasSD2)->D2_TIPO,(cAliasSD2)->D2_CF))
									...
									dbSelectArea("SFT")
							   		dbSetOrder(4)     
							   		If MsSeek(FWxFilial("SFT")+iif(lVinEstDev,"E","S")+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_NFORI)
									...
									IF nRecSFTVin == 0 .and. (lVincNF .Or. lVinEstDev)
										...
										ElseIf lVincNF
											...
											If SFT->(MsSeek(FWxFilial("SFT") + "S" + (cAliasSD2)->D2_NFORI + (cAliasSD2)->D2_SERIORI))
												lSeekSFT := .T.
											EndIf
										endIf
										...
										if lSeekSFT
											...
											SA1->(MsSeek(FWxFilial("SA1") + SFT->FT_CLIEFOR + SFT->FT_LOJA))
										endIf
									...
									If nRecSFTVin > 0 
										...
										If SFT->FT_ESTADO == "EX" .or. ((SubStr(SM0->M0_CODMUN,1,2) == "35" .Or. SubStr(SM0->M0_CODMUN,1,2) == "29") .and. "REMESSA POR CONTA E ORDEM DE TERCEIROS" $ Upper(cNatOper) .and. lOrgaoPub )//(Venda para orgao publico - SP/BA/CFOP Remessa por conta e ordem de terceiros (cfop 5923/6923)- ch:TIDWCY   
											...
											dbSelectArea("SF3")
									   		dbSetOrder(4) 
									   		MsSeek(FWxFilial("SF3")+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE) 
										...
										ElseIf Alltrim(SFT->FT_CFOP) $ cMVREFNFE
											...
											dbSelectArea("SF3")
									   		dbSetOrder(4) 
									   		If (MsSeek(FWxFilial("SF3")+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE))
										   		If cChave <> dToS( SF3->F3_EMISSAO ) + SF3->F3_SERIE + SF3->F3_NFISCAL + SA1->A1_CGC + SM0->M0_ESTCOB + SF3->F3_ESPECIE + SF3->F3_CHVNFE;
													.or. ( cAliasSD2 )->D2_ITEM <> cItemOr
							...
							Else
								...
								//If MsSeek(FWxFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
								If MsSeek(FWxFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI)//+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
									dbSelectArea("SF2")
									dbSetOrder(1)
									MsSeek(FWxFilial("SF2")+SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA)
									If !SD2->D2_TIPO $ "DB"
										dbSelectArea("SA1")
										dbSetOrder(1)
										MsSeek(FWxFilial("SA1")+SD2->D2_CLIENTE+SD2->D2_LOJA)
									Else
										dbSelectArea("SA2")
										dbSetOrder(1)
										MsSeek(FWxFilial("SA2")+SD2->D2_CLIENTE+SD2->D2_LOJA)
										lComplDev := .T.
									EndIf
									//Obtem os dados de nota fiscal de produtor rural referenciada
						...
						cSeek := FWxFilial("CDD") + "S" + (cAliasSD2)->(D2_DOC + D2_SERIE + D2_CLIENTE + D2_LOJA)
						...
						dbSelectArea("SB1")
						dbSetOrder(1)
						MsSeek(FWxFilial("SB1")+(cAliasSD2)->D2_COD)
						
						cInfAdOnu := ""
		   				dbSelectArea("SB5")
						dbSetOrder(1)
						If MsSeek(FWxFilial("SB5")+(cAliasSD2)->D2_COD)
						...
						If lC6_CODINF .And. cInfAdPr <> "2" .And. !Empty(cInfAdPr)
							SC6->(dbSetOrder(2))
							If SC6->(MsSeek(FWxFilial("SD2")+(cAliasSD2)->(D2_COD+D2_PEDIDO+D2_ITEMPV))) 
						...
						If AliasIndic("CD9")			
							dbSelectArea("CD9")
							dbSetOrder(1)
							MsSeek(FWxFilial("CD9") + cChaveD2 )
						EndIf
						//Combustivel
						If AliasIndic("CD6")
							dbSelectArea("CD6")
							dbSetOrder(1)
							MsSeek(FWxFilial("CD6")+"S"+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+Padr((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD)
						EndIf
						//Medicamentos
						If AliasIndic("CD7")			
							dbSelectArea("CD7")
							dbSetOrder(1)
							MsSeek(FWxFilial("CD7") + cChaveD2 )
						EndIf
						// Armas de Fogo
						If AliasIndic("CD8")						
							dbSelectArea("CD8")
							dbSetOrder(1) 
							MsSeek(FWxFilial("CD8") + cChaveD2 )
						EndIf
						...
						If lAnfavea
							dbSelectArea("CDR")
							dbSetOrder(1) 
							MsSeek(FWxFilial("CDR")+"S"+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
	
							dbSelectArea("CDS")
							dbSetOrder(1) 
							cItem := PADR((cAliasSD2)->D2_ITEM,TAMSX3("CDS_ITEM")[1])
							MsSeek(FWxFilial("CDS")+"S"+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+cItem+(cAliasSD2)->D2_COD)
						EndIf  
						...
						dbSelectArea("SC5")
						dbSetOrder(1)
						MsSeek(FWxFilial("SC5")+(cAliasSD2)->D2_PEDIDO)
						...
						dbSelectArea("SC6")
						dbSetOrder(1)
						MsSeek(FWxFilial("SC6")+(cAliasSD2)->D2_PEDIDO+(cAliasSD2)->D2_ITEMPV+(cAliasSD2)->D2_COD)
						...
						//---------------- Início bloco personalizado - SILVA
						/* Posto em comentario por não ser utilizado
						...
						*/
						...
						If !Empty( cNumNfCup )
							...
							Else
								DbSelectArea("SFT")
							    DbSetOrder(1)
							    If SFT->(MsSeek((FWxFilial("SD2")+"S"+ cSerNfCup + cNumNfCup )))
						...
						If Empty(aVeiculo)
							DbSelectArea("DA3")
							DbSetOrder(1)
							If MsSeek(FWxFilial("DA3")+Iif(SC5->(FieldPos("C5_VEICULO")) > 0 ,SC5->C5_VEICULO,""))
								aadd(aVeiculo,DA3->DA3_PLACA)
								aadd(aVeiculo,DA3->DA3_ESTPLA)
								aadd(aVeiculo,Iif(DA3->(FieldPos("DA3_RNTC")) > 0 ,DA3->DA3_RNTC,iif(!Empty(cAnttRntrc),cAnttRntrc,"")))//RNTC				
						...
						If Empty(aProd)
							If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
								_aAux := _MSG(SC5->C5_MENNOTA, 100)
								For _j:= 1 To Len(_aAux) 
									cMensCli += _aAux[_j]
								Next _j
							EndIf

							If !AllTrim(SC5->C5_MENNF2) $ cMensCli
								_aAux := _MSG(SC5->C5_MENNF2, 100)
								For _j:= 1 To Len(_aAux)
									cMensCli += _aAux[_j]
								Next _j
							EndIf
							// Bloco feito por Fabian Maurer no dia 08/09/19, solicita? Henrique Jardim
							/*
							If !Empty(SC5->C5_NUM) 
								if cEmpAnt == '01' 
									_aAux := _MSG("NOSSO PEDIDO: " + SC5->C5_NUM + Space(10) + "Pr?arregamento: " + SC5->C5_REFPCAR, 60)
								else
									_aAux := _MSG("NOSSO PEDIDO: " + SC5->C5_NUM,30)
								endif
								For _j:= 1 To Len(_aAux)
									cMensCli += _aAux[_j]
								Next _j
							EndIf
							*/
							// Fim do Bloco feito por Fabian Maurer
							// Solicitado no dia 12/09/19, por Henrique Jardim que fosse retirado o bloco por motivo de seguran?do rastro dos caminhoes quanto seu numero de pr?arregamento
							...
							If !Empty(SC5->C5_MENPAD) .And. !AllTrim(FORMULA(SC5->C5_MENPAD)) $ cMensFis
								_cMenPad := FORMULA(SC5->C5_MENPAD)
								_aAux    := _MSG(_cMenPad, 100)
								For _j:= 1 To Len(_aAux)
									cMensFis += _aAux[_j]
								Next _j
							EndIf

							If !Empty(SC5->C5_MENNOTA) .And. !AllTrim(FORMULA(SC5->C5_MENNOTA)) $ cMensCli
								//_aAux := _MSG(SC5->C5_MENNOTA, 100)
								For _j:= 1 To Len(_aAux)
									cMensCli += _aAux[_j]
								Next _j
						...
						*/
						...
						SM4->( DbSetOrder( 1 ))
						lC110 := .F.
						If !Empty(SF4->F4_FORINFC) .And. SM4->( MsSeek( FWxFilial("SM4") + SF4->F4_FORINFC ) )
						...
						*/
						...
						If lSb1CT
							If lMvImpFecp  .And. SB1->B1_X_CT$cMVAEHC
								If (lValFecp .Or. lVfecpst) 
									DbSelectArea("SFT")
								    DbSetOrder(1)
									If SFT->(MsSeek((FWxFilial("SFT") + cChaveD2 )))								
										If SFT->FT_VFECPST > 0
	      				...
	      				IF (ExistTemplate("PROCMSG"))
	      					aMens := ExecTemplate("PROCMSG",.f.,.f.,{cAliasSD2})      										 		      					
								...
								If len(aMens) > 0
									...
									If MsSeek(FWxFilial("SD2")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD )
								...
								If !Empty(aCampoCnpj[nX])
									...
									if !empty(cTabCpo) .And. (cTabCpo)->(ColumnPos(aCampoCnpj[nX])) > 0 .and. !Empty((cDadoCpo := (&(cTabCpo+"->"+aCampoCnpj[nX]))))
										...
										if substr(aCampoCnpj[nX],3) == "_REDESP"
											SA4->(dbSetOrder(1))
											If SA4->(MsSeek(FWxFilial("SA4")+cDadoCpo))
						...
						MsSeek(FWxFilial("SFT") + cChaveD2 + "  " + (cAliasSD2)->D2_COD)  
						...
						If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
							...
							If SF2->F2_DESCONT > 0
								If lFirstItem	// Somente faz o looping nos itens na primeira vez
									...
									While !SD2->(Eof()) .And. FWxFilial("SD2") == SD2->D2_FILIAL .And.;
						...
						If !Empty((cAliasSD2)->D2_IDENTB6) .And. lNFPTER  
				         	If SC5->C5_TIPO == "N" 
						         ...
						         SA7->(dbSetOrder(1)) 	         
						         If SA7->(MsSeek( FWxFilial("SA7") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI) 
							...
							ElseIf SC5->C5_TIPO == "B"
						         ...
						         SA5->(dbSetOrder(1)) 	         
						         If SA5->(MsSeek( FWxFilial("SA5") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .and. !empty(SA5->A5_CODPRF) .and. !empty(SA5->A5_DESREF)
						...
						//If AliasIndic("CFD")
							...
							//If CFD->(MsSeek(FWxFilial("CFD")+(cAliasSD2)->D2_FCICOD))
								//-----------------------------------------------------------------------------------
			            ...
			            If lCpoMsgLT .And. lCpoLoteFor .And. SF4->F4_MSGLT $ "1" 
							cNumLotForn := Alltrim(Posicione("SB8",2,FWxFilial("SB8")+(cAliasSD2)->D2_NUMLOTE+(cAliasSD2)->D2_LOTECTL+cCodProd,"B8_LOTEFOR"))
						...
						If Upper(SM0->M0_ESTENT) $ cCodCST //TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR.
							...
							cCodlan := ""
							cChvCdv := FWxFilial("CDV") +'S'+PadR(cTpEspcBen,TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD2)->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_ITEM)
							If MsSeek(cChvCdv)
								cCodlan := retCodCdv(cChvCdv)
								retaBenef(@aBenef,;
										FWxFilial("CDV")+'S'+PadR(cTpEspcBen,TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD2)->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+AllTrim(D2_ITEM)),;
										SM0->M0_ESTENT,;
										@aCredPresum)
							else 
						...
						Else
							...
							If SM0->M0_ESTENT <> "SP" .and. CDA->(ColumnPos("CDA_CODLAN")) > 0
								...
								cSeekCDA := FWxFilial("CDA") + 'S' + PadR(cTpEspcBen,TamSX3("CDA_ESPECI")[1]) + 'S' + (cAliasSD2)->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_ITEM)
						...
						If AliasIndic("D3E")
							...
							dbSetOrder(1)
							cIndEscala :=""
							If MsSeek(PADR(FWxFilial("D3E"),TAMSX3("D3E_FILIAL")[1]) +(cAliasSD2)->D2_COD)
						...
						*/
						...
						aadd(aProd,	{Len(aProd)+1,;
							(cAliasSD2)->D2_COD,; 		// Alterado - SILVA 
							IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
							Transform((cAliasSD2)->D2_QTSEGUM, "@E@Z 9999")+" "+IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI),;  		// Alterado - SILVA para sair o numero de caixas no danfe antes "cDescProd"
							...
							IIF(!(cAliasSD2)->D2_TIPO$"IP",NoRound( ( (cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR )/(cAliasSD2)->D2_QUANT,8),0 ),;	// Alterado - SILVA 
						...
						If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
							...
							If SL1->(MsSeek(FWxFilial('SL1') + SF2->F2_SERIE + SF2->F2_DOC))
								lAchouSL1 := .T.
							Else
								...
								If SL2->(MsSeek(FWxFilial('SL2') + SF2->F2_SERIE + SF2->F2_DOC))
									
									SL1->(DbSetOrder(1)) //L1_FILIAL+L1_NUM
									If SL1->(MsSeek(FWxFilial('SL1') + SL2->L2_NUM))
										lAchouSL1 := .T.
									EndIf	
						...
						*/
						...
						DbSelectArea("CDL")
						DbSetOrder(1)
						if MsSeek(FWxFilial("CDL")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
							aadd(aExp,{})
							lExpCDL := .T.
							While !CDL->(Eof()) .And. CDL->CDL_FILIAL+CDL->CDL_DOC+CDL->CDL_SERIE+CDL->CDL_CLIENT+CDL->CDL_LOJA == FWxFilial("CDL")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA
						...
						If AliasIndic("CD6")  .And. CD6->(FieldPos("CD6_QTAMB")) > 0 .And. CD6->(FieldPos("CD6_UFCONS")) > 0  .And. CD6->(FieldPos("CD6_BCCIDE")) > 0 .And. CD6->(FieldPos("CD6_VALIQ")) > 0 .And. CD6->(FieldPos("CD6_VCIDE")) > 0
							...
							While !Eof() .And. FWxFilial("CD6") == CD6->CD6_FILIAL .And. ;
												CD6->CD6_TPMOV == "S" .And. ;
						...
						DbSelectArea("SF2")
						DbSetOrder(1)
						MsSeek(FWxFilial("SF2")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
						dbSelectArea("CD2")
					    ...
					    DbSelectArea("SFT")
					    DbSetOrder(1)
					    If SFT->(MsSeek(FWxFilial("SFT")+"S"+(cAliasSD2)->(D2_SERIE+D2_DOC+D2_CLIENTE+D2_LOJA+PadR(D2_ITEM,TamSx3("FT_ITEM")[1])+D2_COD)))
						...
						CD2->(MsSeek(FWxFilial("CD2")+"S"+SF2->F2_SERIE+SF2->F2_DOC+SF2->F2_CLIENTE+SF2->F2_LOJA+PadR((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD))
			
						While CD2->(!Eof()) .And. FWxFilial("CD2") == CD2->CD2_FILIAL .And.;
							"S" == CD2->CD2_TPMOV .And.;
							...
							/*DbSelectArea("SF7")				
							DbSetOrder(1)											
								If MsSeek(FWxFilial("SF7")+SB1->B1_GRTRIB+SA1->A1_GRPTRIB)														
									If SF7->F7_BASEICM > 0
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "ISS" 
									...
									If AliasIndic("CDN") .And. CDN->(MsSeek(FWxFilial("CDN")+cCodIss))
									...
									/*If SF3->(FieldPos("F3_RECISS"))>0
										If SF3->F3_RECISS $"1S"       
											If SF3->(MsSeek(FWxFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE))
												While !SF3->(EOF()) .And. FWxFilial("SF3")+SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE==SF2->F2_FILIAL+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE
		              ...
		              EndIf
						...
						If (lCalSol .OR.  lMVCOMPET .OR. lDifal )
							dbSelectArea("SF3")
							dbSetOrder(4)
							If MsSeek(FWxFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
								...
								Elseif  lDifal
									If AliasInDic("F0L")
										dbSelectArea("F0L")
										dbSetOrder(1)
										If MsSeek(FWxFilial("F0L")+cEstado)	//F0L_FILIAL, F0L_UF, F0L_INSCR, R_E_C_N_O_, D_E_L_E_T_
						...
						If SFT->(FieldPos("FT_CSTPIS")) > 0 .And. SFT->(FieldPos("FT_CSTCOF")) > 0
							...
							If MsSeek(FWxFilial("SFT")+"S"+SF2->F2_SERIE+SF2->F2_DOC+SF2->F2_CLIENTE+SF2->F2_LOJA+PadR((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD)
				...
				// Msg Zona Franca de Manaus / ALC
					...
					dbSetOrder(4)
					MsSeek (FWxFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
					Do While !SF3->(Eof()) .AND. FWxFilial("SF3") == SF3->F3_FILIAL .And.;
				...
				//---------------- Início bloco personalizado - SILVA
				// Fórmulas do TES
				For _n := 1 To Len (aFormulas)
					_cMenTes := Formula(aFormulas[_n])
					_aAux    := _MSG(_cMenTes, 100)
					For _j:= 1 To Len(_aAux)
						cMensFis += _aAux[_j]
					Next _j
				Next _n
				...
				// Mensagens do TES
				For _n := 1 To Len (aMensTes)
					_cMenTes := aMensTes[_n]
					_aAux    := _MSG(_cMenTes, 100)
					For _j:= 1 To Len(_aAux)
						cMensFis += _aAux[_j]
					Next _j
				...
				// Busca notas originais (Caso de Devolucao/Beneficiamento)
				If Len(xNUM_NFDV) > 0 .And. !Empty(xNUM_NFDV[1])
					For _nNFDV := 1 To Len(xNUM_NFDV)
						If !Empty(xNUM_NFDV[_nNFDV])
							_cDados := IIf(_cTipoNF == "D","DEVOLUCAO","RETORNO")+;
							" REF N.F. ORIG. No. " + xNUM_NFDV[_nNFDV] +;
							IIf(Alltrim(xPREF_DV[_nNFDV]) == "", "", "-" + xPREF_DV[_nNFDV]) +;
							IIf(!Empty(xDATA_DV[_nNFDV])," DO DIA: " + Dtoc(xDATA_DV[_nNFDV])+" .","")
							_aAux   := _MSG(_cDados, 100)
							For _j:= 1 To Len(_aAux)
								cMensFis += _aAux[_j]
							Next _j
						Endif
				...
				// Busca notas originais (Caso de complementos IPI/ICMS/Preco)
				If Len(xNUM_NFCM) > 0 .And. !Empty(xNUM_NFCM[1])
					_xCOMPLE := ""
					If _cTipoNF == "P"
						_xCOMPLE := "IPI"
					ElseIf _cTipoNF == "I"
					...
					ElseIf _cTipoNF == "C"
						_xCOMPLE := "PRECO"
					Endif
					For _nNFCM = 1 to Len(xNUM_NFCM)
						If !Empty(xNUM_NFCM[_nNFCM])
							_cDados := "COMPLEMENTO DE " + _xCOMPLE + " REF N.F. ORIG. No. " + xNUM_NFCM[_nNFCM] + IIf(Alltrim(xPREF_CM[_nNFCM]) == "", "", "-" + xPREF_CM[_nNFCM]) + IIf(!Empty(xDATA_CM[_nNFCM])," DO DIA: " + Dtoc(xDATA_CM[_nNFCM])+" .","")
							_aAux   := _MSG(_cDados, 100)
							For _j:= 1 To Len(_aAux)
								cMensFis += _aAux[_j]
							Next _j
				...
				//---------------- Início bloco personalizado - SILVA
				If !Empty(SF2->F2_CONTSOC)
					_aAux := alltrim(_MSG("RETENCAO P/ PREVIDENCIA SOCIAL R$ " +AllTrim(Str(SF2->F2_CONTSOC,12,2)), 100))
					For _j:= 1 To Len(_aAux)
						cMensCli += _aAux[_j]
					Next _j
				Endif

				If !Empty(SF2->F2_VALIMP1)
					_aAux := alltrim(_MSG("VALOR FUNDESA R$ " +AllTrim(Str(SF2->F2_VALIMP1,12,2)), 100))
					For _j:= 1 To Len(_aAux)
						cMensCli += _aAux[_j]
					Next _j
				Endif

				If SF2->F2_TIPOCLI <> "X" .And. SF2->F2_TIPO <> "B"
					DbSelectArea("SA1")
					xFORMREC := GetAdvFVal("SA1","A1_FORMREC",FWxFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA,1)
					DO CASE
						CASE xFORMREC = "1"
							_cFormR := "R$"
						CASE xFORMREC = "2"
						...
						OTHERWISE
							_cFormR := "LK"
					ENDCASE
					_aAux := _MSG("REPRESENTANTE: "+SF2->F2_VEND1+"-"+AllTrim(GetAdvFVal("SA3","A3_NREDUZ",FWxFilial("SA3")+SF2->F2_VEND1,1))+Space(03)+"FORMA RECEBIMENTO: " + _cFormR, 100)
					For _j:= 1 To Len(_aAux)
						cMensCli += _aAux[_j]
					Next _j
				Endif
				...
				If SF2->F2_TIPOCLI == "X"
					DbSelectArea("SC5")
					MsSeek(FWxFilial("SC5")+xPEDVEN[1])
					If Found()
						xTIPO_CLI  := SC5->C5_TIPOCLI         // Tipo de Cliente
						xMENS_EXP  := SC5->C5_MENEXP          // Mensagem Exportacao
						xLAC_COM   := SC5->C5_LACCOM          // Lacre Companhia
						xLAC_SIF   := SC5->C5_LACSIF          // Lacre SIF
						xCOD_SIF   := SC5->C5_CODSIF          // Codigo SIF
						xCONT_VEN  := SC5->C5_CONTVEN         // Contrato de Venda
						xNAVIO     := SC5->C5_NAVIO           // Nome do Navio
						xCONTAINER := SC5->C5_CONTAIN         // Container
						xTARA      := SC5->C5_TARA            // Tara
						xPORT_EMB  := SC5->C5_PORTEMB         // Porto de Embarque
						xPORT_DES  := SC5->C5_PORTDES         // Porto de Destino

						If xTIPO_CLI == "X"
							If !Empty(xMENS_EXP)
								_aAux := _MSG(xMENS_EXP, 100)
								For _j:= 1 To Len(_aAux)
									cMensCli += _aAux[_j]
								Next _j
							Endif
							_aAux := _MSG("LACRE DA COMPANHIA: "+AllTrim(xLAC_COM)+" LACRE SIF"+AllTrim(xCOD_SIF)+": "+AllTrim(xLAC_SIF)+" CVE: "+AllTrim(xCONT_VEN), 100)
							For _j:= 1 To Len(_aAux)
								cMensCli += _aAux[_j]
							Next _j
							_aAux := _MSG("NAVIO: "+AllTrim(xNAVIO)+" CONTAINER: "+AllTrim(xCONTAINER)+" TARA: "+AllTrim(xTARA), 100)
							For _j:= 1 To Len(_aAux)
								cMensCli += _aAux[_j]
							Next _j
							_aAux := _MSG("PORTO DE EMBARQUE: "+AllTrim(xPORT_EMB)+" PORTO DE DESTINO: "+AllTrim(xPORT_DES), 100)
							For _j:= 1 To Len(_aAux)
								cMensCli += _aAux[_j]
							Next _j
						Endif
...
Else
	dbSelectArea("SF1")
	dbSetOrder(1)
	If MsSeek(FWxFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³Tratamento temporario do CTe                                            ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ			
		...
		Else				
			...
			If SF1->F1_TIPO $ "DB" 
			    dbSelectArea("SA1")
				dbSetOrder(1)
				MsSeek(FWxFilial("SA1")+cClieFor+cLoja)
				...
				If SF1->(FieldPos("F1_FORRET"))<>0 .And. !Empty(SF1->F1_FORRET+SF1->F1_LOJARET) .And. SF1->F1_FORRET+SF1->F1_LOJARET <> SF1->F1_FORNECE+SF1->F1_LOJA
				    dbSelectArea("SA1")
					dbSetOrder(1)
					IF MsSeek(FWxFilial("SA1")+SF1->F1_FORRET+SF1->F1_LOJARET)
				...
				If SF1->(FieldPos("F1_FORENT")) <> 0 .And. !Empty(SF1->F1_FORENT+SF1->F1_LOJAENT) .And. SF1->F1_FORENT+SF1->F1_LOJAENT <> SF1->F1_FORNECE+SF1->F1_LOJA
				    dbSelectArea("SA1")
					dbSetOrder(1)
					If MsSeek(FWxFilial("SA1")+SF1->F1_FORENT+SF1->F1_LOJAENT)
				...
				*/
				...
				Else
					MsSeek(FWxFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA)						
					...
					If !lUsaCliEnt
						...
						If !Empty(AllTrim(SF1->F1_FORENT)) .And. !Empty(AllTrim(SF1->F1_LOJAENT))			
							If Len(aEntrega) > 0											
								...
								//rejeicao 523 - CFOP não é de Operação Estadual e UF emitente igual à UF destinatário
								If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST)
									MsSeek(FWxFilial("SA1")+SF1->F1_FORENT+SF1->F1_LOJAENT)		
								EndIf
								//Se a UF de entrega for igual a UF do emitente (Operação interna) - busco os dados do cliente para montar como destinatario.
								//Se o CNPJ do emitente for igual ao do destinatário também levo os dados do cliente, mesmo que UFs forem diferente.
								//Se o cliente não for consumidor final e possuir IE, pode ocorrer a rejeição 773 - Operação Interna e UF de destino difere da UF do emitente
			...
			Else
			   	dbSelectArea("SA2")
				dbSetOrder(1)  				
				MsSeek(FWxFilial("SA2")+cClieFor+cLoja)
				If SF1->( ColumnPos( "F1_DEVMERC" ) ) > 0
					cDevMerc := Alltrim(SF1->F1_DEVMERC)
				EndIf
		       ...
		       If SF1->(FieldPos("F1_FORRET"))<>0 .And. !Empty(SF1->F1_FORRET+SF1->F1_LOJARET) .And. SF1->F1_FORRET+SF1->F1_LOJARET<>SF1->F1_FORNECE+SF1->F1_LOJA
					dbSelectArea("SA2")
					dbSetOrder(1)
					If MsSeek(FWxFilial("SA2")+SF1->F1_FORRET+SF1->F1_LOJARET)
						...
						aadd(aRetirada,IIF(Empty(SA2->A2_PAIS),"1058"  ,Posicione("SYA",1,FWxFilial("SYA")+SA2->A2_PAIS,"YA_SISEXP")))
						aadd(aRetirada,IIF(Empty(SA2->A2_PAIS),"BRASIL",Posicione("SYA",1,FWxFilial("SYA")+SA2->A2_PAIS,"YA_DESCR" )))
				...
				If SF1->(FieldPos("F1_FORENT")) <> 0 .And. !Empty(SF1->F1_FORENT+SF1->F1_LOJAENT) .And. SF1->F1_FORENT+SF1->F1_LOJAENT <> SF1->F1_FORNECE+SF1->F1_LOJA
					dbSelectArea("SA2")
					dbSetOrder(1)
					If MsSeek(FWxFilial("SA2")+SF1->F1_FORENT+SF1->F1_LOJAENT)
						...
						aadd(aEntrega,IIF(Empty(SA2->A2_PAIS),"1058"  ,Posicione("SYA",1,FWxFilial("SYA")+SA2->A2_PAIS,"YA_SISEXP")))
						aadd(aEntrega,IIF(Empty(SA2->A2_PAIS),"BRASIL",Posicione("SYA",1,FWxFilial("SYA")+SA2->A2_PAIS,"YA_DESCR" )))
			...
			Else
			    dbSelectArea("SA2")
				dbSetOrder(1)
				MsSeek(FWxFilial("SA2")+SF1->F1_FORNECE+SF1->F1_LOJA)	
			EndIf
			...
			SF3->(dbSetOrder(5))
			if ( SF3->(MsSeek(FWxFilial("SF3")+cSerie+cNota)) ) 
			...
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ		
			//---------------- Início bloco personalizado - SILVA
			//Bloco para busca de dados das faturas da NFP  

			nSF1Recno:= SF1->(RECNO())

			_cDocNFP := GetAdvFVal('SD1','D1_NFORI',FWxfilial('SD1')+SF1->(F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA),1)
			_cSerNFP := GetAdvFVal('SD1','D1_SERIORI',FWxfilial('SD1')+SF1->(F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA),1)
			_cDupNFP := GetAdvFVal('SF1','F1_DUPL',FWxfilial('SF1')+_cDocNFP+_cSerNFP+SF1->(F1_FORNECE+F1_LOJA),1)
			_cPreNFP := GetAdvFVal('SF1','F1_PREFIXO',FWxfilial('SF1')+_cDocNFP+_cSerNFP+SF1->(F1_FORNECE+F1_LOJA),1)
			_cEspNFP := GetAdvFVal('SF1','F1_ESPECIE',FWxfilial('SF1')+_cDocNFP+_cSerNFP+SF1->(F1_FORNECE+F1_LOJA),1)

			SF1->(DbGoTop())
			SF1->(DbSetOrder(1))
			SF1->(MsSeek(FWxFilial("SF1")+cNota+cSerie+cClieFor+cLoja))

			//Variaveis incluidas para resolver problema de contranota emitida para cliente.
			_cTes := GetAdvFVal('SD1','D1_TES',FWxfilial('SD1')+SF1->(F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA),1)
			_cGeraDPL := GetAdvFVal('SF4','F4_DUPLIC',FWxFilial('SF4') + _cTes,1)
			...
			If !Empty(SF1->F1_DUPL)	
				...
				#ELSE
					MsSeek(FWxFilial("SE2")+SF1->F1_PREFIXO+SF1->F1_DOC)
				#ENDIF
				While !Eof() .And. FWxFilial("SE2") == (cAliasSE2)->E2_FILIAL .And.;
			    ...
			    EndIf
			//---------------- Início bloco personalizado - SILVA
			// "ElseIf" para busca de dados das faturas da NFP
			ElseIf alltrim(_cEspNFP) $ 'NFP/SPED'  .and. _cGeraDPL <> 'N' 
				...
				BeginSql Alias cAliasSE2
					...
					WHERE
					SE2.E2_FILIAL = %xFilial:SE2% AND
					SE2.E2_PREFIXO = %Exp:_cPreNFP% AND
					SE2.E2_NUM = %Exp:_cDupNFP% AND
					SE2.E2_FORNECE = %Exp:SF1->F1_FORNECE% AND
					SE2.E2_LOJA = %Exp:SF1->F1_LOJA% AND
					SE2.E2_TIPO = %Exp:MVNOTAFIS% AND
					SE2.%NotDel%
				...
				#ELSE
					MsSeek(FWxFilial("SE2")+_cPreNFP+_cDocNFP)
				#ENDIF
				While !Eof() .And. FWxFilial("SE2") == (cAliasSE2)->E2_FILIAL .And.;
					_cPreNFP == (cAliasSE2)->E2_PREFIXO .And.;
					_cDocNFP == (cAliasSE2)->E2_NUM
					If (cAliasSE2)->E2_TIPO==MVNOTAFIS .And. (cAliasSE2)->E2_FORNECE==SF1->F1_FORNECE .And. (cAliasSE2)->E2_LOJA==SF1->F1_LOJA
			...
			If FieldPos("F1_TRANSP") > 0 .And. !Empty(SF1->F1_TRANSP)
				dbSelectArea("SA4")
				dbSetOrder(1)
				MsSeek(FWxFilial("SA4")+SF1->F1_TRANSP)
				...
				If !Empty(SF1->F1_PLACA)
					dbSelectArea("DA3")
					dbSetOrder(3)
					If MsSeek(FWxFilial("DA3")+SF1->F1_PLACA)
						aadd(aVeiculo,DA3->DA3_PLACA)
						aadd(aVeiculo,DA3->DA3_ESTPLA)
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN,D1_DESCICM,D1_II,D1_FORMUL,D1_VALPS3,D1_ORIGLAN,D1_VALCF3,D1_TESACLA,D1_IDENTB6,D1_PICM,D1_DESC,
						D1_DESCRI,D1_QTSEGUM,D1_NFMENS,D1_NFPROD  %Exp:cField%
						FROM %Table:SD1% SD1
						WHERE
			...
			xPREF_CM   := {}   // Serie quando houver Complemento
			xDATA_CM   := {}   // Data quando houver Complemento
			xNFPROD    := {}   // Nota Fiscal Produtor
			xNFMENS    := {}   // Mensagem NFE
			...
			nCount	 := 0
			nCountIT := 0			
			While !Eof() .And. FWxFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				dbSelectArea("SF4")
				dbSetOrder(1)
				MsSeek(FWxFilial("SF4")+(cAliasSD1)->D1_TES)
				...
				DbSelectArea("SFT")
			    DbSetOrder(1)
			    IF SFT->(MsSeek(FWxFilial("SFT")+"E"+(cAliasSD1)->(D1_SERIE+D1_DOC+D1_FORNECE+D1_LOJA)))
				...
				If SF1->(FieldPos("F1_STATUS"))>0 .And.SD1->(FieldPos("D1_TESACLA"))>0 .And. SF1->F1_STATUS='C' 
					MsSeek(FWxFilial("SF4")+(cAliasSD1)->D1_TESACLA)
				Else 
					MsSeek(FWxFilial("SF4")+(cAliasSD1)->D1_TES)
				EndIf				
				...
				SFT->( dbSetOrder( 1 ) )
				//utiliza a funcao SpedNatOper ( SPEDXFUN ) que possui o tratamento para a natureza da operacao/prestacao
				if FindFunction( "SpedNatOper" ) .And. SFT->( MsSeek( FWxFilial( "SFT" ) + cChaveD1 ) )
					If !Alltrim(SpedNatOper( nil , lNatOper , "SFT" , "SF4" , .T. )[ 2 ])$cNatOper
						If	Empty(cNatOper)
							cNatOper := SpedNatOper( nil , lNatOper , "SFT" , "SF4" , .T. )[ 2 ]
						Else
				...
				*/
				...
				if SF4->F4_AJUSTE =="S" .and. aDest[1] == SM0->M0_CGC .and. (cAliasSD1)->(D1_TIPO) == "D" .and. (cAliasSD1)->D1_FORMUL == "S"
					...
					dbSelectArea("SF2")
					dbSetOrder(1)
					if SF2->(MsSeek(FWxFilial("SF2")+(cAliasSD1)->(D1_NFORI)+(cAliasSD1)->(D1_SERIORI))) 
						dbSelectArea("SA1")
						dbSetOrder(1)						
						if SA1->(MsSeek(FWxFilial("SA1")+(SF2->F2_CLIENTE)+(SF2->F2_LOJA)))
							...
							if !Empty(SA1->A1_PAIS)
								dbSelectArea("SYA")
								dbSetOrder(1)									
								if SYA->(MsSeek(FWxFilial("SYA")+(SA1->A1_PAIS)))
				...
				//---------------- Início bloco personalizado - SILVA
				_nRecAlias := SF1 -> (RecNo ())                 // Guarda o registro corrente do Alias() em uso

				_nContSocOri  := SF1->F1_CONTSOC
				_nValImp1Ori  := SF1->F1_VALIMP1
				_nValImp2Ori  := SF1->F1_VALIMP2
				_cNFOri       := alltrim((cAliasSD1)->D1_NFORI) + '/' + alltrim((cAliasSD1)->D1_SERIORI)
				_cTesOri      := GetAdvFVal('SD1','D1_TES',FWxfilial('SD1') + (cAliasSD1)->(D1_NFORI+D1_SERIORI+D1_COD+D1_ITEMORI),1)

				//AADD(xNFPROD, (cAliasSD1)->D1_NFPROD) 

				AADD(xNFMENS, (cAliasSD1)->D1_NFMENS)
				AADD(xPEDIDO, (cAliasSD1)->D1_PEDIDO)
				If SF4 -> (fieldpos ("F4_FORMNF")) > 0 .And. !Empty(SF4->F4_FORMNF) .And. aScan(aFormulas, SF4->F4_FORMNF) == 0
					AADD(aFormulas, SF4->F4_FORMNF)
				EndIf
				If SF4 -> (fieldpos ("F4_FORMNF2")) > 0 .And. !Empty(SF4->F4_FORMNF2) .And. aScan(aFormulas, SF4->F4_FORMNF2) == 0
				...
				// Devolucao / Beneficiamento
				If aScan(xNUM_NFDV, (cAliasSD1)->D1_NFORI) == 0 .And. (SF1->F1_TIPO $ "B/D") .And. !Empty((cAliasSD1)->D1_NFORI)
					AADD(xNUM_NFDV , (cAliasSD1)->D1_NFORI)//IIf(Empty((cAliasSD1)->D1_NFORI),"",(cAliasSD1)->D1_NFORI))
					AADD(xPREF_DV  , (cAliasSD1)->D1_SERIORI)
					AADD(xDATA_DV  , GetAdvFVal("SF2","F2_EMISSAO",FWxFilial("SF2")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI+SF1->F1_FORNECE+SF1->F1_LOJA,1))
				Else
					AADD(xNUM_NFDV , "")
					AADD(xPREF_DV  , "")
					AADD(xDATA_DV  , Ctod(""))
				...
				// Complementos (IPI/ICMS/Preco/)
				If aScan(xNUM_NFCM, (cAliasSD1)->D1_NFORI)==0 .And. !(SF1->F1_TIPO $ "N/B/D")
					AADD(xNUM_NFCM , IIf(Empty((cAliasSD1)->D1_NFORI),"",(cAliasSD1)->D1_NFORI))
					AADD(xPREF_CM  , (cAliasSD1)->D1_SERIORI)
					AADD(xDATA_CM  , GetAdvFVal("SF1","F1_EMISSAO",FWxFilial("SF1")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI+SF1->F1_FORNECE+SF1->F1_LOJA,1))
				Else
					AADD(xNUM_NFCM , "")
					AADD(xPREF_CM  , "")
					AADD(xDATA_CM  , Ctod(""))
				Endif
				SF1 -> (DbGoto(_nRecAlias))                 // Retorna o registro corrente do Alias() guardado
				//---------------- Fim bloco personalizado - SILVA
				...
				If !Empty((cAliasSD1)->D1_NFORI)
					...
					If MsSeek(FWxFilial("SD1")+cSeekD1)
						cTipoNF :=  SD1->D1_TIPO 
					EndIf
					...
					If !MsSeek(FWxFilial("SD1")+cSeekD1) .and. ((cAliasSD1)->D1_TIPO) $ "C|I"
						cSeekD1:= cSeekAux
					EndIf
					...
					If MsSeek(FWxFilial("SD1")+cSeekD1)
						...
						SF1->( dbSetOrder( 1 ) )
						SF1->( msSeek( FWxFilial( "SF1" ) + SD1->D1_DOC + SD1->D1_SERIE + SD1->D1_FORNECE + SD1->D1_LOJA + SD1->D1_TIPO ) )
						lSeekOk := .T.
						If SD1->D1_TIPO $ "DB"
							dbSelectArea("SA1")
							dbSetOrder(1)
					...
					If !(cAliasSD1)->D1_TIPO $ "DBN" .Or. lRural
						...
						If (cAliasSD1)->D1_TIPO $ "I" .And. (cAliasSD1)->D1_FORMUL == "S" .And. SF4->F4_AJUSTE == "S" .And. (IIF( !lEndFis, ConvType(SM0->M0_ESTCOB), ConvType(SM0->M0_ESTENT) ) == "RJ")
							...
							dbSelectArea("SD2")
							dbSetOrder(3)
							cMsSeek := FWxFilial("SD2")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI
							If MsSeek(cMsSeek)
								aAreaSF2 := SF2->(GetArea())
								dbSelectArea("SF2")
        						dbSetOrder(1)
					...
					Else
						...
						IF (cAliasSD1)->D1_ORIGLAN =="LO"
						    If (cAliasSD1)->(FieldPos("D1_FILORI")) > 0
								cFilDev := Iif(Empty((cAliasSD1)->D1_FILORI),FWxFilial("SD2"),(cAliasSD1)->D1_FILORI)
							Else
								cFilDev := FWxFilial("SD2")
							EndIf
						...
						ElseIf (cAliasSD1)->D1_TIPO == "N" .And. (cAliasSD1)->D1_FORMUL = "S"
							...
							dbSelectArea("SFT")
					   		...
					   		If MsSeek(FWxFilial("SFT")+"E"+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_SERIORI+(cAliasSD1)->D1_NFORI)
							  	nRecSFTVin := SFT->(Recno())
							EndIf
							...
							IF nRecSFTVin == 0 .and. lVincNF
										dbSelectArea("SFT")
							   			dbSetOrder(6) //FT_FILIAL+FT_TIPOMOV+FT_NFISCAL+FT_SERIE
										If MsSeek(FWxFilial("SFT")+"E"+(cAliasSD1)->D1_NFORI + (cAliasSD1)->D1_SERIORI)
											nRecSFTVin := SFT->(Recno())
											aAreaSA2B := SA2->(getArea())
											DbSelectArea("SA2")
											SA2->(DbSetOrder(1))	// A1_FILIAL+A1_COD+A1_LOJA
											SA2->(MsSeek(FWxFilial("SA2") + SFT->FT_CLIEFOR + SFT->FT_LOJA))
										EndIf
					...
					// --------------- Inicio bloco customizado para buscar NF de produtor rural - SILVA                                                 
					If (cAliasSD1)->D1_FORMUL == "S" .And. !Empty((cAliasSD1)->D1_NFORI) .And. ((cAliasSD1)->D1_TIPO <> "D" .And. (cAliasSD1)->D1_TIPO <> "B")
						_AchaNFProd(cAliasSD1, @aNFVincRur)
					Endif
					// --------------- Fim bloco customizado para buscar NF de produtor rural - SILVA
				...
				Else
					...
					If Alltrim( (cAliasSD1)->D1_ORIGLAN ) $ "D-DP-FR" .And. Alltrim( (cAliasSD1)->D1_TIPO ) == "C"
						dbSelectArea("SF8")
						dbSetOrder(3)
						If MsSeek(cChavesf8:=FWxFilial("SF8")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA)
							aAreaSD1 := SD1->(GetArea())
							aAreaSF1 := SF1->(GetArea())
							Do While cChavesf8 == SF8->(F8_FILIAL+F8_NFDIFRE+F8_SEDIFRE+F8_TRANSP+F8_LOJTRAN)
								dbSelectArea("SD1")
								dbSetOrder(1)  
								If MsSeek(FWxFilial("SD1")+SF8->F8_NFORIG+SF8->F8_SERORIG+SF8->F8_FORNECE+SF8->F8_LOJA)
									dbSelectArea("SF1")
									dbSetOrder(1)
									If MsSeek(FWxFilial("SF1")+SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE+SD1->D1_LOJA+SD1->D1_TIPO)
				...
				dbSelectArea("SB1")
				dbSetOrder(1)
				MsSeek(FWxFilial("SB1")+(cAliasSD1)->D1_COD)
				//Veiculos Novos
				If AliasIndic("CD9")			
					dbSelectArea("CD9")
					dbSetOrder(1)
					MsSeek(FWxFilial("CD9")+cChaveD1)
				EndIf			
				//Combustivel
				If AliasIndic("CD6")
					dbSelectArea("CD6")
					dbSetOrder(1)
					MsSeek(FWxFilial("CD6")+cChaveD1)
				EndIf
				//Medicamentos
				If AliasIndic("CD7")
					dbSelectArea("CD7")
					dbSetOrder(1)
					MsSeek(FWxFilial("CD7")+cChaveD1)
				EndIf
	            // Armas de Fogo
	            If AliasIndic("CD8")
					dbSelectArea("CD8")
					dbSetOrder(1)
					MsSeek(FWxFilial("CD8")+cChaveD1)
				EndIf
				//Anfavea
				If lAnfavea
					dbSelectArea("CDR")
					dbSetOrder(1) 
					MsSeek(FWxFilial("CDR")+"S"+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA)

					dbSelectArea("CDS")
					dbSetOrder(1) 
					MsSeek(FWxFilial("CDS")+"S"+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM+(cAliasSD1)->D1_COD)
				EndIf   
				//RASTREABILIDADDE
	         	If AliasIndic("F0A")
					dbSelectArea("F0A")
					dbSetOrder(1)
					MsSeek(FWxFilial("F0A")+cChaveD1)
				EndIf

				cSeek := FWxFilial("CDD") + "E" + (cAliasSD1)->( D1_DOC + D1_SERIE + D1_FORNECE + D1_LOJA )		
				...
				dbSelectArea("SB5")
				dbSetOrder(1)
				If MsSeek(FWxFilial("SB5")+(cAliasSD1)->D1_COD)
					If SB5->(FieldPos("B5_DESCNFE")) > 0 .And. !Empty(SB5->B5_DESCNFE)
						cInfAdic := Alltrim(SB5->B5_DESCNFE)
					Else	
						cInfAdic := ""				
				...
				MsSeek(FWxFilial("SFT")+"E"+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM+(cAliasSD1)->D1_COD) 
				If SFT->(FieldPos("FT_DS43080")) <> 0 .And. SFT->FT_DS43080 > 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG"
					nDescRed := SFT->FT_DS43080 
					nDesTotal+= nDescRed
				EndIF
		       ...
		       EndIf   		
				...
				If !Empty((cAliasSD1)->D1_IDENTB6) .And. lNFPTER  
					If (cAliasSD1)->D1_TIPO == "N" 
						//--A5_FILIAL + A5_FORNECE + A5_LOJA + A5_PRODUTO
						SA5->(dbSetOrder(1)) 	         
						If SA5->(MsSeek( FWxFilial("SA5") + (cAliasSD1)->(D1_FORNECE+D1_LOJA+D1_COD) )) .and. !empty(SA5->A5_CODPRF) .and. !empty(SA5->A5_DESREF)
							cCodProd  := SA5->A5_CODPRF 
							cDescProd := SA5->A5_DESREF 	            
						EndIf
					ElseIf (cAliasSD1)->D1_TIPO == "B"
			         //--A7_FILIAL + A7_CLIENTE + A7_LOJA + A7_PRODUTO
						SA7->(dbSetOrder(1)) 	         
						If SA7->(MsSeek( FWxFilial("SA7") + (cAliasSD1)->(D1_FORNECE+D1_LOJA+D1_COD) )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI) 
							cCodProd  := SA7->A7_CODCLI 
							cDescProd := SA7->A7_DESCCLI	            						
						EndIf
					EndIf
				...
				If lMvImpFecp .and. !(cCSTrib $ "40,41,50")
					If (lValFecp .Or. lVfecpst) 
				    	DbSelectArea("SFT")
						DbSetOrder(1)
						If SFT->(MsSeek((FWxFilial("SFT") + cChaveD1 )))	
					    	nValTFecp += SFT->FT_VFECPST + SFT->FT_VALFECP  + SFT->FT_VFECPMG + SFT->FT_VFESTMG	
							nValIFecp := SFT->FT_VFECPST + SFT->FT_VALFECP  + SFT->FT_VFECPMG + SFT->FT_VFESTMG						
						Endif
				...
				If Upper(SM0->M0_ESTENT) $ cCodCST	//TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR.
					...
					dbSetOrder(4)
					cCodlan :=""
					cChvCdv	:= FWxFilial("CDV") +'E'+PadR('SPED',TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD1)->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_ITEM)
					If MsSeek(cChvCdv)
						cCodlan := retCodCdv (cChvCdv)
						retaBenef(@aBenef,;
								  FWxFilial("CDV")+'E'+PadR('SPED',TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD1)->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+AllTrim(D1_ITEM)),;
								  SM0->M0_ESTENT,;
								  @aCredPresum)
					else
						cCodlan := getCodLan( alltrim(SM0->M0_ESTENT), SF4->F4_SITTRIB, cCodCST )
				...
				Else
					...
					If SM0->M0_ESTENT <> "SP" .and. CDA->(ColumnPos("CDA_CODLAN")) > 0
						dbSelectArea("CDA")
						dbSetOrder(1)
						cSeekCDA := FWxFilial("CDA") +'E'+PadR('SPED',TamSX3("CDA_ESPECI")[1])+'S'+(cAliasSD1)->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_ITEM)
				...
				If AliasIndic("D3E")
					...
					dbSetOrder(1)
					cIndEscala :=""
					If MsSeek(PADR(FWxFilial("D3E"),TAMSX3("D3E_FILIAL")[1]) +(cAliasSD1)->D1_COD)
						If D3E->(ColumnPos("D3E_INDESC")) > 0
							If	!Empty(D3E->D3E_INDESC)  .AND.  D3E->D3E_INDESC == "1"
								cIndEscala:= "S"
							EndIF	
				...
				*/				
				...
				aadd(aProd,	{Len(aProd)+1,;  
					cCodProd,;
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					IIF(AllTrim((cAliasSD1)->D1_CF)=="1949",(cAliasSD1)->D1_DESCRI,Transform((cAliasSD1)->D1_QTSEGUM, "@E@Z 9999")+" "+(cAliasSD1)->D1_DESCRI),; 	// Alterado - SILVA para obter 2a. UM antes "cDescProd"
					cNcmProd,;//B1_POSIPI
					SB1->B1_EX_NCM,;
					(cAliasSD1)->D1_CF,;
					SB1->B1_UM,;
				...
				DbSelectArea("SC7")
				DbSetOrder(1)
				If MsSeek(FWxFilial("SC7")+(cAliasSD1)->D1_PEDIDO+(cAliasSD1)->D1_ITEM)
				...
				*/
				...
				If !Empty((cAliasSD1)->D1_NFORI) .and. ((cAliasSD2)->D2_DOC == (cAliasSD1)->D1_NFORI) .and. ((cAliasSD2)->D2_SERIE == (cAliasSD1)->D1_SERIORI) .and.;
					CDL->(MsSeek(FWxFilial("CDL")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA))
					aadd(aExp,{})
					lExpCDL := .T.
					While !CDL->(Eof()) .And. CDL->CDL_FILIAL+CDL->CDL_DOC+CDL->CDL_SERIE+CDL->CDL_CLIENT+CDL->CDL_LOJA == FWxFilial("CDL")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA
						If CDL->(FieldPos("CDL_PRODNF")) <> 0 .And. CDL->(FieldPos("CDL_ITEMNF")) <> 0 .And. AllTrim(CDL->CDL_PRODNF)+AllTrim(CDL->CDL_ITEMNF) == AllTrim((cAliasSD2)->D2_COD)+AllTrim((cAliasSD2)->D2_ITEM)
				...
				DbSelectArea("SF2")
				DbSetOrder(1)
				If MsSeek(FWxFilial("SF2")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI)
				...
				ElseIf !lEasy .Or. (lEasy .AND. (cAliasSD1)->D1_TIPO $ "NDCPI")
					...
					If MsSeek(FWxFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
							aAdd(aDI,{;
				...
				If SFT->(MsSeek(FWxFilial("SFT")+"E"+(cAliasSD1)->(D1_SERIE+D1_DOC+D1_FORNECE+D1_LOJA+D1_ITEM+D1_COD)))
				   aadd(aCSTIPI,{SFT->FT_CTIPI})
				   //TRATAMENTO DA AQUISIÇÃO DE LEITE DO PRODUTOR RURAL CONFORME ARTIGO 207-B, INCISO II RICMS/MG
				   //PEGA OS VALORES E PERCENTUAL DO INNCENTIVO NOS ITENS NA SFT.
				   If SFT->(FieldPos("FT_PRINCMG")) > 0 .And. SFT->(FieldPos("FT_VLINCMG")) > 0
				...
				SF1->(MsSeek(FWxFilial("SF1")+(cAliasSD1)->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_TIPO)))
				CD2->(MsSeek(FWxFilial("CD2")+"E"+SF1->F1_SERIE+SF1->F1_DOC+SF1->F1_FORNECE+SF1->F1_LOJA+PadR((cAliasSD1)->D1_ITEM,4)+(cAliasSD1)->D1_COD))
				While !CD2->(Eof()) .And. FWxFilial("CD2") == CD2->CD2_FILIAL .And.;
					"E" == CD2->CD2_TPMOV .And.;
					...
					IF !(cAliasSD1)->D1_TIPO $ "DB"
						If SA2->(MsSeek(FWxFilial("SA2")+SF1->F1_FORNECE+SF1->F1_LOJA))
							If SF7->(MsSeek(FWxFilial("SF7")+SB1->B1_GRTRIB+SA2->A2_GRPTRIB))														
            	        ...
            	        EndIf
                    Else
						If SA1->(MsSeek(FWxFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA))
							If SF7->(MsSeek(FWxFilial("SF7")+SB1->B1_GRTRIB+SA1->A1_GRPTRIB))														
								If  SF7->F7_BASEICM > 0 .And. SF7->F7_BASEICM < 100
									nMargem :=  100 - SF7->F7_BASEICM
								EndIf										
							EndIf					
					...
					Do Case
						...
						Case AllTrim(CD2->CD2_IMP) == "ISS"
							...
							// If SF3->(FieldPos("F3_RECISS"))>0
							// 	If SF3->F3_RECISS $"1|S"       
							// 		If SF3->(MsSeek(FWxFilial("SF3")+SF1->F1_FORNECE+SF1->F1_LOJA+SF1->F1_DOC+SF1->F1_SERIE))
							// 			While !SF3->(EOF()) .And. FWxFilial("SF3")+SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE==SF1->F1_FILIAL+SF1->F1_FORNECE+SF1->F1_LOJA+SF1->F1_DOC+SF1->F1_SERIE
							// 				If SF3->F3_TIPO=="S" //Serviço
							// 					nValISSRet+= SF3->F3_VALICM
							// 				EndIf
							// 				SF3->(dbSkip())
				...
				dbSelectArea("SFT") //Livro Fiscal Por Item da NF
				dbSetOrder(1) //FT_FILIAL+FT_TIPOMOV+FT_SERIE+FT_NFISCAL+FT_CLIEFOR+FT_LOJA+FT_ITEM+FT_PRODUTO
				If MsSeek(FWxFilial("SFT")+"E"+SF1->F1_SERIE+SF1->F1_DOC+SF1->F1_FORNECE+SF1->F1_LOJA+PadR((cAliasSD1)->D1_ITEM,4)+(cAliasSD1)->D1_COD) .And. ;
			...
			//---------------- In?o bloco personalizado - SILVA
			// Fórmulas do TES
			For _n := 1 To Len (aFormulas)
				_cMenTes := Formula(aFormulas[_n])
				_aAux    := _MSG(_cMenTes, 100)
			...
			// Dados da NFP
			If _cTesOri = '200'
				_aAux := _MSG("NF. PRODUTOR: " + _cNfOri)
				For _j:= 1 To Len(_aAux)
					cMensCli += _aAux[_j]
				Next _j
			Endif
			...
			If !Empty(xNFMENS[1])
				_aAux := _MSG(AllTrim(xNFMENS[1]), 100)
				For _j:= 1 To Len(_aAux)
					cMensCli += _aAux[_j]
				Next _j
			Endif
			If !Empty(xPEDIDO[1])
				DbSelectArea("SC7")
				DbSetOrder(1)
				MsSeek(FWxFilial("SC7")+xPEDIDO[1])
				if cEmpAnt == '01'
					If Found()
						_cComprador := SC7->C7_COMPR
						DbSelectArea("SA3")
						MsSeek(FWxFilial("SA3")+_cComprador)
						If Found()
							_aAux := _MSG("COMPRADOR: " + SA3->A3_NOME, 100)
							For _j:= 1 To Len(_aAux)
								cMensCli += _aAux[_j]
							Next _j
						Endif
			...
			If Len(xNUM_NFDV) > 0 .And. !Empty(xNUM_NFDV[1])
				For _nNFDV := 1 To Len(xNUM_NFDV)
					If !Empty(xNUM_NFDV[_nNFDV])
						_cDados := IIf(SF1->F1_TIPO == "D","DEVOLUCAO","RETORNO")+;
						" REF N.F. ORIG. No. " + xNUM_NFDV[_nNFDV] +;
						IIf(Alltrim(xPREF_DV[_nNFDV]) == "", "", "-" + xPREF_DV[_nNFDV]) +;
						IIf(!Empty(xDATA_DV[_nNFDV])," DO DIA: " + Dtoc(xDATA_DV[_nNFDV])+" .","")
						_aAux   := _MSG(_cDados, 100)
			...
			If Len(xNUM_NFCM) > 0 .And. !Empty(xNUM_NFCM[1])
				If cEmpAnt == "01"
					_cNumAm := SF1->F1_NUMAM
					_cLote  := SF1->F1_LOTE
				Else
					_cNumAm := Space(08)
					_cLote  := Space(06)
				Endif
				If Empty(_cNumAm) .And. Empty(_cLote)
					_xCOMPLE := ""
					If SF1->F1_TIPO == "P"
						_xCOMPLE := "IPI"
		...
		//---------------- Início bloco personalizado - SILVA
		If !Empty(SF1->F1_MENCRD1)   // Mensagem 1 para notas de credito
			_aAux := _MSG(SF1->F1_MENCRD1, 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		EndIf
		If !Empty(SF1->F1_MENCRD2)   // Mensagem 2 para notas de credito
			_aAux := _MSG(SF1->F1_MENCRD2, 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif
		If !Empty(SF1->F1_MENCRD3)   // Mensagem 3 para notas de credito
			_aAux := _MSG(SF1->F1_MENCRD3, 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif
		If !Empty(SF1->F1_MENCRD4)   // Mensagem 4 para notas de credito
			_aAux := _MSG(SF1->F1_MENCRD4, 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif

		If !Empty(SF1->F1_MENDFE1)   // Mensagem 1 para notas de entrada
			_aAux := _MSG(SF1->F1_MENDFE1, 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif
		If !Empty(SF1->F1_MENDFE2)   // Mensagem 2 para notas de entrada
			_aAux := _MSG(Substr(SF1->F1_MENDFE2,001,100), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
			_aAux := _MSG(Substr(SF1->F1_MENDFE2,101,080), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif
		If !Empty(SF1->F1_MENDFE3)   // Mensagem 3 para notas de entrada
			_aAux := _MSG(Substr(SF1->F1_MENDFE3,001,100), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
			_aAux := _MSG(Substr(SF1->F1_MENDFE3,101,080), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif
		If !Empty(SF1->F1_MENDFE4)   // Mensagem 4 para notas de entrada
			_aAux := _MSG(Substr(SF1->F1_MENDFE4,001,100), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
			_aAux := _MSG(Substr(SF1->F1_MENDFE4,101,080), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif
		...
		If SF1->F1_VALIMP1 > 0
			_aAux := _MSG("VALOR FUNDESA R$ " +AllTrim(Str(SF1->F1_VALIMP1,12,2)), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif
		...
		If SF1->F1_CONTSOC > 0
			_aAux := _MSG("VALOR FUNRURAL R$ " +AllTrim(Str(SF1->F1_CONTSOC,12,2)), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif  
			
		// Alterado para nova regra Senar
		If SF1->F1_VLSENAR > 0
			_aAux := _MSG("VALOR SENAR R$ " +AllTrim(Str(SF1->F1_VLSENAR,12,2)), 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif  
			
		//Acrescentado frase de contribuinte solicitado pela Progepec, feita por Fabian Maurer 21/01/19
		/*
		_cCodFornec := SF1->F1_FORNECE
		_cPosEmp := GetAdvFVal('SA2','A2_POSEMP',FWxfilial('SA2')+alltrim(_cCodFornec),1)
		If _cPosEmp = 1
			_aAux := _MSG("O contribuinte declarou ser optante pelo pagamento das contribui?s previdenci?as pela folha de sal?os", 100)
			For _j:= 1 To Len(_aAux)
				cMensCli += _aAux[_j]
			Next _j
		Endif
...
If FunName() <> "SPEDNFSE"
	...
	Else		
		...
		dbSelectArea("SE4")
		dbSetOrder(1)	
		If MsSeek(FWxFilial("SE4")+cChvPag)
...
//---------------- Início bloco personalizado - SILVA
//Bloco inserido para verificar a utiliza? de EDI e avisar para gerar e enviar arquivo EDI
if  cTipo = '1'
	...
	SD2->(DbSetOrder(3))
	if SD2->(MsSeek(FWxfilial('SD2')+cNota+cSerie+cClieFor+cLoja))
		while SD2->(!eof())             .and.;
		FWxfilial('SD2') = SD2->D2_FILIAL .and.;
		SD2->D2_DOC = cNota             .and.;
		SD2->D2_SERIE = cSerie          .and.;
		SD2->D2_CLIENTE = cClieFor      .and.;
		SD2->D2_LOJA = cLoja

			SC5->(DbSetOrder(1))
			if SC5->(MsSeek(FWxfilial('SC5') + SD2->D2_PEDIDO))
				if  (!empty(SC5->C5_PCOMPRA) .and. !_lExistEDI .and. cClieFor <> '011534') //.or. (cClieFor = '007980' .and. !_lExistEDI)   //exclui cliente Grupo P?de A?ar
					_cNomCli := GetAdvFVal('SA1','A1_NOME',FWxfilial('SA1')+cClieFor+cLoja,1)

					_cMens := 'NF nr. ' + cNota + ' do Cliente ' + _cNomCli + '('+cClieFor+') Loja '
					_cMens +=  cLoja + 'foi gerado arquivo de NF por EDI!' + CHR(13)+CHR(10)
					_cMens += 'Verifique o arquivo gerado na pasta "I:\Saida" e envie-o utilizando o Client de transmiss?'+ CHR(13)+CHR(10)
					_cTit  := 'Workflow Frigor?co Silva: Aviso para gera? de arquivos digitais para envio em EDI'
					_cDest := 'faturamento@frigorificosilva.com.br,faturamento2@frigorificosilva.com.br,'
					//_cDest += 'valmir.tomazetti@frigorificosilva.com.br' - Solicitado por Valmir 15/09/21, para retirar (Esta sobrecarregando email)
					...
					// Função que gera o arquivo EDI
					u_GJF103GE(cNota,cSerie,cClieFor,cLoja)
...
// ------------------- Inicio do bloco inserido - SILVA
// Para quando for moida tubete colocar "UN" na unidade de medida em 13/11/2015
DbSelectArea('SB1')
cPUnd  := getmv('SI_PRDUNID')  
cPUnd2 := getmv('SI_PRDUNI2')
cPUnd3 := getmv('SI_PRDUNI3')
cPUnd4 := getmv('SI_PRDUNI4')
cPUnd5 := getmv('SI_PRDUNI5')
cPUnd6 := getmv('SI_PRDUNI6')

/*	
_cUnPri :=iif(alltrim(ConvType(aProd[02])) $ _cPrdUnd,'UN',;
          iif(alltrim(ConvType(aProd[02])) $ (alltrim(_cPrdUnd2)),'UN',;
          iif(alltrim(ConvType(aProd[02])) $ (alltrim(_cPrdUnd3)),'UN',ConvType(aProd[08]) )))	          
*/
// Dia 04/11/21 - Ajuste feito por Flávio 
if  ( alltrim(ConvType(aProd[02])) $ cPUnd ) .or. ( alltrim(ConvType(aProd[02])) $ cPUnd2 ) .or. ( alltrim(ConvType(aProd[02])) $ cPUnd3 ) .or. ( alltrim(ConvType(aProd[02])) $ cPUnd4 ) .or. ( alltrim(ConvType(aProd[02])) $ cPUnd5 ) .or. ( alltrim(ConvType(aProd[02])) $ cPUnd6 ) 
	_cUnPri := 'UN'
	_cUnTri := 'UN'
else
	_cUnPri := ConvType(aProd[08])
	_cUnTri := ConvType(aProd[11])
endif

/*
_cUnTri :=iif(alltrim(ConvType(aProd[02])) $ _cPrdUnd,'UN',;
          iif(alltrim(ConvType(aProd[02])) $ (alltrim(_cPrdUnd2)),'UN',;
          iif(alltrim(ConvType(aProd[02])) $ (alltrim(_cPrdUnd3)),'UN',ConvType(aProd[11]) )))
Dia 29/10/21 - ajustado 
*/

cString += '<uCom>'+_cUnPri+'</uCom>'                                   
// ------------------- Fim do bloco inserido - SILVA
...
// Alterado para que a Danfe saia com o unidade de medida "UN"
cString += '<uTrib>'+_cUnTri+'</uTrib>' 
...
If len(aEEC) > 0 .and. empty(cInfRem)
	For nY := 1 To Len(aEEC)        
	   	dbSelectArea("SF1")
		dbSetOrder(1)
		If MsSeek(FWxFilial("SF1")+aEEC[ny][2][1]+aEEC[ny][2][2]+aEEC[ny][2][3]+aEEC[ny][2][4])
			If cValidCh <> (FWxFilial("SF1")+aEEC[ny][2][1]+aEEC[ny][2][2]+aEEC[ny][2][3]+aEEC[ny][2][4])
				cCfor      := SF1->F1_FORNECE
			    cLojaEn    := SF1->F1_LOJA
			    dEmisEn    := SF1->F1_EMISSAO  
				cDocEn	   := SF1->F1_DOC
				cSerieEn   := SF1->F1_SERIE
			    cValidCh   := (FWxFilial("SF1")+aEEC[ny][2][1]+aEEC[ny][2][2]+aEEC[ny][2][3]+aEEC[ny][2][4])
			    
			    dbSelectArea("SA2")
				dbSetOrder(1)
				If MsSeek(FWxFilial("SA2")+cCfor+cLojaEn)
					cCnpjen    := SA2->A2_CGC
				EndIf   
				
				dbSelectArea( "SD1" )
				dbSetOrder( 1 )
				cChave1 := FWxFilial( "SD1" ) + cDocEn + cSerieEn + cCfor + cLojaEn

				if( MsSeek( cChave1 ) )
					dbSelectArea( "SB1" )
			   		dbSetOrder( 1 )

					while !SD1->(eof()) .and. cChave1 == FWxFilial( "SD1" ) + SD1->D1_DOC + SD1->D1_SERIE + SD1->D1_FORNECE + SD1->D1_LOJA
						if( MsSeek( FWxFilial( "SB1" ) + SD1->D1_COD ) )
...
If Len(cMsgCli)>0 .and. !Empty(cMsgCli)
	...
	IF (SM0->M0_ESTENT) $ "SP" .AND. cTipo=='0' .And. !Empty(cSerRefcup + cNfRefcup)
		SFT->(dbSetOrder(6))
		If SFT->(MsSeek(FWxFilial("SFT")+"S"+cNfRefcup+cSerRefcup))
			...
			Else
				if SM0->M0_ESTENT == 'SP' .And. !(aNfSa[5] $ "I/P/C") .And. !Empty(aNfVinc[nZ][13]) .And. !lChvCdd
					...
					if (nPos := aScan(aDadosDest,{|x| x[1]+x[2]+x[10] == aNfVinc[nZ][12]+aNfVinc[nZ][13]+AllTrim(Str(aNfVinc[nZ][11])) }) ) == 0
						...
						If aNfVinc[nZ][11]==1
							aInfoDest := GetAdvFVal("SA1", { "A1_COD","A1_LOJA","A1_NOME", "A1_MUN", "A1_EST", "A1_END", "A1_BAIRRO", "A1_CGC", "A1_INSCR"}, FWxFilial("SA1")+aNfVinc[nZ][12]+aNfVinc[nZ][13], 1, { "", "", "", "", "", "", "" })
							aAdd(aInfoDest, "1")
						Else
							aInfoDest := GetAdvFVal("SA2", { "A2_COD","A2_LOJA","A2_NOME", "A2_MUN", "A2_EST", "A2_END", "A2_BAIRRO", "A2_CGC", "A2_INSCR"}, FWxFilial("SA2")+aNfVinc[nZ][12]+aNfVinc[nZ][13], 1, { "", "", "", "", "", "", "" })
							aAdd(aInfoDest, "2")
						EndIf
...
If SC6->( FieldPos("C6_PEDVINC") ) > 0 .And. !Empty(SC6->C6_PEDVINC)  
   ...
   SC5->( dbSetOrder(1) ) 
	...
	If SC5->( MsSeek( cChave := FWxFilial("SC5") + cPedVinc ) )	
	...
	If lAtuSC5 .And. SC5->( MsSeek( FWxFilial("SC5") + cNumPed )   ) 
...
Local aNfViRuNFP:={}
	...
	SD1->(dbSetOrder(1))
		If SD1->(MsSeek(FWxFilial("SD1")+SD1->D1_NFORI+SD1->D1_SERIORI))//D1_NFORI,D1_SERIORI
   				SF1->(dbSetOrder(1))
   				If SF1->(MsSeek(FWxFilial("SF1")+SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE+SD1->D1_LOJA) .And. AllTrim(SF1->F1_ESPECIE)=="NFP")
...
dbSelectArea("SB8")
dbSetOrder(2)
if MsSeek(FWxFilial("SB8")+cNumLote+cLoteCtl+cProduto)      		
...
If lProdItem
	...
	If MsSeek( FWxFilial("SB1") + AllTrim( aProd[2] ) )
...
IF cTipo == "1" .And. cF2Tipo == "C" .And. len (aProd) > 0
	DbSelectArea("SD2")
	DbSetOrder(3)//D2_FILIAL, D2_DOC, D2_SERIE, D2_CLIENTE, D2_LOJA, D2_COD, D2_ITEM,
	If SD2->( MsSeek(FWxFilial("SD2")+ SD2->D2_DOC + SD2->D2_SERIE ))
		If !Empty(SD2->D2_NFORI).And. !Empty(SD2->D2_SERIORI) .And. !Empty(SD2->D2_ITEMORI)
			DbSelectArea("SF2")
			DbSetOrder(1)//F2_FILIAL, F2_DOC, F2_SERIE, F2_CLIENTE, F2_LOJA, F2_FORMUL, F2_TIPO,
			IF SF2->( MsSeek(FWxFilial("SF2")+ SD2->D2_NFORI + SD2->D2_SERIORI ))
...
If SC5->(MsSeek(FWxFilial("SC5")+aNfVinc[1][9]))
	cNEmp:= Iif(SC5->(FieldPos("C5_NTEMPEN")) > 0,Alltrim(SC5->C5_NTEMPEN),"")
EndIf
	   
If SC6->(MsSeek(FWxFilial("SC6")+aNfVinc[1][9]))
...
Static Function RetInfoSBZ(cProduto, cUmDipi, nConvDip)
    ...
    If SBZ->( MsSeek(FWxFilial("SBZ") + cProduto) )
...
lRet := !Empty((cAliasSD2)->D2_IDENTB6) .And. SD1->(MsSeek(FWxFilial("SD1")+(cAliasSD2)->D2_IDENTB6)) .And. SD1->(D1_DOC+D1_SERIE+D1_COD) == (cAliasSD2)->(D2_NFORI+D2_SERIORI+D2_COD)
Return lRet
...
Static Function AjustaDest(aDest,aNfVinc,cCliefor,cLoja)
	...
	SELECT max(AIF_DATA) AIF_DATA
		...
		WHERE
			...
			AIF.AIF_FILTAB = %Exp:FWxFilial("SA1")%  AND
...
Static Function EstDevSeek(cAliasSD2)
	...
	local cChave 	:= FWxFilial("SF1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI
	local cCgc		:= AllTrim(Posicione("SA2",1,FWxFilial("SA2")+(cAliasSD2)->(D2_CLIENTE+D2_LOJA),"A2_CGC"))
	...
	if SF1->(MsSeek(cChave))
		While SF1->(!EOF()) .And. SF1->(F1_FILIAL+F1_DOC+F1_SERIE) == cChave
			...
			if SF1->F1_FORMUL == 'S' .And. SF1->F1_TIPO == "D" .And.;
				AllTrim(Posicione("SA1",1,FWxFilial("SA1")+SF1->(F1_FORNECE+F1_LOJA),"A1_CGC")) == cCgc
				lRet := .T.
				Exit
			endIf

			if SF1->F1_FORMUL == 'S' .And. SF1->F1_TIPO == "N" .And. SF1->F1_EST = "EX" .and. ;
				AllTrim(Posicione("SA2",1,FWxFilial("SA2")+SF1->(F1_FORNECE+F1_LOJA),"A2_EST")) == "EX"
				lRet := .T.
				Exit
			endIf
	...
	if lRet
		...
		if SFT->(MsSeek(FWxFilial("SFT")+"E"+SF1->(F1_SERIE+F1_DOC+F1_FORNECE+F1_LOJA)))
			lRet := .T.
		endif
	endIf
...
if !empty(cIntermediador) .and. cIndIntermed == "1" .and. aliasInDic("A1U")
	...
	if A1U->(msSeek(FWxFilial("A1U")+cIntermediador))
		cString += "<infIntermed>"
...
if lSpedCodOnu .and. allTrim(superGetMv("MV_NONUINF",,"0")) == "1" 
	...
	if DY3->(MsSeek(FWxFilial("DY3")+cCodONU))
...
static function IcmsCbr(aNota,aProd)
	...
	if len(aProd) > 55
		...
		SFT->(dbSetOrder(1))
		if SFT->(MsSeek(FWxFilial("SFT")+aProd[56]+aNota[1]+aNota[2]+aNota[7]+aNota[8]+aProd[55]))
...
If lNfCup .And. cCSTrib <> "00" .And. (SM0->M0_ESTCOB $ "CE|SP" .Or. lAuto)
	...
	If SD2->(MsSeek(FWxFilial("SD2")+cNotaOri+cSerieOri+cClieFor+cLoja+cCodProd+cItem))	
...
Static Function IsVendaLoj(lLegado)
	...
	Else
		...
		dbSelectArea("SL1")
		SL1->(DbSetOrder(2)) //L1_FILIAL+L1_SERIE+L1_DOC+L1_PDV
		lRet := SL1->(MsSeek(FWxFilial("SL1")+SF2->F2_SERIE+SF2->F2_DOC))
	EndIf
...
Static Function aCompCDD(aNfVCdd, aValTotCDD, cTpMov, cDoc, cSerie, cFil, cCliente, cLoja, cSeek)
	...
	If _lCDD
		...
		If MsSeek(cSeek) 
			While !CDD->(Eof()) .And. FWxFilial("CDD") == cFil .And.;
				...
				If !Empty(CDD->CDD_CHVNFE) .and. aScan(aValTotCDD, {|x| x[1] == CDD->CDD_CHVNFE }) == 0
					...
					If MsSeek(FWxFilial(cArea)+CDD->CDD_DOCREF+CDD->CDD_SERREF+CDD->CDD_PARREF+CDD->CDD_LOJREF)
...
//---------------- Início bloco personalizado - SILVA
//----------------------------------------------------------
/*/{Protheus.doc} _AchaNFProd
Encontra a NF do Produtor
@return		Nil
@author  	Evandro
/*/
//----------------------------------------------------------
Static Function _AchaNFProd(_sAliasSD1, _aNFProd)
	Local _aAreaAnt := U_ML_SRArea()
	SA2->(DbSetOrder(1))
	If SA2->(MsSeek(FWxFilial("SA2") + (_sAliasSD1)->D1_FORNECE + (_sAliasSD1)->D1_LOJA, .F.))     
		// Nao acrescenta se a NF de produtor ja existe na array
		If Ascan(_aNFProd, {|_aVal| _aVal [2] + _aVal [3] == (_sAliasSD1)->D1_SERIORI + (_sAliasSD1)->D1_NFORI}) == 0 
			AADD (	_aNFProd, {SF1->F1_EMISSAO      ,;
				(_sAliasSD1)->D1_SERIORI 	,;
				(_sAliasSD1)->D1_NFORI   	,;
...
//----------------------------------------------------------
/*/{Protheus.doc} _MSG
Função auxiliar para tratamento de quebra dos dados adicionais p/
...
Static Function _MSG(_cObs, _nTam)
	...
	_cObs := iif(!empty(_cObs),StrTran(_cObs, " ", ";"),"")
	Do While At(";;", _cObs) != 0
		_cObs := StrTran(_cObs, ";;", ";")
	EndDo
