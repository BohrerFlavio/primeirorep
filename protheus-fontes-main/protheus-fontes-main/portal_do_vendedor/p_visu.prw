...
User Function p_visu(__aCookies,__aPostParms,__nProcID,__aProcParms,__cHTTPPage)
	Local cHTML:= ""
	Local cPedido:= ""
	Local nX:= 0
	...
	If !Empty(cPedido)
		...
		cHTML+= '<title>PORTAL DO VENDEDOR - Visualizar Pedido de Venda - '+cPedido+'</title>'
		cHTML+= "<p><img border='0' src='imagens/logo2.jpg'><br>"
		cHTML+= "<input type='button' value='Fechar' onclick='window.close()' />"
		cHTML+= '<div style="position:absolute;top:105;left:20">Cliente'
		...
		cHTML+= '</div>'
		cHTML+= '<div style="position:absolute;top:105;left:500">Loja'
		...
		cHTML+= '</div>'
		cHTML+= '<div style="position:absolute;top:130;left:20">Nome Cliente'
		...
		cHTML+= '</div>'
		cHTML+= '<div style="position:absolute;top:155;left:20">Cidade'
		...
		//cHTML+= '</div>'
		cHTML+= '<div style="position:absolute;top:180;left:20">Data'
		...
		cHTML+= '</div>'
		cHTML+= '<div style="position:absolute;top:180;left:500">Qt.Prev.Peso'
		...
		cHTML+= '</div>'
		cHTML+= '<div style="position:absolute;top:205;left:20">Qt.Prev.Caixa'
		cHTML+= '<input type="text" name="cab_caix" id="cab_caix" readonly="readonly" value="'+Transform(ZZ4->ZZ4_QPCAIX, '@e 999,999,999')+'" style="position:absolute;top:0;left:100;text-align:right;border:1px solid" maxlength="7" size="14" />'
		cHTML+= '</div>'
		cHTML+= '<div style="position:absolute;top:205;left:500">Total'
		cHTML+= '<input type="text" name="cab_tota" id="cab_tota" readonly="readonly" value="'+Transform(ZZ4->ZZ4_TOTAL, '@e 999,999,999.99')+'" style="position:absolute;top:0;left:100;text-align:right;border:1px solid" maxlength="6" size="14" />'
		cHTML+= '</div>'
		cHTML+= '<div style="position:absolute;top:230;left:20">Carregamento'
		...
		cHTML+= '</div>'  
		cHTML+= '<div style="position:absolute;top:230;left:500">Marca'
		...
		cHTML+= '<div style="position:absolute;top:255;left:20">Observacao'
		...
		cHTML+= '</div>'
		cHTML+= '<div id="grid" style="position:absolute;top:315;left:0px;width:98%;overflow:auto;">'
		...
		cHTML+= '<td bgcolor="#FFFFFF"><font face="arial" size="2">Qt.Prev.Cx</font></td>'
		cHTML+= '<td bgcolor="#FFFFFF"><font face="arial" size="2">Qt.Real.Cx</font></td>'
		cHTML+= '<td bgcolor="#FFFFFF"><font face="arial" size="2">Qt.Prev.Ps</font></td>'
		cHTML+= '<td bgcolor="#FFFFFF"><font face="arial" size="2">Qt.Real.Ps</font></td>'
		cHTML+= '<td bgcolor="#FFFFFF"><font face="arial" size="2">Prior.</font></td>'
		...
		Do While !ZZ5->(EOF()) .AND. xFilial("ZZ5")+cPedido == ZZ5->ZZ5_FILIAL+ZZ5->ZZ5_NUM
			...
			cHTML+= '<td><input type="text" readonly="readonly" style="border:none" value="'+EncodeUTF8(Transform(ZZ5->ZZ5_QRCAIX, "@e 999,999"))+'" size="7" /></td>'
			...
			cHTML+= '<td><input type="text" readonly="readonly" style="border:none" value="'+EncodeUTF8(Transform(ZZ5->ZZ5_QRPESO, "@e 99,999.99"))+'" size="9" /></td>'
			...
			cHTML+= '<td><input type="text" readonly="readonly" style="border:none" value="'+EncodeUTF8(Transform(ZZ5->ZZ5_TOLERA, "@e 99"))+'" size="2" /></td>'
			cHTML+= '<td><input type="text" readonly="readonly" style="border:none" value="'+EncodeUTF8(Transform(ZZ5->ZZ5_PRECO, "@e 999.99"))+'" size="6" /></td>'
			_cAux:= IIF(ZZ5->ZZ5_TPBONI=='D', 'Desconto', IIF(ZZ5->ZZ5_TPBONI=='A','Acrescimo',''))
