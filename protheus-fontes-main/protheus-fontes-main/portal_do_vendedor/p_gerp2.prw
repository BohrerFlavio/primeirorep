...
User Function p_gerp2(__aCookies,__aPostParms,__nProcID,__aProcParms,__cHTTPPage)   
	Local cUser:= ""
	Local cPass:= ""
	Local cCli:= ""
	Local cLoja:= ""
	...
	If !Empty(cUser) .AND. !Empty(cPass) .AND. !Empty(cMail)
		...
		cHTML+= "var url = '"+GetNewPar("SI_URLPORT", 'http://portaldovendedor.frigorificosilva.com.br:91')+"/u_p_brow2.apl?rdm='+encodeURI(Math.random())+'&dataini='+dini+'&datafin='+dfin+'&status='+stt+'&vendedor="+cUser+"';"
		...
		cHTML+= "<center><b>Lista Geral de Pedidos de venda</b><br><br></center><br>"
