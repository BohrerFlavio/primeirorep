...
User Function EMBALAGEM()
	...
	Private _cModoCap := 'S'    //Modo de captura do peso: S=Serial ou I=IP
	...
	elseif _cModoCap = 'I'
		...
		//Criando conexão ethernet
		oObj  := tSocketClient():New()
		nResp := oObj:Connect(1701, "10.0.0.207", 1000 )

		if !(nResp == 0 )
			alert( "Erro na Conexão IP! ", nResp)
			oStatus := '[Falha conexão IP]'
			_lFailCon := .t.
			...
			if( !oObj:IsConnected() )
				...
				alert( "Ainda está conectado, erro na desconexao" )
			endif
...
//Função que vai fazer a pesagem das caixas via porta Serial
//Utiliza porta COM3
...
Static Function Leitura(_cString)
	...
	_cCodPro  := substr(_cString,2,6)    //Codigo do produto da pre-etiqueta
	//_cSeqPETQ := substr(_cString,8,13)    //Sequencial da pré-etiqueta
	_cSeqPETQ := alltrim(substr(_cString,8,6))    //Sequencial da pré-etiqueta
	...
	if (len(alltrim(_cSeqPETQ)) < 6) .and. !empty(_cSeqPETQ)
		oSayCod:SetText(oCodigo)
		oDescri := ''
		oSayDesc:SetText(oDescri)
		//falha 5         
		_me1 := 'Leitura da pré-etiqueta(14)'
	...
	//a etiqueta testeira.
	//em caso negativo, continua o processo de 
	//produção da caixa  
	//alert(_cSeqPETQ + _cCodPro)
	SZ8->(DbSetOrder(13))
	if SZ8->(DbSeek(xfilial('SZ8') + dtos(date()) + _cSeqPETQ + _cCodPro))
		_lProduz  := .f. 
...
Static Function Proc01()
	...
	if !leitura(_cString)
		...
		if _lProduz
			...
			_nPeso := val(substr(_cString,15,13)) 

			//Verificação se o ultimo caractere verificador
			//da string existe  
			_cFim     := substr(_cString,21,1)     //caracter verficador do final do string 
			_cLimpa   := ''
			...
			if _cFim <> chr(03) 
				...
				oSayDesc:SetText(oDescri)
				//falha 5         
				_me1 := 'Erro leitura String(16) ' + _cLimpa
				_me2 := 'Rejeite acionado'                       
				rejeite(_nRejeite,_me1,_me2)    
				Mensagens('',_me1,'FA',_me2,16,_cStrBal,_cCodPro)

				if _cModoCap = 'S'
					_cLimpa := captura()     //para limpar o resíduo que tem na porta
				elseif _cModoCap = 'I'
					_cLimpa := CaptIP()   
				endif
			...
			if _nPeso <= 0
				...
				do case
					...
					//Caixa grande
					case (_nTS >= 0.800 .and. _nTS <= 1.150)
			...
			RegEven('Produção já existente','OK','Re-Impressão de etiqueta',13,_cStrBal,_cCodPro)
			oAut:refresh()
		endif    
...
Static Function Etiqueta()
	...
	Local _cModo  := GetMV('SI_MIMPEMB')   //Estes Parametros são da quente
	Local _cIpImp := GetMV('SI_IPIMP')
	//Local _cModo := GetMV('SI_IMPEMB2')     //Estes dois parametros
	//Local _cIpImp := GetMV('SI_IPIMP2')     //São para testes no Galpão
	...
	if SZ8->(DbSeek(xfilial('SZ8')+_cControl))  
		if _cModo = '1'   //Modo Automação
			...
			SZ8->Z8_TARA,SZ8->Z8_PREDES,SZ8->Z8_CLASSIF,SZ8->Z8_TF,SZ8->Z8_DATAP,SZ8->Z8_ETIQ,SZ8->Z8_DATAVAL,1,SZ8->Z8_LOTE,_cIpImp) 
		elseif _cModo = '2' //Modo manual
			...
			SZ8->Z8_TARA,SZ8->Z8_PREDES,SZ8->Z8_CLASSIF,SZ8->Z8_TF,SZ8->Z8_DATAP,SZ8->Z8_ETIQ,SZ8->Z8_DATAVAL,1,_cIpImp) 
		elseif _cModo = '3' //Modo Automação com conexão ethernet
			u_GJF111b("S600","IP",SZ8->Z8_CONTROL,SZ8->Z8_COD,SZ8->Z8_QUANT,SZ8->Z8_PESOBR,SZ8->Z8_PESO,;  
			SZ8->Z8_TARA,SZ8->Z8_PREDES,SZ8->Z8_CLASSIF,SZ8->Z8_TF,SZ8->Z8_DATAP,SZ8->Z8_ETIQ,SZ8->Z8_DATAVAL,1,SZ8->Z8_LOTE,_cIpImp)      		
		endif 		      
	endif
return .t.         
