...
±±ºPrograma  ³GJF129   º Autor ³ Giuliano Forgiariniº Data ³  29/12/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³Rotina de controle de automação da embalagem modelo II      º±±
±±º          ³                                                            º±±
...
User Function GJF129()
	...
	if  empty(mv_par01) .or. empty(mv_par02) .or. empty(mv_par03) 
		//falha 1
		alert('Parametros iniciais em branco!(1)')
		return
	endif  
	...
	//@ 230,335  BUTTON 'Exc.Logs' SIZE 40,15 ACTION ExcEven()  OBJECT oBtn4
...
Static Function  Rejeite(_par,_m1,_m2)
	if _par = 1
		...
		u_GJF111c(alltrim(mv_par02),alltrim(mv_par03),_m1,_m2)
...
Static Function Leitura(_cString)
	_cNumPrev := ''
	_cPreDes  := ''

	_cCodPro  := substr(_cString,2,6)
	//_cCodPro  := substr(_cString,mv_par04,mv_par05)
	...
	//Verificar se conseguiu pegar alguma coisa
	if empty(_cCodPro)
		return .f. 
	endif
	...
	if !SB1->(DbSeek(xfilial('SB1')+_cCodPro))
		...
		Mensagens('',_me1,'FA',_me2,5,_cStrBal)
	...
	if _cSegUM = 'PC'
		...
		_me2 := 'Rejeite acionado'
		Mensagens('',_me1,'FA',_me2,6,_cStrBal)
		rejeite(_nRejeite,_me1,_me2) 

		return .f.  
	endif

	//3º Verificar se existe previsão de produção do produto
	...
	if VER->CONTA = 0
		...
		_me2 :=	'Rejeite acionado'
		Mensagens('',_me1,'FA',_me2,7,_cStrBal) 
		rejeite(_nRejeite,_me1,_me2)   	
		return .f.  
	endif

	//4º Coletar os dados necessários do produto para registro de produção   
	if !empty(oCodigo) 
		oSayCod:SetText(oCodigo)
		oSayDesc:SetText(oDescri)  
		oMens1 := ''          
...
Static Function Proc01()
	...
	//Faz uma segunda tentativa de captura
	if empty(_cString) 
		_cString := Captura()
	endif
	...
	//Parametro que, se ativado, mostra em alert a string pura sem modelagem
	if mv_par08 = 2
		alert(_cString)
	endif
	...
	if len(_cString) = 13
		_nPeso := val(substr(_cString,9,13))
		//_nPeso := val(substr(_cString,mv_par06,mv_par07))
	else
		_nPeso := val(substr(_cString,15,19))
	endif
	...
	if _nPeso <= 0  
		...
		_me2 := 'Rejeite acionado'
		Mensagens('',_me1,'FA',_me2,8,_cStrBal)  
		rejeite(_nRejeite,_me1,_me2)     
		...
		if !SB1->(DbSeek(xfilial('SB1')+_cCodPro))  
			...
			_me2 := 'Rejeite acionado'
			Mensagens('',_me1,'FA',_me2,9,_cStrBal) 
			rejeite(_nRejeite,_me1,_me2)  
			return .f.
		endif 
		...
		_NTARAp := FBuscaCPO('SB1',1,xfilial('SB1')+SB1->B1_COD,'B1_CTARAP')      // Linhas inseridas para buscar
		...
		if _nPesoL <= 0
			...
			_me2 := 'Rejeite acionado'
			Mensagens('',_me1,'FA',_me2,14,_cStrBal)  
			rejeite(_nRejeite,_me1,_me2)     
			return .f.
		endif
		...
		do case 
			...
			if !(_nPesoL >= 5 .and. _nPesoL <= 18)
				...
				_me2 := 'Rejeite acionado'
				Mensagens('',_me1,'FA',_me2,10,_cStrBal)  
				rejeite(_nRejeite,_me1,_me2)     
				return .f.
			endif
			//Caixa grande	
			case (_nTS >= 0.800 .and. _nTS <= 1.000)
			if !(_nPesoL >= 10 .and. _nPesoL <= 31)
				//Falha 11
				_me1 := 'Peso Inconsistente por tara!(11)'
				_me2 := 'Rejeite acionado'
				Mensagens('',_me1,'FA',_me2,11,_cStrBal)  
				rejeite(_nRejeite,_me1,_me2)     
				return .f.
			endif   
			//Caixa plástica	
			case (_nTS >= 2.100 .and. _nTS <= 2.255)
			if !(_nPesoL >= 2 .and. _nPesoL <= 40)
				//Falha 12 
				_me1 := 'Peso Inconsistente por tara!(12)'
				_me2 := 'Rejeite acionado'
				Mensagens('',_me1,'FA',_me2,12,_cStrBal)    
				rejeite(_nRejeite,_me1,_me2)       
				return .f.
			endif
			...
			_me2 := 'Rejeite acionado'
			Mensagens('',_me1,'FA',_me2,13,_cStrBal)    
			rejeite(_nRejeite,_me1,_me2)       
		endcase
	...
	RegEven('Registro efetivado','OK','Etiqueta enviada',0,_cStrBal) 
...
static function previsao()
	...
	if !SZU->(DbSeek(xfilial('SZU')+_cPreEmb))
		//	Falha 13 
		_me1 := 'Produção inexistente para esse produto!(13)'
		_me2 := 'Rejeite acionado'
		Mensagens('',_me1,'FA',_me2,13,_cStrBal)  
		rejeite(_nRejeite,_me1,_me2) 
		return .f.
	...
	do case
		...
		if SZU->(ZU_QPCAIX + round(ZU_QPCAIX *(ZU_TOLERA/100),1) ) = qRealC + 1    
		...
		if (qRealQ + _nQuant) > qPrevQ
			Mensagens('','Produção encerrada antecipadamente!','OK','Programação PCP',0,_cStrBal)  
			if _cCExat = 'S'                      //Variavel que determina se o cumprimento da previsão deve ser exato ou não
				overflow := .t.                    //se a previsão foi cumprida, então o overflow determina o seu encerramento  
			endif
			overflow2 := .t.
...
Static Function Etiqueta()
	...
	if SZ8->(DbSeek(xfilial('SZ8')+_cControl)) 
		if mv_par09 = 1
			u_GJF111b(alltrim(mv_par02),alltrim(mv_par03),SZ8->Z8_CONTROL,SZ8->Z8_COD,SZ8->Z8_QUANT,SZ8->Z8_PESOBR,SZ8->Z8_PESO,;  
			SZ8->Z8_TARA,SZ8->Z8_PREDES,SZ8->Z8_CLASSIF,SZ8->Z8_TF,SZ8->Z8_DATAP,SZ8->Z8_ETIQ,SZ8->Z8_DATAVAL,1,SZ8->Z8_LOTE)  
		else
			u_GJF111a(alltrim(mv_par02),alltrim(mv_par03),SZ8->Z8_CONTROL,SZ8->Z8_COD,SZ8->Z8_QUANT,SZ8->Z8_PESOBR,SZ8->Z8_PESO,;  
			SZ8->Z8_TARA,SZ8->Z8_PREDES,SZ8->Z8_CLASSIF,SZ8->Z8_TF,SZ8->Z8_DATAP,SZ8->Z8_ETIQ,SZ8->Z8_DATAVAL,1,SZ8->Z8_LOTE)  	
		endif	      
	endif
return .t.         
...
Static Function RegEven(_desc,_status,_resp,_cod,_string) 
...
Static Function Mensagens(_mens1,_mens2,_even1,_even2,_cod,_String)
	oMens1 := _mens1          
	oSayMens1:SetText(oMens1)
	oMens2 := _mens2 
	oSayMens2:SetText(oMens2)
	oAut:refresh() 
	RegEven(oMens2,_even1,_even2,_cod,_String)
return
