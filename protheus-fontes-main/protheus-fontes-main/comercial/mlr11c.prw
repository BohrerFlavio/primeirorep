...
User Function MLR11c()
	...
	Local cDesc2          := "para conferencia do Faturamento por Cliente/Produto Para familia 105"
	Local cDesc3          := ""
	//Local cPict           := "xxxxxxxxx"
	Local titulo          := "RELATORIO DE CONF. DE FATURAMENTO POR CLIENTE/PRODUTO"
	Local Cabec1          := "Estado           Cod. Prod.                Cliente                                            Peso           Valor"
	Local Cabec2          := "Cod. Cliente                                                                                  (Kg)            (R$)"                   
	//Local imprime         := .T.
	Local aOrd            := {}  
	Private nLin          := 80
	Private lEnd          := .F.
	...
	Private limite        := 80
	Private tamanho       := "M"
	Private nomeprog      := "MLR11" // Coloque aqui o nome do programa para impressao no cabecalho
	Private nTipo         := 18
	...
	Private CONTFL        := 01
	Private m_pag         := 01
	Private wnrel         := "MLR11" // Coloque aqui o nome do arquivo usado para impressao em disco    
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	While CLI->(!EOF())
		...
		if FAT->TOTAL <> 0 .or. DEV->TOTAL <> 0
			...
			_cDescCli := fBuscaCPO('SA1',1,xfilial('SA1') + CLI->CLIENTE + CLI->LOJA,'A1_NOME')
			...
			if _cCli <> CLI->CLIENTE
				...
				@nlin,092 psay "Peso(Kg):"
				@nlin,107 psay "Valor(R$): "
				nlin++
				_cCli  := CLI->CLIENTE
				_cLoj  := ''
			endif
			...
			if _cLoj <> CLI->LOJA
				...
				if mv_par11 = 2 //se for analitico
					@nlin,031 psay '|' + replicate('-',19) + "PRODUTOS FATURADOS" + replicate('-',19) + '|'
				endif
				nlin++
				_cLoj  := CLI->LOJA
			...
			while FAT->(!eof())
				if mv_par11 = 2	//se for analitico
					...
					_cfam3	:=  fBuscaCPO('SB1',1,xfilial('SB1') + FAT->PROD,'B1_FAM')
					
					if 	alltrim(_cfam3) == '105'
						// Deixa aparecer Normal
						_cDescPro := fBuscaCPO('SB1',1,xfilial('SB1') + FAT->PROD,'B1_DESC')
						@nlin,010 psay " | "
						@nlin,019 psay alltrim(FAT->PROD)
						@nlin,030 psay " | "
						@nlin,040 psay substr(_cDescPro,1,35)
						@nlin,087 psay " | "
						@nlin,090 psay Transform(FAT->PESO,'@E 999,999.99') + " | "
						@nlin,105 psay Transform(FAT->TOTAL,'@E 999,999.99')  + " | "
						nlin++
		...
		if (_cLoj <> CLI->LOJA .or. _cCli <> CLI->CLIENTE) .or. CLI->(eof())
			...
			if _nTotDevPesLoj <> 0 .and. _nTotDevTotLoj <> 0     
				...
				nlin++
				@nlin,020 psay "Total Faturado da Loja --------> "
				@nlin,086 psay transform(_nPesLoj,'@E 999,999,999.99')
				@nlin,101 psay transform(_nTotLoj,'@E 999,999,999.99')
				nlin++
				@nlin,020 psay "Total de Devoluções da Loja --------> "
				@nlin,086 psay transform(_nDevPesLoj,'@E 999,999,999.99')
				@nlin,101 psay transform(_nDevTotLoj,'@E 999,999,999.99')
				nlin++
				@nlin,020 psay "Total Liquido da Loja ------------------> "
				@nlin,086 psay transform(_nTotDevPesLoj,'@E 999,999,999.99')
				@nlin,101 psay transform(_nTotDevTotLoj,'@E 999,999,999.99')
				nlin++
			endif
		...
		if (_cCli <> CLI->CLIENTE .or. CLI->EST<>_cEst) .or. CLI->(eof())
			...
			if _nTotDevPesCli <> 0  .and. _nTotDevTotCli <> 0
				nlin++
				@nlin,020 psay "Total Faturado do Cliente -----------> "
				@nlin,086 psay transform(_nPesCli,'@E 999,999,999.99')
				@nlin,101 psay transform(_nTotCli,'@E 999,999,999.99')
				nlin++
				@nlin,020 psay "Total de Devoluções do Cliente --------> "
				@nlin,086 psay transform(_nDevPesCli,'@E 999,999,999.99')
				@nlin,101 psay transform(_nDevTotCli,'@E 999,999,999.99')
				nlin++
				@nlin,020 psay "Total Liquido do Cliente ------------------> "
				@nlin,086 psay transform(_nTotDevPesCli,'@E 999,999,999.99')
				@nlin,101 psay transform(_nTotDevTotCli,'@E 999,999,999.99')
				nlin++
			endif
	...
	@nlin,000 psay replicate('-',132)			
	nlin++
	@nlin,020 psay "Total Bruto Faturado ------------------> "
	@nlin,086 psay transform(_nGeralPes,'@E 999,999,999.99')
	@nlin,101 psay transform(_nGeralTot,'@E 999,999,999.99')
	nlin++														 
	@nlin,020 psay "Total Liquido Faturado -------------------> "
	@nlin,086 psay transform(_nPesoGerDev,'@E 999,999,999.99')
	@nlin,101 psay transform(_nTotGerDev,'@E 999,999,999.99')
	nlin++																		                                                                                                                             	
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
Static Function TrataFat()
	...
	_cQuery += " FROM "  + RetSQLTab('SD2') + ", " + RetSQLTab('SF4') + ", " + RetSQLTab('SA1') + ", " + RetSQLTab('SF2') + ", " + RetSQLTab('SB1')
	_cQuery += " WHERE " + RetSQLFil('SD2') + " AND " + RetSQLFil('SA1') + " AND " + RetSQLFil('SF4') + " AND " + RetSQLFil('SF2') + " AND"  + RetSQLFil('SB1') + " AND"
	...
	_cQuery += " D2_COD = B1_COD AND  F4_TIPO = 'S' AND F4_DUPLIC = 'S' AND"
	_cQuery += " B1_FAM = '" + _cfam + "' AND"
	...
	_cQuery += " AND " + RetSQLDel('SD2') + " AND " + RetSQLDel('SA1') + " AND " + RetSQLDel('SF4') + " AND " + RetSQLDel('SF2') + " AND " + RetSQLDel('SB1')
...
Static Function TrataDev()
	...
	_cQuery2 += " FROM " + RetSQLTab('SD1') + "," + RetSQLTab('SA1') + ", " + RetSQLTab('SF4') + ", " + RetSQLTab('SF1') + ", " + RetSQLTab('SB1')
	...
	_cQuery2 += " D1_COD = B1_COD AND D1_TES = F4_CODIGO  AND"
	_cQuery2 += " F4_TIPO = 'E' AND F4_DUPLIC = 'S' AND F1_TIPO = 'D' AND"  
	_cQuery2 += " B1_FAM = '" + _cfam2 + "' AND"
	...
	_cQuery2 += " AND " + RetSQLDel('SD1') + " AND " + RetSQLDel('SA1') +" AND " + RetSQLDel('SF4') + " AND " + RetSQLDel('SF1') + " AND " + RetSQLDel('SB1')
...
Static Function TrataCli()
	...
	_cQuery3 := " SELECT A1_COD AS CLIENTE, A1_LOJA AS LOJA, A1_EST AS EST,A1_VEND AS VEND
	...
	_cQuery3 += " GROUP BY A1_EST, A1_COD, A1_LOJA,A1_VEND"
