...
User Function ML_310()
	...
	±±³              ³        ³      ³D1_CUSTO.                               ³±±
	±±³Iusp a        ³19/12/00³7170  ³parametro inclui devol. compra s/n      ³±±
	±±³Evandro Mugnol³04/11/04³XXXXXX³Incluido controle de Qtde na Segunda UM ³±±
...
Static Function C310Imp(lEnd,WnRel,cString,titulo,Tamanho)
	...
	LOCAL nCntImpr   := 0
	LOCAL lDevolucao := .F.
	LOCAL nAp1,nAp2,nAp3,nAp4,nAp5,nAp12
	LOCAL nAt1,nAt2,nAt3,nAt4,nAt5,nAt12
	LOCAL nAg1,nAg2,nAg3,nAg4,nAg5,nAg12
	LOCAL cCodAnt    := "",cTipant := "",cUm := "",cSegUm := "",nTotFat:=0,nTotLuc:=0
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	nMoeda := mv_par09
	titulo += " - "+GetMv("MV_SIMB"+Alltrim(Str(nMoeda)))
	...
	*****                     01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
	...
	aTam := TamSx3("D2_UM")
	Aadd(aCampos,{"D2_UM","C",aTam[1],aTam[2]})
	aTam := TamSx3("D2_QTSEGUM")
	Aadd(aCampos,{"D2_QTSEGUM","N",aTam[1],aTam[2]})
	aTam := TamSx3("D2_SEGUM")
	Aadd(aCampos,{"D2_SEGUM","C",aTam[1],aTam[2]})
	...
	U_ArqTrb("Cria", "TRB", aCampos, {}, @_aArqTrb)
		...
		If !(TRB->D2_ORIGLAN == "LF") .And. AvalTes(TRB->D2_TES,cEstoq,cDupli)
			...
			Do Case
				...
				If DbSeek( xFilial( "SD1" )+TRB->D2_COD,.F. )
					While !SD1->(Eof()) .And. (SD1->D1_COD == TRB->D2_COD)
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
						//³ Despreza Notas Fiscais Lancadas Pelo Modulo do Livro Fiscal    ³
						//³ Despreza Itens em que a TES NAO Se Ajusta a Selecao do Usuario ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	While lContinua .And. !TRB->(Eof())
		...
		While lContinua .And. !TRB->(Eof()) .And. (TRB->D2_TP == cTipant)
			...
			Else
						...
						If !(TRB->D2_ORIGLAN == "LF") .And. AvalTes(TRB->D2_TES,cEstoq,cDupli)
							If TRB->D2_TIPO != "D"
								...
								If (mv_par09 > 1)
									...
									If cPaisLoc=="BRA"
										nAp5 += xMoeda( TRB->D2_TOTAL+IF(mv_par10 == 1,TRB->D2_VALIPI,0), Val(SB1->B1_MCUSTD), nMoeda, TRB->D2_EMISSAO )
									Else
										nAp5 += xMoeda( SumTaxSD2(.F.), Val(SB1->B1_MCUSTD), nMoeda, TRB->D2_EMISSAO )
									EndIf
								Else
									nAp3 += ValSD2()
									If cPaisLoc=="BRA"
							...
							Do Case
								...
								If DbSeek( xFilial( "SD1" )+TRB->D2_COD+TRB->D2_SERIE+TRB->D2_DOC+TRB->D2_ITEM,.F. )
									...
									If !(SD1->D1_ORIGLAN == "LF") .And. AvalTes(SD1->D1_TES,cEstoq,cDupli)
										...
										If (mv_par09 > 1)
											...
											Else
												nAp5 -= xMoeda(SumTaxSD1(.F.), Val(SB1->B1_MCUSTD), nMoeda, SD1->D1_DTDIGIT)
											EndIf
										Else
											nAp3 -= ValSD1()
											If cPaisLoc="BRA"
								...
								DbSelectArea( "SD1" )
								If DbSeek( xFilial( "SD1" )+TRB->D2_COD,.F. )
									While !SD1->(Eof()) .And. (D1_COD == cCodAnt)
										//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
										//³ Despreza Notas Fiscais Lancadas Pelo Modulo do Livro Fiscal    ³
										//³ Despreza Itens em que a TES NAO Se Ajusta a Selecao do Usuario ³
										//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
										If !(D1_ORIGLAN == "LF") .And. AvalTes(D1_TES,cEstoq,cDupli)
											...
											If (mv_par09 > 1)
												...
												Else
													nAp5 -= xMoeda( SumTaxSD1(.F.), Val(SB1->B1_MCUSTD), nMoeda, D1_DTDIGIT )
												EndIf
											Else
												nAp3 -= ValSD1()
												If cPaisLoc = "BRA"
				...
				If nAp1 != 0 .Or. nAp2 != 0 .Or. nAp3 != 0 .Or. nAp4 != 0 .Or. nAp12 != 0
					If li > 55
					...
					@ li,078 PSay cUm      // SB1->B1_UM
					@ li,084 PSay nAp12 Picture PesqPictQt("D2_QTSEGUM",16)
					@ li,105 PSay cSegUm   // SB1->B1_SEGUM
		...
		If lPassou
			...
			@ li,084 PSay nAt12 Picture PesqPictQt("D2_QTSEGUM",16)
	...
	If li != 80
		...
		@ li,084 PSay nAg12 Picture PesqPictQt("D2_QTSEGUM",16)
	...
	DbSelectArea( "SD1" ) ; RetIndex( "SD1" )
	DbSelectArea( "SD2" ) ; RetIndex( "SD2" )
...
Static Function GeraTrab(cAlias)
	...
	dbSelectArea(cAlias)
	dbGoTop()
	While !Eof() .And. &(Subs(cAlias,2,2)+"_FILIAL") == xFilial()
		If cAlias == "SD2" 
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		...
		Replace TRB->D2_UM      With &(cAlias+"->"+Subs(cAlias,2,2)+"_UM")
		Replace TRB->D2_QTSEGUM With &(cAlias+"->"+Subs(cAlias,2,2)+"_QTSEGUM")
		Replace TRB->D2_SEGUM   With &(cAlias+"->"+Subs(cAlias,2,2)+"_SEGUM")
