...
Static Function FinR190R3()
	...
	Private nomeprog	:= "STI_R190"
	Private aLinha  	:= { }
	Private nLastKey	:= 0
	Private cPerg		:= "STI_R190"
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	wnrel := "STI_R190"            //Nome Default do relat¢rio em Disco
...
Static Function FA190Imp(lEnd,wnRel,cString)
	...
	Local cExp 			:= ""
	Local CbTxt,CbCont
	Local nValor:=0,nDesc:=0,nJuros:=0,nMulta:=0,nJurMul:=0,nCM:=0,nVlMovFin:=0
	...
	Local tamanho		:="G"
	Local aCampos:= {},nVlr,cLinha,lOriginal:=.T.
	Local nAbat 		:= 0
	Local cHistorico
	Local lManual 		:= .f.
	Local cTipodoc
	...
	If TcSrvType() != "AS/400" .and. TCGetDB() != "SYBASE"
		...
		// Incluído em 05/02/2022 por solicitação do Clailton para ter uma pergunta adicional
		// pedindo quais baixas serão impressas. Se não for nenhuma das 3 abaixo, imprime todas.
		If mv_par43 == 1								// Baixas Automáticas (Retorno Bancário)
			cQuery += " AND E5_ORIGEM = 'FINA200' "
		ElseIf mv_par43 == 2							// Baixas Manuais
			cQuery += " AND E5_ORIGEM = 'FINA070' "
		ElseIf mv_par43 == 3							// Outras Baixas (Compensação, Liquidação, Etc.)
			cQuery += " AND E5_ORIGEM NOT IN ('FINA070','FINA200') "
		EndIf
