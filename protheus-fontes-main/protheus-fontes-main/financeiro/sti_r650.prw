...
±±³Fun‡…o    ³ STI_R650 ³ Autor ³ Marcel Borges Ferreira ³ Data ³ 03/08/06 ³±±
...
Static Function ReportDef(aParam)
	...
	oReport := TReport():New("STI_R650",STR0004,"FIN650",{|oReport| ReportPrint(oReport)},STR0001+STR0002+STR0003)
	...
	TRCell():New(oSection1,"SEC1_NOM",,"N o m e",,30,,)
...
//Busca tamanho do detalhe na configuraçãodo banco
	...
	For nZ:= 1 to len(aRetSX5) 		// Filial, Tabela, Chave, Descricao
		AAdd( aTabela, { AllTrim(aRetSX5[nZ][4]), Padr(aRetSX5[nZ][3],Len(IIF(mv_par07==1,SE1->E1_TIPO,SE2->E2_TIPO)))} )
	Next
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	oSection1:Cell("SEC1_NOM"):SetBlock({||cNomeCF})
			...
			If SubStr(xBuffer,1,1) $ "1#F#J#7#2" .or. Substr(xBuffer,1,3) == "001"
				...
				If !Empty(cTipo)
					...
					Else
						...
						If mv_par07==1
							cNomeCF := GetAdvFVal("SA1", "A1_NOME", xFilial("SA1") + SE1->E1_CLIENTE + SE1->E1_LOJA, 1, Space(TamSx3("A1_NOME")[1]), .T.) 
						Else
							cNomeCF := GetAdvFVal("SA2", "A2_NOME", xFilial("SA2") + SE2->E2_FORNECE + SE2->E2_LOJA, 1, Space(TamSx3("A2_NOME")[1]), .T.) 
							cNomeCFB:= GetAdvFVal("SA2", "A2_BNOME", xFilial("SA2") + SE2->E2_FORNECE + SE2->E2_LOJA, 1, Space(TamSx3("A2_NOME")[1]), .T.) 
							
							cNomeCF := IIF(!Empty(cNomeCFB),cNomeCFB,cNomeCF)					
						EndIF 
					EndIF
					If cEspecie $ MVABATIM			// Nao lˆ titulo de abatimento
						nLidos += nTamDet
