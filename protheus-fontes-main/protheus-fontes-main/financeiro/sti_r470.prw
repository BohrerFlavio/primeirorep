...
Static lFWCodFil := .T.
Static lE5TXMoeda
// 22/01/2018 - Convertido para User Function a fim de permitir gerar no layout da versão anterior
...
Static Function ReportPrint(oReport)
	...
	Local cAliasSE5	:= "SE5"
	Local cSql1:= ""
	Local nMoeda	:= GetMv("MV_CENT"+(IIF(mv_par06 > 1 , Alltrim(mv_par06),""))) 
	...
	If lTop .and. IIF( cPaisLoc $ "BRA" ,Mv_par10 == 2,.T.) .AND. FWModeAccess("SA6",3,) == 'C' .and. FWModeAccess("SE8",3,) == 'E' // recompor saldo por Banco
	...
	If	lAllFil
		cOrder  := "%E5_DTDISPO,E5_BANCO,E5_AGENCIA,E5_CONTA,SE5.R_E_C_N_O_,E5_NUMCHEQ,E5_DOCUMEN,E5_PREFIXO,E5_NUMERO%"
	Else
	...
	oMovBanc:Cell("SALDO ATUAL"		):SetBlock({|| nSaldoAtu += Iif(lMoedBco, Round(xMoeda( Iif(VerMoed(nMoedaBco, @nMoedTit), E5_VALOR, E5_VLMOED2), nMoedTit, mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda) ;
	...
	If lMultSld
		If lMoedBco
			oMovBanc:Cell("E5_VALOR-ENTRAD"  ):SetBlock({|| If((cAliasSE5)->E5_RECPAG == "R", Round(xMoeda( Iif(VerMoed(nMoedaBco, @nMoedTit), E5_VALOR, E5_VLMOED2), nMoedTit, mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda), Iif(lDvc, 0, nil))})
			oMovBanc:Cell("E5_VALOR-SAIDA"   ):SetBlock({|| If((cAliasSE5)->E5_RECPAG == "P", Round(xMoeda( Iif(VerMoed(nMoedaBco, @nMoedTit), E5_VALOR, E5_VLMOED2), nMoedTit, mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda), Iif(lDvc, 0, nil))})
		Else
	...
	Else
		If lMoedBco
			oMovBanc:Cell("E5_VALOR-ENTRAD"  ):SetBlock({||If((cAliasSE5)->E5_RECPAG == "R", Round(xMoeda( Iif(VerMoed(nMoedaBco, @nMoedTit), E5_VALOR, E5_VLMOED2),;
			Iif((cPaisLoc<>"BRA".And.((cAliasSE5)->E5_TIPODOC $ MVRECANT+"|ES") .and. Empty((cAliasSE5)->E5_MOEDA)),1, nMoedTit ),;
			mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda),Iif(lDvc, 0, nil))})

			oMovBanc:Cell("E5_VALOR-SAIDA"   ):SetBlock({||; 
			If((cAliasSE5)->E5_RECPAG == "P", ;
			Round(	xMoeda( Iif(VerMoed(nMoedaBco, @nMoedTit), E5_VALOR, E5_VLMOED2),Iif((cPaisLoc<>"BRA".And.((cAliasSE5)->E5_TIPODOC $ MVRECANT+"|ES") .and. Empty((cAliasSE5)->E5_MOEDA)),1, nMoedTit ),;
			mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda)	,Iif(lDvc, 0, nil))})
	...
	While !oReport:Cancel() .And. (cAliasSE5)->(!Eof())
		...
		While !oReport:Cancel() .And. !(cAliasSE5)->(Eof())
			...
			oMovBanc:PrintLine()
			If lMoedBco
				lMoedaMov := VerMoed(nMoedaBco, @nMoedTit) 
				If Empty((cAliasSE5)->E5_RECONC) .AND. (cAliasSE5)->E5_RECPAG == "R"			
					aRecon[1][REC_NAO_CONCILIADO] += Round(xMoeda( Iif(lMoedaMov, E5_VALOR, E5_VLMOED2),;
					Iif((cPaisLoc<>"BRA".And.((cAliasSE5)->E5_TIPODOC $ MVRECANT+"|ES") .and. Empty((cAliasSE5)->E5_MOEDA)),1, nMoedTit ),;
					mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda)
				ElseIf E5_RECPAG == "R"			
					aRecon[1][REC_CONCILIADO] += Round(xMoeda( Iif(lMoedaMov, E5_VALOR, E5_VLMOED2),;
					Iif((cPaisLoc<>"BRA".And.((cAliasSE5)->E5_TIPODOC $ MVRECANT+"|ES") .and. Empty((cAliasSE5)->E5_MOEDA)),1, nMoedTit ),;
					mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda)
				ElseIf Empty( E5_RECONC ) .AND. E5_RECPAG == "P"			
					aRecon[1][PAG_NAO_CONCILIADO] += Round(xMoeda( Iif(lMoedaMov, E5_VALOR, E5_VLMOED2),;
					Iif((cPaisLoc<>"BRA".And.((cAliasSE5)->E5_TIPODOC $ MVRECANT+"|ES") .and. Empty((cAliasSE5)->E5_MOEDA)),1, nMoedTit ),;
					mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda)
				ElseIf E5_RECPAG == "P"			
					aRecon[1][PAG_CONCILIADO] += Round(xMoeda( Iif(lMoedaMov, E5_VALOR, E5_VLMOED2),;
					Iif((cPaisLoc<>"BRA".And.((cAliasSE5)->E5_TIPODOC $ MVRECANT+"|ES") .and. Empty((cAliasSE5)->E5_MOEDA)),1, nMoedTit ),;
					mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda)
				EndIf
			Else
...
Static Function FinR470R3()
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	wnrel := "STI_R470"            //Nome Default do relatorio em Disco
...
Static Function FA470Imp(lEnd,wnRel,cString)
	...
	If TcSrvType() != "AS/400"
		...
		cOrder	  := "E5_DTDISPO, E5_BANCO, E5_AGENCIA, E5_CONTA, E5_NUMCHEQ, R_E_C_N_O_,E5_PREFIXO,E5_NUMERO"
	...
	*/
	...
	If !lLayout//Paisagem
		While &(cCondWhile)
			...
			If E5_RECPAG == "P"
				nMoedTit := Posicione("SE2",1,xFilial("SE2")+(E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_TIPO),"E2_MOEDA")
			Else
				nMoedTit := Posicione("SE1",1,xFilial("SE1")+(E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_TIPO),"E1_MOEDA")
			EndIf	
			...
			If !Empty(E5_TXMOEDA) .And. ( lFxMultSld .AND. FXMultSld() )
				...
				If Empty(nMoedTit)
					lMsmMoeda := .T.	
				Else
					lMsmMoeda := Iif(nMoedTit == 0, .T., (nMoedTit == mv_par06))  
				EndIf
			EndIf
			...
			If Empty(nMoedTit)
				lMoedaMov := .T.
			Else
				lMoedaMov := Iif(nMoedTit == 0, .T., (nMoedTit == nMoedaBco)) 	 
			EndIf
			// Se realiza controle de saldos em multiplas moedas
			If ( lFxMultSld .AND. FXMultSld() )
				If lMoedBco
					nValor   := Round(xMoeda( Iif(lMoedaMov, E5_VALOR, E5_VLMOED2), nMoedTit, mv_par06, IIF(MV_PAR09==1,dDataBase,E5_DTDISPO), IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedTit), TxMoeda(cAlias, nMoedTit))),nMoeda)
				Else
		...
		If ((li > nLin - 5 .And. nLin >  46) .or. ( li > 58)) .Or. !lExistSE5
		...
		li+=2
		If (li > nLin - 5 .And. nLin >  46) .or. ( li > 58)
			cabec(@titulo,cabec1,cabec2,nomeprog,tamanho,nTipo)
			@ li, 00 PSAY cabec3
			@ li,153 + Iif(cPaisLoc <> "BRA", Iif( cPaisLoc == "MEX", 15, 4 ), 0 ) PSAY nSaldoAtu Picture tm(nSaldoAtu,16,nMoeda)
			li++
...
Static Function FR470Tab14()
	...
	for nZ:= 1 to len(aRetSX5) 		// Filial, Tabela, Chave, Descricao
		cTabela14 += AllTrim(aRetSX5[nZ][3]) + "/"
	Next
...
Static Function VerMoed(nMoedaBco, nMoedTit)
