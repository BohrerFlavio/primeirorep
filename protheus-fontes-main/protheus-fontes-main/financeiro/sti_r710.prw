...
ฑฑณFuno	 ณ STI_R710	ณ Autor ณ Wagner Xavier 	    ณ Data ณ 05.10.92 ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณDescrio ณ Bordero de Pagamento.									  ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณSintaxe e ณ STI_R710(void)	.					  					  ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณParametrosณ 															  ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤดฑฑ
...
Local oSection1
Local oSection2
Local cReport 	:= "STI_R710" 				// Nome do relatorio
...
//TRCell():New(oSection2, "A6_NREDUZ" , "SA6", ""		 	, PesqPict("SA6","A6_NREDUZ") , 15						,/*lPixel*/,/*CodeBlock*/)	//"BANCO"
//TRCell():New(oSection2, "EF_NUM"    , "SEF", STR0047 		, PesqPict("SEF","EF_NUM")    , TamSX3("EF_NUM")[1]     ,/*lPixel*/,/*CodeBlock*/)	//"HISTORICO"
TRCell():New(oSection2, "A2_FBANCO" , "SA2", STR0048 		, PesqPict("SA2","A2_FBANCO") , TamSX3("A2_FBANCO")[1]  ,/*lPixel*/,/*CodeBlock*/)	//"BCO"
TRCell():New(oSection2, "A2_FAGENCI", "SA2", STR0049 		, PesqPict("SA2","A2_FAGENCI"), TamSX3("A2_FAGENCI")[1] ,/*lPixel*/,/*CodeBlock*/)	//"AGENC"
TRCell():New(oSection2, "A2_FDVAGE" , "SA2", "DV"    		, PesqPict("SA2","A2_FDVAGE") , TamSX3("A2_FDVAGE")[1]	,/*lPixel*/,/*CodeBlock*/)  //"DV AGENCIA"
TRCell():New(oSection2, "A2_FNUMCON", "SA2", "NUM. CONTA" 	, PesqPict("SA2","A2_FNUMCON"), TamSX3("A2_FNUMCON")[1] ,/*lPixel*/,/*CodeBlock*/)	//"NUMERO CONTA"
TRCell():New(oSection2, "A2_FDVCTA" , "SA2", "DV"    		, PesqPict("SA2","A2_FDVCTA") , TamSX3("A2_FDVCTA")[1]  ,/*lPixel*/,/*CodeBlock*/)  //"DV CONTA"
TRCell():New(oSection2, "A2_CGC"  	, "SA2", STR0051 		, "@R XXXXXXXXXXXXXXXX"       , 18						,/*lPixel*/,/*CodeBlock*/) 	//"CNPJ/CPF"
...
TRCell():New(oSection2, "CHAVEPIX"  , ""   , "PIX" 	 		, PesqPict("F72","F72_CHVPIX"), 35						,/*lPixel*/,/*CodeBlock*/) 	//"PIX"

//oSection2:Cell("A2_FDVAGE"):SetCanPrint( { || !Empty( F710BcoFor("FA2_AGENCI", cAliasSea) ) } )
//oSection2:Cell("A2_FDVCTA"):SetCanPrint( { || !Empty( F710BcoFor("FA2_NUMCON" , cAliasSea) ) } )
...
BeginSql Alias cAliasSea
	SELECT 	SEA.EA_FILIAL, SEA.EA_FILORIG, SEA.EA_NUMBOR, SEA.EA_CART, SEA.EA_PREFIXO, SEA.EA_NUM,
				...
				SA2.A2_FBANCO, SA2.A2_FAGENCI, SA2.A2_FDVAGE, SA2.A2_FNUMCON, SA2.A2_FDVCTA, SA2.A2_CGC,
				SA2.A2_BBANCO, SA2.A2_BAGENCI, SA2.A2_BDVAGE, SA2.A2_BNUMCON, SA2.A2_BDVCTA, CASE WHEN SA2.A2_BCGC = '              ' THEN SA2.A2_CGC ELSE SA2.A2_BCGC END CNPJ,
...
oSection2:Cell("A2_CGC"    ):SetBlock( { || Transform(CNPJ, IIF(Len(Alltrim(CNPJ))>11,"@R 99999999/9999-99","@R 999999999-99")) } )

oSection2:Cell("E2_VENCREA"):SetBlock( { || IIF(Empty(SE2->E2_FDTPGTO),SE2->E2_VENCREA,SE2->E2_FDTPGTO) } )

oSection2:Cell("A2_FBANCO"):SetBlock( { || F710BcoFor("A2_FBANCO", cAliasSea) } )
oSection2:Cell("A2_FAGENCI"):SetBlock( { || F710BcoFor("A2_FAGENCI", cAliasSea) } )
oSection2:Cell("A2_FDVAGE"):SetBlock( { || F710BcoFor("A2_FDVAGE", cAliasSea) } )
oSection2:Cell("A2_FNUMCON"):SetBlock( { || F710BcoFor("A2_FNUMCON", cAliasSea) } )
oSection2:Cell("A2_FDVCTA"):SetBlock( { || F710BcoFor("A2_FDVCTA", cAliasSea) } )
...
If (cAliasSea)->EA_MODELO == "45"
	oSection2:Cell("CHAVEPIX"):SetBlock( { || FNRETPIX(2) } )
EndIf
...
If SE2->(DbSeek(xFilial("SE2", (cAliasSea)->EA_FILORIG)+(cAliasSea)->(EA_PREFIXO+EA_NUM+EA_PARCELA+EA_TIPO+EA_FORNECE+EA_LOJA)))
	...
	If (cAliasSea)->EA_MODELO $ "CH/02"
		...
		EndIf
	Else
		cBenef := IIF(!Empty(SA2->A2_BNOME),SA2->A2_BNOME,SA2->A2_NOME)
	Endif
EndIf
...
Case	(cAliasSea)->EA_MODELO $ "CH/02"
	...
	oSection2:Cell("A2_FBANCO" ):Disable()
	oSection2:Cell("A2_FAGENCI"):Disable()
	oSection2:Cell("A2_FDVAGE" ):Disable()
	oSection2:Cell("A2_FNUMCON"):Disable()
	oSection2:Cell("A2_FDVCTA" ):Disable()
...
Case	(cAliasSea)->EA_MODELO $ "CC/01/03/04/05/10/41/43"
	//oSection2:Cell("A6_NREDUZ" ):Enable()
	oSection2:Cell("A2_FBANCO" ):Enable()
	oSection2:Cell("A2_FAGENCI"):Enable()
	oSection2:Cell("A2_FDVAGE" ):Enable()
	oSection2:Cell("A2_FNUMCON"):Enable()
	oSection2:Cell("A2_FDVCTA" ):Enable()
...
Static Function F710BcoFor(cCampo As Char, cAliasSea As Char) As Char
	...
	If cCampo$"A2_FBANCO"
		cRet := If(lBcoTit,SE2->E2_FORBCO,If(!Empty((cAliasSea)->A2_FBANCO),(cAliasSea)->A2_FBANCO,(cAliasSea)->A2_BBANCO))
	Elseif cCampo$"A2_FAGENCI"
		cRet := If(lBcoTit,SE2->E2_FORAGE,If(!Empty((cAliasSea)->A2_FAGENCI),(cAliasSea)->A2_FAGENCI,(cAliasSea)->A2_BAGENCI) )
	Elseif cCampo$"A2_FDVAGE"
		cRet := If(lBcoTit,SE2->E2_FAGEDV,If(!Empty((cAliasSea)->A2_FDVAGE),(cAliasSea)->A2_FDVAGE,(cAliasSea)->A2_BDVAGE) )
	Elseif cCampo$"A2_FNUMCON"
		cRet := If(lBcoTit,SE2->E2_FORCTA,If(!Empty((cAliasSea)->A2_FNUMCON),(cAliasSea)->A2_FNUMCON,(cAliasSea)->A2_BNUMCON) )
	Elseif cCampo$"A2_FDVCTA"
		cRet := If(lBcoTit,SE2->E2_FCTADV,If(!Empty((cAliasSea)->A2_FDVCTA),(cAliasSea)->A2_FDVCTA,(cAliasSea)->A2_BDVCTA) )
	//Elseif cCampo$"A6_NREDUZ"
	//	cBanco := If(lBcoTit,SE2->E2_FORBCO,If(!Empty((cAliasSea)->A2_FBANCO),(cAliasSea)->A2_FBANCO,(cAliasSea)->FIL_BANCO))
	//	cAgenc := If(lBcoTit,SE2->E2_FORAGE,If(!Empty((cAliasSea)->A2_FAGENCI),(cAliasSea)->A2_FAGENCI,(cAliasSea)->FIL_AGENCI) )
	//	cConta := If(lBcoTit,SE2->E2_FORCTA,If(!Empty((cAliasSea)->A2_FNUMCON),(cAliasSea)->A2_FNUMCON,(cAliasSea)->FIL_CONTA) )
