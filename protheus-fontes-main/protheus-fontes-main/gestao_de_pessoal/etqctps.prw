...
User Function ETQCTPS()        // incluido pelo assistente de conversao do AP5 IDE em 05/07/00
...
±±³Equipe RH   ³08/09/05³078027³Ajustar a impressao da expressao "Por Mes"³±±
±±³            ³08/03/06³------³e "Por hora" para Estagiarios Mens/Horista³±±
...
ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ*/
cAcessaSRA	:= &( " { || " + ChkRH( "FB_ETQCT" , "SRA" , "2" ) + " } " )
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
wnrel:="FB_ETQCT"            //Nome Default do relatorio em Disco
...
// 						Fonte   	tam    	Negrito  	Sublinhado
oFont1:= TFont():New( 	"Arial",,	06,,	.T.,,,,,	.F. )

Private oPrn := TMSPrinter():New()
oPrn:SetLandscape()     // ou SetPortrait()
oPrn:setPaperSize(9) 	// 9 é tamanho A4
   ...
   Endif
	...
	While !Eof() .And. SRA->RA_FILIAL == cFil 
		...
		If nTipImp==1
			...
			Else
				cTipPagto	:= 	If(SRA->RA_TIPOPGTO="M",STR0010,STR0028) 			//-- Mensalista  * Semanlista 
			Endif
			...
			nTamanho:=60-Len(STR0013+ALLTRIM(TRANSFORM(SRA->RA_SALARIO,"@E 999,999,999.99"))+" ")	//"Remuneracao R$ "
			...
			aVetor[nAlin,5]:= STR0016+DescFun(SRA->RA_CODFUNC,SRA->RA_FILIAL) + Iif(cPaisLoc=="BRA",STR0017 +  fCodCBO(SRA->RA_FILIAL,SRA->RA_CODFUNC,dDataBase),"") 	//"CARGO "###"  CBO "
			aVetor[nAlin,6]:= " "
			...
			aVetor[nAlin,8]:= STR0020+SRA->RA_REGISTR + "  "+SRA->RA_MAT									//"Registro No. "###" Fls./Ficha "
			aVetor[nAlin,9]:= STR0022+ALLTRIM(TRANSFORM(SRA->RA_SALARIO,"@E 999,999,999.99"))+" "+aLinha[1]	//"Remuneracao R$ "
			aVetor[nAlin,10]:= aLinha[2]
			aVetor[nAlin,11]:= PadR(aLinha[3],40)+" "
			aVetor[nAlin,12]:= " "
		Else
...
//+--------------------------------------------------------------+
oPrn:EndPage()
IF oPrn:Setup() // para configurar impressora
	oPrn:Preview()
...
If (cTipo == "I" .And. nAlin == nColunas) .Or. (cTipo == "F" .And. nAlin > 0)
	...
	For C:= 1 To nCont
		...
		For I:= 1 To nColunas
			IF C == 1
				oPrn:Line (Li, nCol-20, Li, nCol + 800)
				oPrn:Line (Li, nCol-20, Li+(_nAltLin*nCont), nCol - 20)
				oPrn:Line (Li +(_nAltLin*nCont), nCol-20, Li+(_nAltLin*nCont), nCol + 800)
				oPrn:Line (Li, nCol + 800, Li+(_nAltLin*nCont), nCol + 800)
			endif
			oPrn:Say( Li+10, nCol , alltrim (aVetor[I,C]), oFont1)
			nCol += 850
			aVetor[I,C]:= " "
		Next
		Li += _nAltLin
