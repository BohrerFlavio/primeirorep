...
User Function rh_cexp()        // incluido pelo assistente de conversao do AP5 IDE em 11/10/00

	// Programa..: rh_CEXP.PRX
...
Static Function RptDetail()
	...
	Do While !Eof() .and. xFilial() == SRA->RA_FILIAL .And. SRA->RA_MAT <= mv_par02
		...
		li := li + 1
		@ li, 000 PSAY "e "+SRA->RA_NOME+space(10)+"portador da CTPS "+SRA->RA_NUMCP+"/"+SRA->RA_SERCP+" a seguir"
		li := li + 1
		@ li, 000 PSAY "designado apenas EMPREGADO é celebrado o presente CONTRATO DE TRABALHO POR PRAZO
		li := li + 1
		@ li, 000 PSAY "DETERMINADO  que tera vigencia a partir da data de inicio da prestacao de servi-"  
		li := li + 1
		@ li, 000 PSAY "ços de acordo com as condicoes a seguir especificadas:"
		li := li + 2
		...
		li := li + 1
		@ li, 000 PSAY "PREGADORA, para exercer as funcoes de "+SRJ->RJ_DESC+SPACE(01)+"a circunstancia,po-"
		li := li + 1
		@ li, 000 PSAY "rem, de ser especificada, não importa na intransferibilidade do EMPREGADO,   para"
		li := li + 1
		@ li, 000 PSAY "outro serviço, no qual demonstre melhor capacidade de adaptação, desde que compa-"
		li := li + 1
		@ li, 000 PSAY "tível com suas condições pessoais.                                               "
		li := li + 2
		...
		li := li + 1   
		@ li, 000 PSAY "nao de horas - TURNO: "+Left(SR6->R6_DESC,60) 
		li := li + 1                                             
		@ li, 000 PSAY "de segundas as sextas-feiras."
		li := li + 2
		@ li, 000 PSAY "05-O Presente contrato, vigira  durante o periodo de "+DTOC(SRA->RA_ADMISSA)+" a "+DTOC(SRA->RA_VCTOEXP)+", ven-"
		li := li + 1
		@ li, 000 PSAY "cendo-se independentemente de quaisquer interrupções ou suspensões e, findo o pra-             
		li := li + 1
		@ li, 000 PSAY "zo, estará extindo, sem que caiba a qualquer das partes aviso previo ou indenizacao."
		li := li + 2
		...
		li := li + 1
		@ li, 000 PSAY "seu deslocamento, residencia-trabalho e vice-versa, devera solicitar a EMPREGADO-"
		li := li + 1
		@ li, 000 PSAY "RA, por escrito e contra-recibo, o fornecimento de vale-transporte."
		li := li + 2 
		...
		li := li + 2
		@ li, 000 PSAY "12-As partes elegem o  Foro desta cidade como único competente a  dirimir quais-"
		li := li + 1
		@ li, 000 PSAY "quer litigios oriundos do presente contrato."
		li := li + 2
