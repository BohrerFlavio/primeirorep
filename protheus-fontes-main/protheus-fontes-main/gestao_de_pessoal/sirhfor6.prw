...
USER FUNCTION  SIRHFOR6()
	...
	//_nVal105:=fBuscaPD("105")
	//_nVal106:=fBuscaPD("106") 
	...
	if SR3->(DbSeek(xfilial('SR3')+SRA->RA_MAT))
		while SR3->(!eof()) .and. SR3->R3_FILIAL = xfilial('SR3') .and. SRA->RA_MAT = SR3->R3_MAT
			if SR3->R3_TIPO = '006'
				_cTipo := '006'
				//	_nMonth := MONTH(SR3->R3_DATA)
				//_nYear  := YEAR(SR3->R3_DATA)
				exit
			endif
			SR3->(DbSkip())
		enddo
	...
	if  _nHrs992 > 0  
		...
		if _cTipo = '006' .AND. MONTH(SRA->RA_VCTEXP2) == MONTH(ddatabase) .AND. year(SRA->RA_VCTEXP2) == YEAR(DDATABASE)
		...
		fGeraVerba("111",_nValPdr111,_nHrsPdr111,,,"H","E")  	
	endif 
	...
	if  MONTH(SRA->RA_VCTEXP2) == MONTH(ddatabase) .AND. year(SRA->RA_VCTEXP2) == YEAR(ddatabase)
		...
		_nValPdr090 :=	(day(SRA->RA_VCTEXP2))*(((SRA->RA_SALEXPE)/30))
		_nHrsPdr090	:=	day(SRA->RA_VCTEXP2)
