...
/*Fonte padrão correspondente ao Rdmake RECIBO.PRX; Alterações realizadas neste fonte devem ser compatibilizadas também no Rdmake.*/
...
±±³Isabel N.   ³28/08/17³DRHPAG-4969     ³Ajuste: exibir CEI/CNPJ da empresa conforme³±±
±±³            ³        ³                ³cadastro do CFG ou pelo P.E. GPEXINSCR.    ³±±
±±³Jaqueline L ³26/12/17³DRHPAG-9988     ³Ajuste para fazer uma cópia do relatório   ³±±
±±³            ³        ³                ³que esta no diretório informado nomv_relt, ³±±
±±³            ³        ³                ³para não ter problemas ao enviar por e-mail³±±
±±³Jaqueline L ³15/02/18³DRHPAG-13674    ³Correção para verificar o cPath para vir c ³±±
±±³            ³        ³                ³string em Lower e adicionado em uma varíavel³±±
±±³            ³        ³                ³ppis o sistema n estava reconhecendo direto ³±±
±±³Jonathan Glz³17/04/18³DMINA-248       ³Se realiza replica de localización de COL  ³±±
±±³            ³        ³                ³pues esta solo se encuentra en version 11.8³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function DTI111(lTerminal,cFilTerminal,cMatTerminal,cProcTerminal, nRecTipo, cPerTerminal, cSemanaTerminal, lMeuRH)
...
nTipRel    := IF( !( lTerminal ), mv_par05,IIF(!lMeuRH,3,4) )	//Tipo de Recibo (Pre-Impressão/Zebrado/EMail/PDF)
...
If nTipRel == 4 .And. !lMeuRH //Apenas visualizar o PDF
	lPDFEmail	:= .F.
EndIf
...
IF nTipRel==3
	...
	EndIF
ElseIf nTipRel==4 .And. lMeuRH
	dbSelectArea("SRA")
	dbSetOrder(1)
	dbSeek( cFilTerminal + cMatTerminal )

	cHtml := R030Imp(.F.,wnRel,cString,cMesAnoRef,lTerminal,.T.)
	If Select("SRC")>0
		SRC->(DbCloseArea())
	EndIf
Else
...
*/
Static Function R030Imp(lEnd,WnRel,cString,cMesAnoRef,lTerminal,lMeuRH)
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Define Variaveis Locais (Basicas)                            ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local aCodBenef   	:= {}
...
If lTerminal
	...
	If !Eof()
		While RCH->( !Eof() .and. &(cInicio) == &(cKey))
            //13 salario ou Valores extras não exigem outros períodos em aberto
			If Empty(RCH->RCH_DTFECH) .or. cTipoRot $ "5,6,EXT"
...
While SRA->( !Eof() .And. &cInicio <= cFim )
	...
	If cSitFunc == "D" .And. ( (!Empty(SRA->RA_DEMISSA) .And. MesAno(SRA->RA_DEMISSA) > MesAno(dDtPesqAf)) .Or. ( cRoteiro == fGetCalcRot("F") .And. GetMvRH("MV_PLRDEM",,"N") == "S" ) )
		cSitFunc := " "
	Endif

	//-Busca o Salario Base do Funcionario
	nSalario := fBuscaSal(dDataRef,,,.F.)
	If nSalario == 0 .And. !(fRetSR7(dDataRef, dDataRef, .T.) == "C")
	...
	If cRoteiro <> "EXT"
		For nReg := 1 to Len(aVerbasFunc)
			...
			If cPaisLoc $ "ANG*PER*COL" .AND. cBaseAux = "N"
	...
	ElseIf nTipRel == 2 .and. !( lTerminal )
		...
		ASize(ADesco,0)
		ASize(aBases,0)
	ElseIf nTipRel == 3 .or. (lTerminal .And. !lMeuRH)
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		...
		//Endif
	ElseIf (nTipRel == 4 .OR. nTipRel == 5) .and. (!( lTerminal ) .Or. lMeuRH) //4 Visualizar o PDF 5 Enviar PDF por email
		fImprPDF(lPDFEmail,cPath, lMeuRH)
		ASize(AProve,0)
		ASize(ADesco,0)
		ASize(aBases,0)
	Endif
...
@ PROW(),PCOL() PSAY If(lIsDriver, &cNormal, "")
LI ++
@ LI,01 PSAY DESC_Fil
LI ++
@ LI,01 PSAY DESC_END
...
If !Empty(Semana) .And. Semana # '99' .And.  Upper(SRA->RA_TIPOPGT) == 'S'
	...
	cSem_Ate + ')'
Else
	@ LI,20 PSAY "- " + AllTrim(cDescRot) + " - " + MesExtenso(MONTH(dDataRef))+"/"+STR(YEAR(dDataRef),4) + " - " + Semana
EndIf
...
Else
	@ LI,53 PSAY PADC(ALLTRIM(SRA->RA_CC),12)
EndIf
@ LI,73 PSAY ORDEM_REL PICTURE "99"
LI ++

cDet := STR0015 + cCodFunc		//'FUNC: '
cDet += Left( cDescFunc, 30 ) + " "
cDet += PadC( AllTrim( DescCc(SRA->RA_CC,SRA->RA_FILIAL) ), 15 )
...
//Busca Centro de Custo do Funcionario na Data Referencia
fBuscaCC(dDataRef, @cCodCC, @cDescCC, Nil, .F. , .T. )
...
If cPaisLoc == "BRA"
	If lUseCEI
		@ LI,92+7  PSAY STR0215 + Desc_CGC	//"CEI: "
	Else
...
LI ++
@ LI,00 PSAY STR0020 + cCodCC + " - " + cDescCC		//"| C Custo   : "
If !Empty(Semana) .And. Semana # "99" .And.  Upper(SRA->RA_TIPOPGT) == "S"
...
@ LI,33  PSAY STR0024 + If( lLpt1, fTAcento(SRA->RA_NOME), SRA->RA_NOME )	//"Nome  : "
@ LI,99  PSAY STR0025						//"Ordem : "
@ LI,107 PSAY StrZero(ORDEMZ,4) Picture "9999"
@ LI,131 PSAY "|"
...
oPrint:Say(2180,0200,SRA->RA_BCDEPSA+"-"+SUBSTR(DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T.),1,44),oArial08N)
...
@ LI,40 PSAY Alltrim(SRA->RA_BCDEPSA) + "-" + DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T.)
Li +=2
...
@ LI,35 PSAY STR0072
@ LI,70 PSAY STR0073 + Alltrim(SRA->RA_BCDEPSA) + "-" + DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T.)
@ LI,131 PSAY "|"
@ ++LI,00 PSAY "|"+REPLICATE("-",130)+"|"
...
If cPaisLoc <> "ARG"
	...
	if(lImprBases)
		...
		@ LI,aPos[2] PSAY STR0033+SRA->RA_BCDEPSA+"-"+ Substr(DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T.),1,25)	//"| CREDITO:"
		@ LI,aPos[4] PSAY STR0034 + SPACE(05)+TRANSFORM((TOTVENC-TOTDESC) , cPict3 )			//"| LIQUIDO A RECEBER     "
		@ LI,131 PSAY "|"
	else
		@ LI,aPos[1]- 2 PSAY STR0033+SRA->RA_BCDEPSA+"-"+ Substr(DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T.),1,25)	//"| CREDITO:"
...
cRodape := Transform( nSalario, cPict2 )
If cTipoRot == "2"  // Bases de Adiantamento
	If cBaseAux == "S" .And. nBaseIr != 0
		cRodape += Space(37) + Transform( nBaseIr, cPict2 )
	Endif
ElseIf cTipoRot == "1" .Or. cTipoRot == "6"  .Or. cTipoRot == "9"// Bases de Folha e 13o. 2o.Parc.
	If cBaseAux == "S"
		cRodape += Transform( nAteLim, cPict2 )
		cRodape += If( nBaseFgts != 0, Transform( nBaseFgts , cPict2 ), Space(12) )
		cRodape += If( nFgts	 != 0, Transform(	nFgts	, cPict3 ), Space(11) )
		cRodape += If( nBaseIr	 != 0, Transform(	nBaseIr , cPict2 ), Space(11) )
		cRodape += Transform( nBaseIrfE, cPict3 )
	Endif
ElseIf cTipoRot == "5" // Bases de FGTS e FGTS Depositado da 1¦ Parcela
	If cBaseAux == "S" .And. nBaseFgts != 0
		cRodape += Space(13) + Transform( nBaseFgts, cPict2 )
		cRodape += If( nFgts != 0, Transform( nFgts, cPict3 ), "" )
	Endif
Endif
@ LI,01 PSAY cRodape
...
IF SRA->RA_BCDEPSA # SPACE(8)
	Desc_Bco := If( lLpt1, fTAcento(DescBco( SRA->RA_BCDEPSA, SRA->RA_FILIAL, , .T.)), DescBco( SRA->RA_BCDEPSA, SRA->RA_FILIAL, , .T.))
...
IF !( lTerminal )
	...
	If Empty(cEmail)
	    ...
	    EndIf
		Aadd( aLog, { PADR( SRA->RA_FILIAL, FWGETTAMFILIAL) + " - " + PADR(SRA->RA_MAT, TamSx3("RA_MAT")[1]) + " - " + PADR( SRA->RA_NOME, 45) + " - " + OemToAnsi(STR0211) } ) //"Sem e-mail cadastrado e/ou nao recebe e-mail (verif. campos: RA_EMAIL/RA_RECMAIL)"
		Return
	Endif
...
IF !( lTerminal )
	...
	cHtml +=    '&nbsp;&nbsp;&nbsp;&nbsp;' + STR0049 + SRA->RA_BCDEPSA+" - "+DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T.)+ '&nbsp;'+  SRA->RA_CTDEPSA //"Bco/Conta - "
	cHtml += '</p></font>'
	...
	ELSE //Cabecalho utilizado pela localizacao Peru
		...
		cHtml +=	'&nbsp;&nbsp' + STR0049 + SRA->RA_BCDEPSA+"-"+DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T.)+ '&nbsp;'+  SRA->RA_CTDEPSA //"Bco/Conta - "
		cHtml += '</b></p></font>'
		cHtml += '<!Proventos e Desconto>'
		cHtml += '<div align="center">'
		cHtml += '<Center>'
...
If cTipoRot = "2"
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
ElseIf (cTipoRot = "1" .Or. cTipoRot = "6"  .Or. cTipoRot == "9") .And. cPaisloc == "BRA"
...
cHtml +=					"<P align=left>" + CRLF
If lUseCEI
	cHtml +=						"<span class='etiquetas'>" + STR0215 + CRLF	//"CEI:"
	cHtml +=						"</span>" + CRLF
	cHtml +=						"<span class='dados'>" + CRLF + Desc_CGC + CRLF
Else
...
cHtml +=						"</span>" + CRLF
cHtml +=						"<span class='dados'>" + CRLF
cHtml +=								Alltrim(SRA->RA_BCDEPSA) + "-" + DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T.) + CRLF
cHtml +=						"</span>" + CRLF
cHtml +=					"</P>" + CRLF
cHtml +=				"</TD>" + CRLF
cHtml +=			"</TR>" + CRLF
...
LI ++
@ LI,00 PSAY STR0020 + cCodCC + Space(9) + " - "+ Space(4) + STR0069 + "  :" + Space(1) + cDescCC	//"| C Custo   : " "Desc. CC.:
@ LI,131 PSAY "|"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function fImprPDF(lPDFEmail,cPath, lMeuRH)
Local nConta 		:= 0
Local cFile		:= ""
Local cEmail		:= ""
Local cIdentUni	:= ""
...
If lPDFEmail .And. !lMeuRH
	...
	If Empty(cEmail)
		...
		Return()
	Else
		aAdd(aEmail, {{Alltrim(SRA->RA_NOME), cEmail,If(SRA->RA_RECMAIL=="S",.T.,.F.)},Alltrim(SRA->RA_FILIAL),Alltrim(SRA->RA_MAT),dtos(dDataRef),cIdentUni})
    EndIf
ElseIf lMeuRH
	cPath := GetSrvProfString("STARTPATH","")
	oPrinter	:= FWMSPrinter():New(AllTrim(SRA->RA_FILIAL) + "_" + Alltrim(SRA->RA_MAT)  ,6,.F.,cPath,.T.,,,,.T.,.F.)
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³fCabecZCol   ³ Autor ³ Alfredo Medrano  ³ Data ³ 18.10.2013 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ IMRESSAO Cabe‡alho Form ZEBRADO                            ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Sintaxe   ³ fCabecZCol()                                               ³±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function fCabecZCol()   // Cabecalho do Recibo Zebrado
Local cCodFunc		:= ""		//-- codigo da Funcao do funcionario
Local cDescFunc	:= ""		//-- Descricao da Funcao do Funcionario
Local cTipoSal		:= ""		//-- Tipo Salario
Local nSuelSal		:= 0		//-- Sueldo o Salario
Local cEtiqueta	:= ""		//-- Etiqueta Sueldo o Salario
Local cDescRot		:= PosAlias("SRY", cRoteiro, SRA->RA_FILIAL, "RY_DESC")
Local cDescProc	:= PosAlias("RCJ", cProcesso, SRA->RA_FILIAL, "RCJ_DESCRI")
Local cDescCent	:= fDesc("RGC", SRA->RA_KEYLOC, "RGC->RGC_DESLOC", 50)
Local cNombreC		:= substr(SRA->RA_NOMECMP,0,50)
/*
ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
  ³ Carr ega Funcao do Funcion. de acordo com a Dt Referencia     ³
  ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ*/
fBuscaFunc(dDataRef, @cCodFunc, @cDescFunc   )
...
LI ++
@ LI,00  PSAY STR0018 + SRA->RA_FILIAL + DESC_Fil		//"| Empresa   : "
@ LI,106 PSAY STR0025						//"Ordem : "
@ LI,115 PSAY StrZero(ORDEMZ,4) Picture "9999"
@ LI,131 PSAY "|"


If SRA->RA_TIPOSAL $ '1|3'
			cTipoSal := STR0188 // Básico

			cEtiqueta:= STR0190//Sueldo:
Else//If SRA->RA_TIPOSAL $ '2|4'
			cTipoSal := STR0189 // Integral
			// := SRA->RA_SALBAS
			cEtiqueta:= STR0191 //Salario:
EndIf
nSuelSal := SRA->RA_SALARIO
LI ++
ORDEMZ ++
@ LI,000  PSAY STR0023 + SRA->RA_MAT		//"| Matricula : "
@ LI,023  PSAY padr(STR0024 + If( !Empty(SRA->RA_NOMECMP),cNombreC,SRA->RA_NOME ),61,"")	//"Nome  : "
@ LI,085  PSAY STR0187 + cTipoSal	 //Tipo Sal:
@ LI,105  PSAY cEtiqueta + Transform(nSuelSal, "@E 999,999,999.99")
@ LI,131  PSAY "|"

LI ++
@ LI,00	 PSAY padr(STR0020 + SRA->RA_CC + " - " + DescCc(SRA->RA_CC,SRA->RA_FILIAL),38,"")	//"| C Custo   : "
@ LI,40  PSAY padr(STR0182 + cCodFunc+" - "+alltrim(cDescFunc),52,"")  //"Funcion   : "
@ LI,104 PSAY STR0183 + DTOC(SRA->RA_ADMISSA)									//"Ingreso : "
@ LI,131 PSAY "|"

LI ++
@ LI,00  PSAY padr(STR0184 + cDescCent,40,"")										//" C. Trabajo: "
@ LI,40  PSAY STR0132 + SRA->RA_DEPTO + " - " + Trim(PADR(fDesc('SQB',SRA->RA_DEPTO,'QB_DESCRIC'),30,""))	//-9caracteres+30caracteres
@ LI,90  PSAY STR0185 + SRA->RA_TIPOPRC	     //Tipo Ret:
If SRA->RA_TIPOPRC == "2"
	@ LI,107  PSAY STR0186 + AllTrim(str(SRA->RA_PERCIR)) + "%"     //% Ret:
EndIf
@ LI,131 PSAY "|"

//| Proceso  :/Proced.: /Periodo: /Nº Pago:
LI ++

@ LI,00  PSAY STR0117 +" "+ cProcesso + Substr(cDescProc,0,15)
@ LI,40  PSAY padr(STR0118 + cRoteiro + SPACE(5) + cDescRot,44,"")
@ LI,85  PSAY STR0119 + cPeriodo
@ LI,105 PSAY STR0120 + Semana
@ LI,131 PSAY "|"

LI ++
@ LI, 00  PSAY "| " + STR0192 +  SRA->RA_CIC
@ LI,131 PSAY "|"
...
nLi += 20
oPrinter:SayBitmap(nLi,004,cStartPath+"lgrl"+".bmp",080,040) // Tem que estar abaixo do RootPath
oPrinter:Say(nLi,80,Space(3)+Substr(STR0018, 3) +  DESC_Fil,oFont2N)	//"| Empresa   : "

If cPaisLoc == "BRA"
	If lUseCEI
		oPrinter:Say(nLi, 450, STR0215 + " " + Desc_CGC, oFont2N)	//"CEI: "
	Else
...
oPrinter:Say(nLi, 80,Space(3)+Substr(STR0020, 3) + SRA->RA_CC + " - " + cDescCC,oFont2N)	//"| C Custo   : "
...
nLi += 10
oPrinter:Say(nLi,80,Space(3)+Substr(STR0023,3) + SRA->RA_MAT + "   " + STR0024 + If( lLpt1, fTAcento(SRA->RA_NOME), SRA->RA_NOME ),oFont2N)	//"| Matricula : " XXXXX +"   " + Nome  : " XXXXXXXXXXXXXXX
oPrinter:Say(nLi,447,STR0019 + SRA->RA_FILIAL,oFont2N)	//" Local : "

nLi += 10
oPrinter:Say(nLi,80,Space(3)+Substr(STR0026,3)+cCodFunc+" - "+If( lLpt1, fTAcento(cDescFunc), cDescFunc ),oFont2N)	//"| Funcao    : "
If !Empty(SRA->RA_DEPTO)
	oPrinter:Say(nLi,260,STR0177 + ": " + SRA->RA_DEPTO + " - " + fDesc('SQB',SRA->RA_DEPTO,'QB_DESCRIC'),oFont2N)	//-9caracteres+30caracteres
EndIf
//oPrinter:Say(nLi,448,STR0019 + SRA->RA_FILIAL,oFont2N)	//" Local : "
ORDEMZ ++
oPrinter:Say(nLi,450,STR0025 + StrZero(ORDEMZ,4),oFont2N)	//"Ordem : "
nLi += 10

oPrinter:Say(nLi,80,Space(2) + STR0213 + "  " + Alltrim(Transform(nSalario,cPict2)),oFont2N)	//Salario
...
Local nLi			:= nPosFCab
	...
	For nCont := 1 To nTermina
		...
		IF nCont <= LEN(aProve)
			...
			oPrinter:SayAlign(nLi,aPos[2],TRANSFORM(aProve[nCont,3] , cPict3 ),oFont1,188,005,,ALIGN_H_RIGHT)
		ENDIF
		//Imprime Descontos
		IF nCont <= LEN(aDesco)
	...
	oPrinter:SayAlign(nLi-10,aPos[4],Space(2)+Substr(STR0033,3)+ Space(2) + AllTrim(SRA->RA_BCDEPSA)+" - "+substr(DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL, , .T. ),1,25),oFont2,196,005, , 0, 1 )  	//"| CREDITO:"
...
Private cMailSenha	:= NIL
	...
	For nI := 1 To Len(aEmail)
		...
		If File(cPathTmp+cFile+".rel")	// com lInJob = .F. o arquivo .rel fica na temp
