...
User Function rh_avp()        // incluido pelo assistente de conversao do AP5 IDE em 23/08/00

	//* Programa..: rh_avp.PRX
	//* Autor.....: Claudioir Macedo
	//* Data......: 10/04/02
	//* Nota......: Emissao Relatorio Aviso Previo indenizado
	...
	cDesc2  :="de Aviso Previo Indenizado"
	cDesc3  :=""
	tamanho :="P"
	...
	nLastKey:=0
	cPerg   :="RH_AVP"
	titulo  :="Aviso Previo Indenizado"
	wnrel   :="rh_AVP"
	nTipo   :=0
	nLin    :=0
	nTamNf  :=32
...
Static Function RptDetail()
	...
	DbselectArea("RCE")   
	_xfim:=Chr(10)+Chr(13) //chr: Enter 7 
	IF RCE->RCE_MESDIS == SUBSTR(Dtos(mv_par03+30),05,02) //dtos
		//mSginfo("error")
		mSginfo(+space(12)+"ATENCAO!"+_Xfim+; 
		""+_Xfim+;
		"DIAS DE AVISO PREVIO DENTRO DO MES ANTERIOR AO DISSÍDIO!")
	EndIf
	...
	Do While !Eof() .and. xFilial() == SRA->RA_FILIAL .And. SRA->RA_MAT <= mv_par02
		...
		li := li + 2
		@ li, 025 PSAY "         (INDENIZADO)"
		li := li + 3
		@ li, 000 PSAY "De....:"+Left(SM0->M0_NOMECOM,30) +Space(10)+"     CGC...:"
		...
		li := li + 2
		@ li, 004 PSAY "Pela presente, levamos ao conhecimento de V.Sa. que nao mais convindo a esta"
		li := li + 1
		@ li, 000 PSAY "empresa manter seu Contrato de  Trabalho, vimos por  meio deste, rescindi-lo, na"
		li := li + 1
		@ li, 000 PSAY "forma da legislacao  pertinente,  devendo  V.Sa. cessar  suas atividades em"
		@ li, 075 PSAY +str(day(mv_par03),2,0)+" de"
		li := li + 1
		@ li, 000 PSAY mesextenso(month(mv_par03))+" de "+str(year(mv_par03),4,0)+"."
		li := li + 2
		@ li, 004 PSAY "Esclarecemos que o presente nao implica em prejuizo no pagamento de todos os"
		li := li + 1
		@ li, 000 PSAY "seus  direitos legais."
		li := li + 2
		@ li, 004 PSAY "Agradecemos a cooperacao prestada por V.Sa. ate a presente data, e solicita-"
		li := li + 1
		@ li, 000 PSAY "mos a devolucao deste aviso com o seu CIENTE."
		li := li + 4
