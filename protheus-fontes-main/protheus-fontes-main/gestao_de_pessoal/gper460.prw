...
User FUNCTION _GpeR460(__lAnotacoes)
...
Local cCabCpo,cCampo,cFolder,cOrdem,cAux,i
Local nSal 		:= 0
Local lPrint 	:= .F.
...
For i := 1 To Len(_aCpoSX3)
	...
	uCampo := GetSx3Cache(_aCpoSX3[i], 'X3_CAMPO')		//SX3->X3_CAMPO
	...
	If lFicha 
		...
		If lPrint
			IF ( GetSx3Cache(_aCpoSX3[i], 'X3_CONTEXT') == "V"  ) 
				...
				Else
					cCampo := CriaVar(GetSx3Cache(_aCpoSX3[i], 'X3_CAMPO'))
				Endif
			Else
			...
			Elseif ValType(cCampo) = "N"
				cPict := Alltrim(GetSx3Cache(_aCpoSX3[i], 'X3_PICTURE'))
				If ! lAnotacoes
			...
			EndIf
			aAdd(aCampo,{cCabCpo,cCampo,cFolder,cOrdem,GetSx3Cache(_aCpoSX3[i], 'X3_TIPO')})
		EndIf
	Else
		If Alltrim(uCampo)$ 'RA_NASC | RA_PAI | RA_MAE | RA_NACIONA | RA_NATURAL | RA_NUMCP | RA_SERCP | RA_PIS | RA_ADMISSA | RA_CARGO | RA_DCARGO | RA_CODFUNC | RA_DESCFUN | RA_SALARIO | RA_TNOTRAB | RA_DESCTUR'
			If SRA->(FieldPos(uCampo)) >0
				cCabCpo := GetSx3Cache(_aCpoSX3[i], 'X3_TITULO')
				dbSelectArea("SRA")
				...
				ElseIf ValType(cCampo)="L"
					...
					Endif
				Endif
				cFolder := GetSx3Cache(_aCpoSX3[i], 'X3_FOLDER')
				cOrdem  := GetSx3Cache(_aCpoSX3[i], 'X3_ORDEM')
...
For N:=1 To Len(aSal)
	...
	Else
		...
		//ImprG(aSal[n,6],"C",,, 1325,.f.,nLin,oFont08)	// Detalhe campo ( Valor )  Flávio comentou para não aparecer no relatório
...
IF nLinha == 0.00
	...
	For nCb := 1 to wCabec0-1
		IF ValType((cWCabec := "wCabec"+Alltrim(Str(nCb)))) != "U"
			cDetCab := &(cWCabec)
			aAdd(aCabec,cDetCab)
		EndIF
	Next nCb
