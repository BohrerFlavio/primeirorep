...
User Function rh_pdm()        // incluido pelo assistente de conversao do AP5 IDE em 23/08/00

	//* Programa..: ML_PDM.PRX
	//* Autor.....: Claudioir Macedo     
	//* Data......: 10/04/02
	//* Nota......: Emissao Relatorio Pedido de Demissao c/ dispensa do Aviso Previo
	...
	cDesc2  :="de Pedido de Demissao com Dispensa de Aviso Prévio."
	cDesc3  :=""
	tamanho :="P"
...
Static Function RptDetail()
	...
	Do While !Eof() .and. xFilial() == SRA->RA_FILIAL .And. SRA->RA_MAT <= mv_par02
		...
		Endif
		li := li + 10
		@ li, 023 PSAY "P E D I D O    D E    D E M I S S A O"
		li := li + 2
		@ li, 023 PSAY "      DISPENSA DO AVISO PREVIO"
		li := li + 4
		...
		li := li + 2
		@ li, 004 PSAY "Pela presente, e por motivos particulares, venho apresentar a V.Sa. em cara-"
		li := li + 2
		@ li, 000 PSAY "ter definitivo o meu Pedido de Demissao do emprego que ocupo nesta empresa desde"
		li := li + 2
		@ li, 000 PSAY +str(day(SRA->RA_ADMISSA),2,0)+" de "+mesextenso(month(SRA->RA_ADMISSA))+" de  "+str(year(SRA->RA_ADMISSA),4,0)+"."
		li := li + 3
		@ li, 004 PSAY "Tendo interesse em retirar-me do servico imediatamente, solicito de  V.Sas."
		li := li + 2
		@ li, 000 PSAY "a dispensa de  quaisquer formalidades, especialmente do Aviso  Previo Legal, bem"
		li := li + 2
		@ li, 000 PSAY "como sua remuneracao."
		li := li + 3
