...
±±ºPrograma  ³GJF198  º Autor ³ Giuliano Forgiraini  º Data ³ 30/01/2015  º±±
...
User Function GJF198() 
	...
	_oEmp1  := 'Quantidade a empenhar(kg): 
	...
	oSayEmp2 :=  tSay():New(170, 370,{|| _oEmp2 },oDlg2,,oFont,,,,.T.,CLR_HBLUE,CLR_HBLUE,200,30) 
	oSayEmp3 :=  tSay():New(170, 370,{|| _oEmp3 },oDlg2,,oFont,,,,.T.,CLR_HRED,CLR_HRED,200,30) 
	oSayEmp1 :=  tSay():New(170, 420,{|| _oEmp4 },oDlg2,,oFont,,,,.T.,,,200,30) 
	...
	While TMP->(!eof())
		...
		if !empty(TMP->ZAS_OK) 	
			...
			if ZAS->(DbSeek(xfilial('ZAS')+TMP->ZAS_CONTRO))
				reclock('ZAS',.f.)
				ZAS->ZAS_PREPOR := ZAR->ZAR_NUM 
				msunlock()

				u_obit02(ZAS->ZAS_CONTRO, retCodUsr(), ZAS->ZAS_LOCAL, ZAS->ZAS_LOCALI, "Empenho para PPP "+alltrim(ZAS->ZAS_PREPOR), ZAS->ZAS_PALLET, "ZAS", alltrim(FUNNAME()))
...
Static Function GeraTMP()
	...
	//Procedencia
	if mv_par03 = 1
		cQuery += " ZAS_TIPO = 'MP' AND ZAS_TERC = ' ' AND "
	elseif mv_par03 = 2
