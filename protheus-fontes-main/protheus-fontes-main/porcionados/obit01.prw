...
/*/{Protheus.doc} obit01
Relatório de estoque com % de gordura
@type function
@author André R. Lerner
@since 10/29/2023
/*/
User Function obit01()
	...
	//Local cPict         := ""
	Local titulo         := "Relatório de gordura"
	...
	Private limite       := 80
	Private tamanho      := "M"
	Private nomeprog     := "OBIT01" // Coloque aqui o nome do programa para impressao no cabecalho
	Private nTipo        := 18
	Private aReturn      := { "Zebrado", 1, "Administracao", 2, 2, 1, "", 1}
	Private nLastKey     := 0
	Private cPerg   	 := "OBIT01"
	//Private cbtxt      	 := Space(10)
	Private cbcont     	 := 00
	Private CONTFL     	 := 01
	Private m_pag      	 := 01
	Private wnrel      	 := "OBIT01" // Coloque aqui o nome do arquivo usado para impressao em disco
	pergunte(cPerg,.F.)
	...
	if mv_par03 <= 3
		...
		_cQuery1 += " FROM " + RetSQLTab('ZAS') + " "
		_cQuery1 += " INNER JOIN " + RetSQLTab('SB1') + " ON B1_COD = ZAS_COD AND B1_RELGORD = 'S' AND " + RetSQLDel('SB1')
		_cQuery1 += " WHERE " + RetSQLFil('ZAS')
		_cQuery1 += " AND ZAS_DTPROD BETWEEN '" + dtos(mv_par01) + "' AND '" + dtos(mv_par02) + "'"
		_cQuery1 += " AND ZAS_DATAS = '' AND ZAS_HORAS = '' "
		_cQuery1 += " AND ZAS_PERCCX BETWEEN " + STR(mv_par09) + " AND " + STR(mv_par10) + " "
		...
		_cQuery1 += " INNER JOIN " + RetSQLTab('SB1') + " ON B1_COD = Z8_COD AND B1_RELGORD = 'S' AND " + RetSQLDel('SB1')
...
Static Function buscaCaix(_cod)
	...
	if mv_par03 < 4
		...
		_cQuery2 += " AND ZAS_PERCCX BETWEEN " + STR(MV_PAR09) + " AND " + STR(MV_PAR10)
				/*criar uma forma de leitura para identificar em qual range a caixa está...*/
		if !empty(mv_par07)
