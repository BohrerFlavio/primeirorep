...
Static Function ReportDef()
	...
	TRCell():New(oSection1,"B1_CONSUMO"	,"SB1", ,  ,12,,)
	...
	TRCell():New(oSection3,"dDtMov"		,"   ",STR0038+CRLF+STR0039,"@D"  			,nTamData	,/*lPixel*/,/*{|| code-block de impressao }*/)
	...
	TRCell():New(oSection3,"D1_CONTA"	,"SD1", ,  ,12,,)
	TRCell():New(oSection3,"D1_TOTAL"	,"SD1", ,  ,12,,)
	...
	TRCell():New(oSection3,"nSALDCus"	,"   ",STR0050+CRLF+STR0051	,cPicB2Tot		,cTamB2Tot	,/*lPixel*/,/*{|| code-block de impressao }*/)
...
Static Function ReportPrint(oReport,oSection)
	...
	If mv_par07 == 1
		...
		cUnion += ", SB1EXS.B1_CONSUMO"   //COSTUMISADO
		cUnion += ", ''"						//COSTIMUSADO
		cUnion += ", 0"						//COSTUMISADO
		cUnion += ", ''"					// 08
		cUnion += ", ''"					// 09
		cUnion += ", ''"					// 10
		cUnion += ", ''"					// 11
	...
	BeginSql Alias cAliasTop
		...
		SB1.B1_POSIPI, 			//-- 07
		SB1.B1_CONSUMO,	  		//CUSTOMIZADO
		D1_CONTA,			  		//CUSTOMIZADO
		D1_TOTAL,			  		//CUSTOMIZADO
...
Static Function R910Imp(lEnd,WnRel,tamanho,titulo)
	...
	PRIVATE cabec1  := "Operacao             Documento C. Contábil       Total     |          Entradas           | Custo Médio | Saídas | Saldo | P.V, For.,"
	//PRIVATE cabec2  := IIF(cPaisLoc == "CHI",STR0032,STR0012)
	PRIVATE cabec2  := "Data       TES C.F.   Numero                               | Quantidade      Custo Total | Do Movimento | Quantidade		Custo Total | Quantidade		Valor Total | Cc, Pj ou OP
	...
	If !(TcSrvType()=="AS/400") .And. !("POSTGRES" $ TCGetDB())
		If lImpLivro
			...
			cQueryD1P += ", SB1.B1_CONSUMO"				// CUSTOMIZADO
			...
			cQueryD1P += ", D1_CONTA CONTA"		// CUSTOMIZADO
			cQueryD1P += ", D1_TOTAL TOTAL"		// CUSTOMIZADO
			...
			cQueryD2P += ", SB1.B1_CONSUMO" //CUSTOMIZADO
			...
			cQueryD2P += ", D2_CONTA AS CONTA"	//CUSTOMIZADO
			cQueryD2P += ", D2_TOTAL AS TOTAL"	//CUSTOMIZADO
			...
			cQueryD3P += ", SB1.B1_CONSUMO" //CUSTOMIZADO
			...
			cQueryD3P += ", D3_CUSTO1 AS CUSTO1"		// CUSTOMIZADO
			cQueryD3P += ", D3_OP AS OP"		// CUSTOMIZADO
			...
			If mv_par07 == 1
				...
				cQuery += ", SB1EXS.B1_CONSUMO"		// CUSTOMIZADO
				cQuery += ", ''"					// 08
				cQuery += ", ''"					// 09
				cQuery += ", ''"					// 10
				cQuery += ", ''"					// 11
				...
				While !Eof() .And. (cAliasTop)->PRODUTO = cProdAnt
					...
					ElseIf Alltrim((cAliasTop)->ARQ) == "SD3"
						...
						If mv_par17 == 2 .And. Substr((cAliasTop)->CF,3,1) == "4"
							...
							aDadosTran:={(cAliasTOP)->TES,(cAliasTOP)->QUANTIDADE,(cAliasTOP)->CUSTO,(cAliasTOP)->QUANT2UM,(cAliasTOP)->TIPO,;
							(cAliasTOP)->DATA,(cAliasTOP)->CF,(cAliasTOP)->SEQUENCIA,(cAliasTOP)->DOCUMENTO,(cAliasTOP)->PRODUTO,;
							(cAliasTOP)->OP,(cAliasTOP)->PROJETO,(cAliasTOP)->CC,(cAliasTOP)->ARQ, (cAliasTOP)->CONTA, (cAliasTOP)->TOTAL}//escreve os itens
							dbSkip()                                                                                //15         16
							If (cAliasTOP)->(PRODUTO+SEQUENCIA+ARMAZEM) == cNumSeqTr
								dbSkip()
								Loop
							Else
								...
								EndIf
								If lFirst
									@Li ,000 PSay STOD(aDadosTran[6]) //escreve a data
									@Li ,011 PSay aDadosTran[1]  //escreve o tes
									If ( cPaisLoc=="BRA" )
									...
									_cTotal := transform(aDadosTran[16],'@E 9,999,999.9999999')
									@Li , 020 PSay PadR(IIf(mv_par09 $ "Ss",aDadosTran[8],aDadosTran[9]),12)+ aDadosTran[15] + _cTotal+" |"//escreve numero da nota
					...
					If lFirst .And. !lContinua
						...
						_cTotal := transform(TOTAL,'@E 9,999,999.9999999')						
						@Li , 020 PSay PadR(IIF(mv_par09 $ "Ss",SEQUENCIA,DOCUMENTO),12)//+ " |"
						if len(alltrim(str(CONTA))) < 10
							@Li , 031 PSay replicate('-',10) + _cTotal
						else
							@Li , 018 PSay iif(!empty(str(CONTA)),str(CONTA),SPACE(10)) + _cTotal //+ " |"   //Fabian Maurer	
						endif
						//@Li , 018 PSay iif(!empty(str(CONTA)),str(CONTA),SPACE(10)) + _cTotal //+ " |"   //Fabian Maurer 
						@Li ,059 PSay "|"
					EndIf
					...
					Do Case
						...
						lDev:=MTR910Dev(cAliasTop)
						If (cAliasTOP)->TES <= "500" .And. !lDev
							@Li ,062 PSay (cAliasTOP)->QUANTIDADE //Picture cPicD1Qt  //38
							@Li ,077 PSay (cAliasTOP)->CUSTO //Picture cPicD1Cust
							@Li ,090 PSay "|"
							If (cAliasTOP)->TIPONF != "C"
								@Li ,093 PSay (cAliasTOP)->CUSTO / (cAliasTOP)->QUANTIDADE //Picture cPicB2Cust
							EndIf
							@Li ,104 PSay "|"
						...
						Else
							@Li ,095 PSay "|"
							If (cAliasTOP)->TIPONF != "C"
								@Li ,0100 PSay (cAliasTOP)->CUSTO / (cAliasTOP)->QUANTIDADE // Picture cPicB2Cust
							EndIf
							@Li ,104 PSay "|"
							If lDev
...
Static Function MR910ImpCb(aSalAtu,nCusMed,Li,cPicB2Qt,cPicB2Cust, cAliasTop,cPicB2Tot) 
	...
	If !(TcSrvType()=="AS/400") .And. !("POSTGRES" $ TCGetDB())
		If ! lVEIC
			...
			@ Li,010 PSay SubStr((cAliasTop)->B1_DESC,1,30)
			@ Li,042 PSay STR0023+(cAliasTop)->B1_UM			//"UM : "
			@ Li,055 PSay STR0024+(cAliasTop)->TIPO	   		//"TIPO : "
			@ Li,068 PSay STR0025+(cAliasTop)->B1_GRUPO		//"GRUPO : "			
			@ Li,083 PSay 'Cta. Contabil: '+(cAliasTop)->B1_CONSUMO//Consumo			
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function SI_910CUnf(lCusUnif)
